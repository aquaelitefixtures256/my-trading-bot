...
def _kyoto_final_send_request(mt5_module, req):
    ...
    try:
        ...
        if original_side in {"buy", "long"} and buy_const is not None:
            req["type"] = buy_const
        elif original_side in {"sell", "short"} and sell_const is not None:
            req["type"] = sell_const
        elif isinstance(req.get("type"), str):
        ...
        if req.get("price") in (None, 0, 0.0):
            if tick is not None:
                if original_side in {"buy", "long"} and hasattr(tick, "ask") and tick.ask:
                    req["price"] = float(tick.ask)
                elif original_side in {"sell", "short"} and hasattr(tick, "bid") and tick.bid:
        ...
        # Fill or repair SL/TP so MT5 does not reject the request.
        try:
            sl = req.get("sl", None)
            tp = req.get("tp", None)
            if sl in (None, 0, 0.0, ""):
                if original_side in {"buy", "long"}:
                ...
                if abs(price - req["sl"]) < min_sl_dist:
                    req["sl"] = (price - min_sl_dist) if original_side in {"buy", "long"} else (price + min_sl_dist)
            if tp in (None, 0, 0.0, ""):
                if original_side in {"buy", "long"}:
                ...
                if abs(price - req["tp"]) < min_sl_dist:
                    req["tp"] = (price + (min_sl_dist * 2.0)) if original_side in {"buy", "long"} else (price - (min_sl_dist * 2.0))
        except Exception:
            if original_side in {"buy", "long"}:
                req["sl"] = price - min_sl_dist
                req["tp"] = price + (min_sl_dist * 2.0)
        ...
        # Give MT5 a valid filling mode when possible.
        try:
