...
def load_v15_module():
    ...
    try:
        m.fetch_multi_timeframes = fetch_multi_timeframes
        m._kyoto_rates_to_df = _kyoto_rates_to_df
        m._kyoto_resolve_mt5_symbol = _kyoto_resolve_mt5_symbol
    except Exception:
        pass
    return m
...
def regime_adaptive_stop(entry_price, df_h1, side, base_atr_multiplier=1.25):
    try:
        ...
        if str(side).upper() == "BUY":
            return float(entry_price - stop_dist), float(entry_price + stop_dist * 2.0), float(stop_dist)
        return float(entry_price + stop_dist), float(entry_price - stop_dist * 2.0), float(stop_dist)
    except Exception:
        sd = 0.01 * float(entry_price) if entry_price else 0.01
        if str(side).upper() == "BUY":
            return float(entry_price - sd), float(entry_price + sd * 2.0), float(sd)
        return float(entry_price + sd), float(entry_price - sd * 2.0), float(sd)
...
try:
    if "CONFIG" in globals():
        CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = max(float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.40)), 0.60)
except Exception:
    pass
...
try:
    CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = 0.50
except Exception:
    pass
...
# --- KYOTO FIX: missing multi-timeframe fetch helper (M30 + H1) ---
def _kyoto_resolve_mt5_symbol(mt5_module, symbol: str, symbol_map=None):
    try:
        sym = str(symbol)
        candidates = []
        if symbol_map and isinstance(symbol_map, dict):
            candidates.append(symbol_map.get(sym, sym))
            if sym.endswith("USD"):
                candidates.append(symbol_map.get(sym + "m", sym + "m"))
        candidates.extend([sym, sym + "m"])
        for cand in candidates:
            try:
                if cand and mt5_module.symbol_select(cand, True):
                    return cand
            except Exception:
                continue
        return sym
...
def _kyoto_rates_to_df(rates):
    try:
        ...
        if "tick_volume" in df.columns and "volume" not in df.columns:
            ...
            df["volume"] = df["real_volume"]
        cols = [c for c in ("open", "high", "low", "close", "volume", "tick_volume", "real_volume") if c in df.columns]
        return df[cols].dropna(how="all")
    except Exception:
        logger.exception("_kyoto_rates_to_df failed")
        return None

def fetch_multi_timeframes(symbol, period_days=60, mt5_module=None, symbol_map=None):
    """
    Return a dict with M30 and H1 DataFrames fetched from MT5.
    Safe fallback used by backtest/live MTF paths.
    """
    try:
        import MetaTrader5 as mt5
        mt5_mod = mt5_module or mt5
        ...
        try:
            if not mt5_mod.initialize():
                logger.warning("fetch_multi_timeframes: MT5 initialize failed for %s", symbol)
                return {}
        except Exception:
            logger.exception("fetch_multi_timeframes: MT5 initialize exception for %s", symbol)
            return {}
        sym = _kyoto_resolve_mt5_symbol(mt5_mod, symbol, symbol_map=symbol_map)
        out = {}
        try:
            bars_m30 = max(80, int(period_days * 48))
            rates_m30 = mt5_mod.copy_rates_from_pos(sym, getattr(mt5_mod, "TIMEFRAME_M30", 30), 0, bars_m30)
            df_m30 = _kyoto_rates_to_df(rates_m30)
            if df_m30 is not None and not getattr(df_m30, "empty", True):
                out["M30"] = df_m30
        except Exception:
            logger.exception("fetch_multi_timeframes: M30 fetch failed for %s", symbol)
        try:
            bars_h1 = max(60, int(period_days * 24))
            rates_h1 = mt5_mod.copy_rates_from_pos(sym, getattr(mt5_mod, "TIMEFRAME_H1", 60), 0, bars_h1)
            df_h1 = _kyoto_rates_to_df(rates_h1)
            if df_h1 is not None and not getattr(df_h1, "empty", True):
                out["H1"] = df_h1
        except Exception:
            logger.exception("fetch_multi_timeframes: H1 fetch failed for %s", symbol)
        return out
    except Exception:
        logger.exception("fetch_multi_timeframes failed for %s", symbol)
        return {}
# --- END KYOTO FIX ---
