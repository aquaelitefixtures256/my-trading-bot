...
def run_telethon_news_listener():
    """Real background Telethon listener using a persistent session.

    Never prompts for phone input. If the session is missing or unauthorized,
    the listener stays alive without crashing or restarting.
    """
    try:
        ...
        session_string = CONFIG.get("TELEGRAM_SESSION_STRING") or os.getenv("TELEGRAM_SESSION_STRING") or os.getenv("TG_SESSION_STRING") or ""
        bot_token = CONFIG.get("TELEGRAM_BOT_TOKEN") or os.getenv("TELEGRAM_BOT_TOKEN") or os.getenv("TG_BOT_TOKEN") or ""
    except Exception:
    ...
    try:
        ...
        logger.warning("Telethon api_id is not valid; listener disabled.")
        while True:
            time.sleep(60)
        return
    ...
    async def _main():
        ...
        if session_string and StringSession is not None:
            try:
                ...
                logger.warning("Telethon StringSession invalid; falling back to in-memory session.")
                session_obj = None
        if session_obj is None:
            try:
                session_obj = MemorySession() if MemorySession is not None else session_name
            except Exception:
        ...
        try:
            ...
            logger.exception("Telethon connect failed")
            while True:
        ...
        if not authorized and bot_token:
            try:
                await client.start(bot_token=bot_token)
                authorized = True
            except Exception:
                logger.exception("Telethon bot-token login failed")
                authorized = False

        if not authorized:
            logger.warning("Telethon session not authorized; listener staying idle.")
            try:
    ...
    _runner()
def signal_scanner_loop():
    # Main thread loop: use only live scanner paths; no hardcoded signal fallbacks.
    candidate_names = ["live_scanner_loop", "signal_scanner", "scanner_loop", "run_scanner", "start_scanner", "scan_signals"]
    for name in candidate_names:
        fn = globals().get(name)
        if callable(fn) and name != "live_scanner_loop":
            logger.info(f"Using existing scanner function: {name} as main loop.")
            try:
                fn()
            except Exception:
                logger.exception("Existing scanner function crashed.")
            return
    ...
    try:
        ...
        logger.info("Signal scanner loop interrupted by KeyboardInterrupt; exiting.")
    except Exception:
        logger.exception("Signal scanner loop crashed unexpectedly; exiting.")
...
def compute_position_risk(base_risk_pct, tech_score, fund_score, sent_score):
    try:
        ...
        base = float(globals().get("BASE_RISK_PER_TRADE_PCT", 0.01))
    def _sgn(v):
        try:
            v = float(v)
        except Exception:
...
try:
    ...
    def _kyoto_rt_fill_sl_tp(mt5_mod, sym, side, price, req):
        ...
        sl = _coerce(req.get("sl"))
        tp = _coerce(req.get("tp"))

        try:
            if side_l in ("buy", "long"):
                if sl is not None and tp is not None and sl < price_f < tp:
                    req["sl"] = sl
                    req["tp"] = tp
                    return req
            else:
                if sl is not None and tp is not None and tp < price_f < sl:
                    req["sl"] = sl
                    req["tp"] = tp
                    return req
        except Exception:
        ...
        try:
            ...
            try:
                ...
                spread = 0.0
            fallback_dist = max(point * 10.0, spread * 3.0, point)
            sl_dist = abs(sl) if sl is not None else fallback_dist
            tp_dist = abs(tp) if tp is not None else fallback_dist * 2.0
            if side_l in ("buy", "long"):
                req["sl"] = float(price_f - sl_dist)
                req["tp"] = float(price_f + tp_dist)
            else:
                req["sl"] = float(price_f + sl_dist)
                req["tp"] = float(price_f - tp_dist)
        except Exception:
            try:
                if side_l in ("buy", "long"):
                    req["sl"] = float(price_f - 0.0001)
                    req["tp"] = float(price_f + 0.0002)
                else:
                    req["sl"] = float(price_f + 0.0001)
                    req["tp"] = float(price_f - 0.0002)
            except Exception:
...
if not _KYOTO_RISK_LAYER_INSTALLED:
    ...
    def calc_trade_plan(symbol, tick, atr_value, equity):
        ...
        risk_pct = float(globals().get("RISK_PCT_PER_TRADE", globals().get("BASE_RISK_PER_TRADE_PCT", 0.01)) or 0.01)
