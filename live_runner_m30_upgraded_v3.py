...
def main():
    ...
    try:
        ...
        # ------------------ BEGIN PATCH: ensure mother bot is ready ------------------ 
        import time, inspect, threading as _threading  # time/inspect already available but import locally for the patch

        def ensure_mother_bot_ready(bot, max_wait=30):
            """
            Try to initialize/start the mother bot so compute_signal (or v15.compute_signal)
            becomes available. Returns (v15_module_or_None, ready_bool).
            This is defensive: tries loaders, adapter installers, background starters,
            then polls for compute_signal presence.
            """
            v15 = None
            start_time = time.time()
            logging.info("Ensuring mother bot is ready (timeout %ss)...", max_wait)

            # 1) Try common v15 loader names
            loaders = ["load_v15_module", "load_v15", "load_v15_impl"]
            for name in loaders:
                fn = getattr(bot, name, None)
                if callable(fn):
                    try:
                        logging.info("Calling bot.%s() to obtain v15...", name)
                        v15 = fn()
                        logging.info("Loaded v15 via %s: %s", name, getattr(v15, "__name__", str(v15)))
                        break
                    except Exception as e:
                        logging.warning("bot.%s() raised: %s", name, e)

            # 2) Try adapter installer if no v15 (some bots embed v15 and need adapter install)
            if v15 is None:
                adapter_names = ["install_v15_compute_signal_adapter", "_install_v15_compute_signal_adapter", "install_v15_adapter", "install_v15"]
                for name in adapter_names:
                    fn = getattr(bot, name, None)
                    if callable(fn):
                        try:
                            logging.info("Calling bot.%s() to install adapter...", name)
                            res = fn()
                            # some return module, some return class/None
                            if res:
                                v15 = res
                                logging.info("Adapter installer %s returned: %s", name, getattr(v15, "__name__", str(v15)))
                                break
                        except Exception as e:
                            logging.warning("bot.%s() raised: %s", name, e)

            # 3) Try to start known background task functions (non-blocking)
            bg_names = ["start_background_tasks", "start_background", "run_background", "start_daemon"]
            for name in bg_names:
                fn = getattr(bot, name, None)
                if callable(fn):
                    try:
                        logging.info("Calling bot.%s() to start background tasks...", name)
                        # call in a thread if function blocks
                        try:
                            from threading import Thread as _Thread
                            t = _Thread(target=fn, daemon=True, name=f"bot-bg-{name}")
                            t.start()
                        except Exception:
                            fn()
                        break
            ...
            # 4) Poll until compute_signal appears on v15 or bot, or timeout
            while time.time() - start_time < max_wait:
                # check v15 first
                if v15 is not None:
                    if hasattr(v15, "compute_signal") and callable(getattr(v15, "compute_signal")):
                        logging.info("v15.compute_signal available -> mother bot READY")
                        return v15, True
                # check bot compute_signal
                if hasattr(bot, "compute_signal") and callable(getattr(bot, "compute_signal")):
                    logging.info("bot.compute_signal available -> mother bot READY (no v15)")
                    return v15, True
                # also check common "ready" indicators the bot may set
                for ready_attr in ("BOT_READY", "ready", "_ready", "is_ready"):
                    val = getattr(bot, ready_attr, None)
                    if val:
                        logging.info("Mother bot reports ready via attribute %s", ready_attr)
                        return v15, True
                time.sleep(1)

            # timed out
            logging.warning("Mother bot not ready after %ss. Will continue but runner will force DRY_RUN/skip live orders.", max_wait)
            return v15, False

        # call it immediately after loading the bot 
        v15, mother_ready = ensure_mother_bot_ready(bot, max_wait=30)
        logging.info("ensure_mother_bot_ready -> mother_ready=%s v15=%s", mother_ready, getattr(v15, "name", v15))

        # If not ready, you can choose to force exit or fall back to DRY_RUN. 
        if not mother_ready:
            # safe fallback: continue but force do_execute=False (so no live orders)
            try:
                # this variable should be respected by the runner main loop when deciding do_execute
                do_force_dry = True
                logging.warning("Mother bot not ready -> runner will continue in SAFE mode (no live orders).")
            except Exception:
                pass
        # ------------------ END PATCH ------------------ 
    except Exception as e:
    ...
    try:
        # prefer underscored installer name, but accept either.
        installer_attr = None
        for name in ("_install_v15_compute_signal_adapter", "install_v15_compute_signal_adapter"):
            if v15 is not None and hasattr(bot, name):
                installer_attr = name
                break
        if installer_attr:
            try:
                # try installer with v15 argument first (most common)
                getattr(bot, installer_attr)(v15)
                logging.info("Installed v15 compute_signal adapter via bot.%s(v15)", installer_attr)
            except TypeError:
                try:
                    # fallback: installer without arguments
                    getattr(bot, installer_attr)()
                    logging.info("Installed v15 compute_signal adapter via bot.%s()", installer_attr)
                except Exception as e_inst_noarg:
                    logging.warning("Failed to install v15 adapter (no-arg) via bot.%s: %s", installer_attr, e_inst_noarg)
            except Exception as e_inst_arg:
                logging.warning("Failed to install v15 adapter (with-v15) via bot.%s: %s", installer_attr, e_inst_arg)
        else:
    ...
    do_execute = EXECUTE_ORDERS and is_demo and (not DRY_RUN) and mother_ready
    if not is_demo:
        do_execute = False
