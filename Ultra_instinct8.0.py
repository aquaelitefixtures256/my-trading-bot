...
Minimal surgical fixes: use CURRENT_THRESHOLD for decisions, clamp total_score to [-1,1],
apply proportional + clamp threshold adaptation, and robust live confirmation handling.
...
Run:
  ...
  python Ultra_instinct_fixed.py --backtest
  python Ultra_instinct_fixed.py --loop
"""
...
# Default CURRENT_THRESHOLD can be overridden by env CURRENT_THRESHOLD
CURRENT_THRESHOLD = float(os.getenv("CURRENT_THRESHOLD", "0.13"))
MIN_THRESHOLD = float(os.getenv("MIN_THRESHOLD", "0.12"))
MAX_THRESHOLD = float(os.getenv("MAX_THRESHOLD", "0.50"))
DECISION_SLEEP = int(os.getenv("DECISION_SLEEP", "60"))
ADAPT_EVERY_CYCLES = 6
MODEL_MIN_TRAIN = 40

# ADAPTATION PARAMETERS (proportional + clamp)
ADAPT_MIN_TRADES = int(os.getenv("ADAPT_MIN_TRADES", "40"))
TARGET_WINRATE = float(os.getenv("TARGET_WINRATE", "0.525"))
K_ADAPT = float(os.getenv("K_ADAPT", "0.04"))
MAX_ADJ = float(os.getenv("MAX_ADJ", "0.01"))
...
def record_trade(symbol, side, entry, sl, tp, lots, status="sim", pnl=0.0, rmult=0.0, regime="unknown", score=0.0, model_score=0.0, meta=None):
    ...
    data = {
        "ts": ts, "symbol": symbol, "side": side, "entry": entry, "sl": sl, "tp": tp, "lots": lots,
        "status": status, "pnl": pnl, "rmult": rmult, "rm": rmult, "regime": regime, "score": score, "model_score": model_score, "meta": meta_json
    }
    try:
        conn = sqlite3.connect(TRADES_DB, timeout=5)
        cur = conn.cursor()
        ...
        if not cols:
            ...
            cur = conn.cursor()
            cols = _get_table_columns(conn, "trades")
        insert_cols = [c for c in ["ts","symbol","side","entry","sl","tp","lots","status","pnl","rmult","rm","regime","score","model_score","meta"] if c in cols]
        if not insert_cols:
...
def fetch_fundamental_score(symbol: str, lookback_days: int = 7) -> float:
    ...
    try:
        ...
        params = {
            "q": query, "language": "en", "pageSize": 30, "sortBy": "relevancy",
        ...
        score = 0.0
        for a in articles:
            title = str(a.get("title") or ""); desc = str(a.get("description") or "")
            txt = (title + " " + desc).lower()
            p = sum(1 for w in pos_words if w in txt); n = sum(1 for w in neg_words if w in txt)
            score += (p - n)
...
def _symbol_to_currencies(symbol: str) -> List[str]:
    s = symbol.upper()
    if len(s) >= 6:
        base = s[:3]; quote = s[3:6]; return [base, quote]
    if s.startswith("XAU") or "XAU" in s: return ["XAU","USD"]
    if s.startswith("XAG") or "XAG" in s: return ["XAG","USD"]
    if s.startswith("BTC"): return ["BTC","USD"]
    return [s]
...
def fetch_economic_calendar_events(lookback_hours: int = 6, lookahead_hours: int = 6) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("TradingEconomics non-200: %s", resp.status_code); return []
        events = resp.json(); out = []
        for e in events:
            try: out.append(e)
            except Exception: continue
        return out
    except Exception:
        logger.exception("fetch_economic_calendar_events failed"); return []
...
def fetch_economic_calendar_score(symbol: str, lookback_hours: int = 6, lookahead_hours: int = 6) -> float:
    ...
    try:
        ...
        related = []; currs = _symbol_to_currencies(symbol)
        for e in evs:
            ...
            if str(impact).lower() not in ("high","h","high impact"): continue
            match = False
            for c in currs:
                if c and (c.lower() in title or c.upper() == country or c.upper() in str(e.get("Category","")).upper()): match = True
            if match: related.append(e)
        if not related: return 0.0
        score = 0.0; count = 0
        for e in related:
            ...
            try:
                if actual is None or str(actual).strip() == "": continue
                actual_val = float(str(actual).replace(",",""))
                forecast_val = float(str(forecast).replace(",","")) if forecast not in (None,"","None") else None
                if forecast_val is None: continue
                if actual_val > forecast_val: score += 1.0
                elif actual_val < forecast_val: score -= 1.0
                count += 1
            except Exception:
                count += 1
                continue
        ...
        logger.exception("fetch_economic_calendar_score failed"); return 0.0

def should_pause_for_events(symbol: str, lookahead_minutes: int = 30) -> (bool, Optional[Dict[str, Any]]):
    if not FUNDAMENTAL_AVAILABLE or not TRADING_ECONOMICS_KEY: return False, None
    try:
        ...
        now = datetime.utcnow(); currs = _symbol_to_currencies(symbol)
        for e in evs:
            impact = e.get("Impact") or e.get("importance") or e.get("impact","")
            if not impact or str(impact).lower() not in ("high","h","high impact"): continue
            when = None
            for key in ("Date","date","Scheduled","dateTime"):
                if key in e and e.get(key):
                    try:
                        when = pd.to_datetime(e.get(key)); break
                    except Exception:
                        continue
        ...
        logger.exception("should_pause_for_events failed"); return False, None
...
def load_model():
    ...
    if os.path.exists(MODEL_FILE):
        try:
            model_pipe = joblib.load(MODEL_FILE); logger.info("Loaded ML model"); return model_pipe
        except Exception:
            logger.exception("Load model failed")
    try:
        model_pipe = build_model(); return model_pipe
    except Exception:
        return None
...
def extract_features_for_model(df_h1: pd.DataFrame, tech_score: float, symbol: str, regime_code: int):
    try:
        ...
        entry = float(d["close"].iloc[-1]); atr = float(d["atr14"].iloc[-1] or 0.0)
...
def simulate_strategy_on_series(df_h1, threshold, atr_mult=1.25, max_trades=200):
    ...
    df = add_technical_indicators(df_h1.copy()); trades=[]
    for i in range(30, len(df)-10):
        window = df.iloc[: i + 1]
        score = technical_signal_score(window)
        if score >= threshold: side="BUY"
        elif score <= -threshold: side="SELL"
        else: continue
        entry = float(df["close"].iloc[i]); atr=float(df["atr14"].iloc[i] or 0.0); stop=atr*atr_mult
        if side=="BUY": sl=entry-stop; tp=entry+stop*2.0
        else: sl=entry+stop; tp=entry-stop*2.0
        r_mult = 0.0
        for j in range(i+1, min(i+31, len(df))):
            high=float(df["high"].iloc[j]); low=float(df["low"].iloc[j])
            if side=="BUY":
                if high>=tp: r_mult=2.0; break
                if low<=sl: r_mult=-1.0; break
            else:
                if low<=tp: r_mult=2.0; break
                if high>=sl: r_mult=-1.0; break
        trades.append(r_mult)
        if len(trades) >= max_trades: break
    n=len(trades)
    if n==0: return {"n":0,"net":0.0,"avg_r":0.0,"win":0.0}
...
def light_optimizer(symbols, budget=12):
    ...
    if not candidates:
        logger.info("Optimizer found no candidates"); return None
    ...
    logger.info("Optimizer skipped applying"); return None
...
def place_order_mt5(symbol, action, lot, price, sl, tp):
    ...
    try:
        ...
        try:
            if not si.visible: _mt5.symbol_select(broker, True)
        except Exception:
            pass
        tick = _mt5.symbol_info_tick(broker)
        if tick is None: return {"status":"no_tick","symbol":broker}
        ...
        if stop_level is not None and stop_level >= 0: min_sl_dist = float(stop_level) * float(point)
        else: min_sl_dist = float(point) * 10.0
        ...
        def valid_distance(dist):
            try: return (dist is not None) and (abs(dist) >= min_sl_dist)
            except Exception: return False
        sl_ok = True; tp_ok = True
        if sl is not None:
            sl_dist = abs(entry_price - float(sl)); sl_ok = valid_distance(sl_dist)
        if tp is not None:
            tp_dist = abs(entry_price - float(tp)); tp_ok = valid_distance(tp_dist)
        if not sl_ok:
            if action=="BUY": sl = entry_price - min_sl_dist
            else: sl = entry_price + min_sl_dist
            sl_ok = True
        if not tp_ok:
            if action=="BUY": tp = entry_price + (min_sl_dist * 2.0)
            else: tp = entry_price - (min_sl_dist * 2.0)
            tp_ok = True
        if lots < vol_min: lots = float(vol_min)
        if vol_max and lots > vol_max: return {"status":"volume_too_large","requested":lots,"max":vol_max}
        order_type = _mt5.ORDER_TYPE_BUY if action=="BUY" else _mt5.ORDER_TYPE_SELL
        req = {"action":_mt5.TRADE_ACTION_DEAL,"symbol":broker,"volume":float(lots),"type":order_type,"price":float(order_price),
               "sl":float(sl) if sl is not None else 0.0,"tp":float(tp) if tp is not None else 0.0,"deviation":20,"magic":123456,"comment":"Ultra_instinct",
               "type_time":_mt5.ORDER_TIME_GTC,"type_filling":_mt5.ORDER_FILLING_IOC}
        res = _mt5.order_send(req); retcode = getattr(res,"retcode",None)
        if retcode == 10027: return {"status":"autotrading_disabled","retcode":retcode,"result":str(res)}
        if retcode is not None and retcode != 0: return {"status":"rejected","retcode":retcode,"result":str(res)}
        return {"status":"sent","result":str(res),"used_lots":lots}
    except Exception:
        logger.exception("place_order_mt5 failed"); return {"status":"error"}

def get_today_trade_count():
    try:
        conn = sqlite3.connect(TRADES_DB, timeout=5); cur = conn.cursor()
        cur.execute("SELECT ts FROM trades"); rows = cur.fetchall(); conn.close()
    except Exception:
        logger.exception("get_today_trade_count: DB read failed"); return 0
    ...
    try:
        if reset_mode == "BROKER" and MT5_AVAILABLE and _mt5_connected:
            ...
            if broker_now_ts: broker_now = datetime.utcfromtimestamp(int(broker_now_ts)); broker_date = broker_now.date()
            else: today = datetime.utcnow().date(); broker_date = today
            start_utc = datetime(broker_date.year, broker_date.month, broker_date.day, tzinfo=timezone.utc)
        elif reset_mode == "LOCAL":
            local_now = datetime.now().astimezone(); local_date = local_now.date()
            local_midnight = datetime(local_date.year, local_date.month, local_date.day, tzinfo=local_now.tzinfo)
            start_utc = local_midnight.astimezone(timezone.utc)
        else:
            today = datetime.utcnow().date(); start_utc = datetime(today.year, today.month, today.day, tzinfo=timezone.utc)
    except Exception:
        today = datetime.utcnow().date(); start_utc = datetime(today.year, today.month, today.day, tzinfo=timezone.utc)
    count = 0
    for (ts_raw,) in rows:
        ...
        if pd.isna(parsed):
            try:
                ...
                if pd.isna(parsed_naive): continue
        ...
        try:
            ...
            try:
                ...
                continue
        try:
            parsed_dt = parsed.to_pydatetime() if isinstance(parsed, pd.Timestamp) else parsed
            if parsed_dt.tzinfo is None: parsed_dt = parsed_dt.replace(tzinfo=timezone.utc)
            if parsed_dt >= start_utc: count += 1
        except Exception:
            continue
    return int(count)
...
def make_decision_for_symbol(symbol: str, live: bool=False):
    ...
    try:
        ...
        if df_h1 is None or getattr(df_h1,"empty",True) or len(df_h1) < 40:
            logger.info("Not enough H1 data for %s - skipping", symbol); return None
        scores = aggregate_multi_tf_scores(tfs)
        tech_score = scores["tech"]; model_score = 0.0; fundamental_score = 0.0
        if SKLEARN_AVAILABLE and model_pipe is not None:
            try:
                ...
                try:
                    proba = model_pipe.predict_proba(X)[:,1][0]; model_score = float((proba - 0.5) * 2.0)
                except Exception:
                    try: pred = model_pipe.predict(X)[0]; model_score = 0.9 if pred==1 else -0.9
                    except Exception: model_score = 0.0
            except Exception:
                model_score = 0.0
        try:
            news_sent=0.0; econ_sent=0.0
            try: news_sent = fetch_fundamental_score(symbol)
            except Exception: news_sent = 0.0
            try: econ_sent = fetch_economic_calendar_score(symbol, lookback_hours=12, lookahead_hours=12)
            except Exception: econ_sent = 0.0
        ...
        total_score = max(-1.0, min(1.0, float(total_score)))  # NORMALIZE

        # Use CURRENT_THRESHOLD (absolute) for candidate decision so thresholds match your settings
        candidate = None
        ...
        if total_score >= 0.18:
            ...
            candidate = "SELL"
        final_signal = None
        # require at least abs(total_score) >= CURRENT_THRESHOLD to become final
        ...
        # DEBUG EXECUTION SNAPSHOT (temporary)
        try:
            logger.info(
                "DEBUG_EXEC -> sym=%s agg=%+.5f candidate=%s final_signal=%s "
                "CURRENT_THRESHOLD=%.5f port_scale=%.3f paused=%s",
                symbol,
                float(total_score),
                str(candidate),
                str(final_signal),
        ...
        if final_signal:
            ...
            if os.path.exists(KILL_SWITCH_FILE):
                logger.info("Kill switch engaged - skipping order for %s", symbol); return decision
            if live and get_today_trade_count() >= MAX_DAILY_TRADES:
                logger.info("Daily trade cap reached - skipping"); return decision
            ...
            if live and not DEMO_SIMULATION:
                ...
                # Compose concise telegram message using status mapping
                try:
                    status = res.get("status","unknown")
                    if status == "sent": emoji="✅"; status_text="EXECUTED"
                    elif status == "rejected": emoji="❌"; status_text="REJECTED"
                    elif status == "autotrading_disabled": emoji="⚠️"; status_text="AUTO TRADING DISABLED"
                    else: emoji="⚠️"; status_text=str(status).upper()
        ...
        logger.exception("make_decision_for_symbol failed for %s", symbol); return None
...
def adapt_and_optimize():
    ...
    try:
        ...
        # Proportional + clamp adaptation (your suggested block)
        if n >= ADAPT_MIN_TRADES:
            adj = -K_ADAPT * (winrate - TARGET_WINRATE)  # negative when winrate > target (reduce threshold)
            if adj > MAX_ADJ: adj = MAX_ADJ
            elif adj < -MAX_ADJ: adj = -MAX_ADJ
        ...
        # compute risk scaling by market regime as before
        vols = []
        for s in SYMBOLS:
            tfs = fetch_multi_timeframes(s, period_days=45)
            h1 = tfs.get("H1")
            if h1 is None or getattr(h1,"empty",True): continue
            _, rel, adx = detect_market_regime_from_h1(h1)
            if rel is not None: vols.append(rel)
        if vols:
            ...
            if n >= ADAPT_MIN_TRADES and sum(vals) < 0: new_risk *= 0.7
...
def run_backtest():
    ...
    for s in SYMBOLS:
        ...
        if df is None:
            logger.info("No H1 for %s (MT5 missing) - skipping", s); continue
...
def confirm_enable_live():
    # Robust: honor env var first (non-interactive)
    if os.getenv("CONFIRM_AUTO", "") == "I UNDERSTAND_THE_RISKS":
        return True
    got = input("To enable LIVE trading type exactly: I UNDERSTAND THE_RISKS\nType now: ").strip()
...
def setup_and_run(args):
    ...
    if MT5_AVAILABLE and MT5_LOGIN and MT5_PASSWORD and MT5_SERVER:
        ...
        if ok: logger.info("MT5 connected; preferring MT5 feed/execution")
    else:
        logger.info("MT5 not available or credentials not provided - bot will not fetch data")
    if args.backtest:
        run_backtest(); return
    if args.live:
        # If env var set, confirm_enable_live will auto-allow. Otherwise prompt once.
        if not confirm_enable_live():
            logger.info("Live not enabled"); return
        global DEMO_SIMULATION, AUTO_EXECUTE
        DEMO_SIMULATION = False; AUTO_EXECUTE = True
    if args.loop:
