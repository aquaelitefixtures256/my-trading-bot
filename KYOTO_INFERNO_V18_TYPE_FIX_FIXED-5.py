...
def _kyoto_final_send_request(mt5_module, req):
    ...
    try:
        ...
        if req.get("price") in (None, 0, 0.0, ""):
            ...
            info = None
            try:
                if callable(getattr(mt5_module, "symbol_select", None)):
            ...
            price = None
            if tick is not None:
                if req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0):
                    price = float(getattr(tick, "ask", 0.0) or getattr(tick, "last", 0.0) or getattr(tick, "bid", 0.0) or 0.0)
                elif req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1):
                    price = float(getattr(tick, "bid", 0.0) or getattr(tick, "last", 0.0) or getattr(tick, "ask", 0.0) or 0.0)
                else:
                    price = float(getattr(tick, "last", 0.0) or getattr(tick, "ask", 0.0) or getattr(tick, "bid", 0.0) or 0.0)

            if (price is None or float(price) <= 0.0) and info is not None:
                if req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0):
                    price = float(getattr(info, "ask", 0.0) or getattr(info, "bid", 0.0) or 0.0)
                elif req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1):
                    price = float(getattr(info, "bid", 0.0) or getattr(info, "ask", 0.0) or 0.0)
                else:
                    price = float(getattr(info, "last", 0.0) or getattr(info, "ask", 0.0) or getattr(info, "bid", 0.0) or 0.0)
...
try:
    ...
    def place_order_mt5(*args, **kwargs):
        try:
            mt5_module = (
                kwargs.pop("mt5_module", None)
                or kwargs.pop("module", None)
                or kwargs.pop("mts_module", None)
                or globals().get("mt5")
                or globals().get("MT5")
            )
            req = {}
    ...
    globals()["order_wrapper"] = order_wrapper
    globals()["place_order_mt5"] = place_order_mt5
    globals()["place_order_mts"] = place_order_mt5
...
try:
    ...
    if callable(_KYOTO_FINAL_DOOR):
        ...
        globals()["place_order_mts"] = _KYOTO_FINAL_DOOR
