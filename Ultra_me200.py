#!/usr/bin/env python3
"""
Ultra_instinct (full) - enhanced fundamentals & news + PnL reconciliation
Preserves all original logic; adds:
 - News module (NewsAPI-compatible) with keyword detection (iran, strike, oil, war)
 - Stronger fundamental scoring (news + calendar)
 - Economic calendar blocking (high-impact filtering)
 - Integrates news sentiment into total_score (small weight)
 - reconcile_closed_deals() to update pnl on closed deals
 - Per-symbol max-open-trades, MT5 positions fallback, debug-once snapshot,
   robust live confirmation, normalized total_score, Proportional+Clamp adaptation,
   robust order-confirmation/recording/telegram unchanged.
"""
...
# Execution defaults and environment hooks
DEMO_SIMULATION = False
AUTO_EXECUTE = True
if os.getenv("CONFIRM_AUTO", ""):
    # tolerant accept of confirmation env var (normalized below)
...
# News / calendar keys
NEWS_API_KEY = os.getenv("NEWS_API_KEY", "")           # NewsAPI-compatible key
TRADING_ECONOMICS_KEY = os.getenv("TRADING_ECONOMICS_KEY", "")  # optional calendar provider
NEWS_LOOKBACK_DAYS = int(os.getenv("NEWS_LOOKBACK_DAYS", "2"))  # how many days back to search news
...
# ---------------- News & Fundamentals module (stronger) ----------------
# Keyword lists and simple sentiment words
...
def fetch_news_for_symbol(symbol: str, lookback_days: int = NEWS_LOOKBACK_DAYS) -> Dict[str, Any]:
    """Fetch recent news headlines related to symbol using NewsAPI-compatible endpoint.
    Returns dict: {'count':n,'sentiment':score(-1..1),'keyword_hits':n,'raw':[...] }.
    If NEWS_API_KEY missing or requests not available, returns zeros."""
    out = {"count": 0, "sentiment": 0.0, "keyword_hits": 0, "raw": []}
    ...
    if s in ("XAUUSD", "GOLD"):
        ...
        q = "oil OR crude OR wti OR brent OR usoil"
    else:
        q = symbol
    # add risk keywords to broaden capture (so news re: iran/oil get captured)
    q = f"({q}) OR ({' OR '.join(list(_RISK_KEYWORDS))})"
    try:
        ...
        params = {
            ...
            "sortBy": "publishedAt",
            "pageSize": 50,
            "from": (datetime.utcnow() - pd.Timedelta(days=lookback_days)).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "apiKey": NEWS_API_KEY,
        }
        resp = requests.get(url, params=params, timeout=8)
        if resp.status_code != 200:
            logger.debug("fetch_news_for_symbol non-200: %s %s", resp.status_code, resp.text[:200])
            return out
        j = resp.json()
        articles = j.get("articles", [])[:50]
        if not articles:
            return out
        total_score = 0.0
        keyword_hits = 0
        for a in articles:
            ...
            # keyword hits
            kh = sum(1 for k in _RISK_KEYWORDS if k in txt)
            keyword_hits += kh
            # sentiment: simple bag-of-words
            p = sum(1 for w in _POS_WORDS if w in txt)
            n = sum(1 for w in _NEG_WORDS if w in txt)
            # base score normalized by length
            norm = max(1.0, len(txt.split()))
            score = (p - n) / norm
            total_score += score
        avg_score = total_score / max(1, len(articles))
        # clamp to [-1,1] (approx)
        avg_score = max(-1.0, min(1.0, avg_score * 10.0))
        out.update({"count": len(articles), "sentiment": float(avg_score), "keyword_hits": int(keyword_hits), "raw": articles})
        return out
    except Exception:
        logger.exception("fetch_news_for_symbol failed")
        return out

def fetch_fundamental_score(symbol: str, lookback_days: int = 7) -> float:
    """
    Returns a normalized fundamental score in [-1,1]:
     - news sentiment contributes (small weight)
     - economic calendar high-impact events cause blocking or weighting
    """
    news_sent = 0.0
    calendar_sent = 0.0
    try:
        news = fetch_news_for_symbol(symbol, lookback_days=NEWS_LOOKBACK_DAYS)
        news_sent = float(news.get("sentiment", 0.0))
        # boost on risk keywords presence
        kh = int(news.get("keyword_hits", 0))
        if kh >= 1:
            # push sentiment toward risk: treat as negative for risky keywords when they include war/strike
            # if news sentiment positive and contains risk keywords, reduce confidence
            news_sent = max(-1.0, min(1.0, news_sent - 0.25 * min(kh, 4)))
    except Exception:
        news_sent = 0.0
    try:
        # use calendar score if available (tries TRADING_ECONOMICS or other providers)
        cal_score = 0.0
        # attempt the existing fetch_economic_calendar_score if provider set
        try:
            cs = fetch_economic_calendar_score(symbol, lookback_hours=24, lookahead_hours=24)
            cal_score = float(cs)
        except Exception:
            cal_score = 0.0
        calendar_sent = cal_score
    except Exception:
        calendar_sent = 0.0
    # combine: news has moderate weight, calendar smaller; clamp
    combined = 0.6 * news_sent + 0.4 * calendar_sent
    combined = max(-1.0, min(1.0, combined))
    return float(combined)

# ---------------- Economic calendar & blocking (robust) ----------------
...
def fetch_economic_calendar_events(lookback_hours: int = 6, lookahead_hours: int = 6) -> List[Dict[str, Any]]:
    # Try TradingEconomics or other provider. If not configured, return []
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("fetch_economic_calendar_events non-200: %s", resp.status_code)
            return []
        events = resp.json()
...
def should_pause_for_events(symbol: str, lookahead_minutes: int = 30) -> (bool, Optional[Dict[str, Any]]):
    """
    Improved: Accepts numeric impact codes (3=high), handles multiple datetime keys,
    uses timezone-aware comparison, and returns (True, info) when high-impact event imminent.
    """
    try:
        if not FUNDAMENTAL_AVAILABLE or not TRADING_ECONOMICS_KEY:
            return False, None
        ...
        for e in evs:
            try:
                # normalize impact
                impact_raw = e.get("Impact") or e.get("importance") or e.get("impact") or e.get("importanceLevel")
                ...
                when = None
                for key in ("Date", "date", "Scheduled", "dateTime", "scheduled_time", "timestamp", "time"):
...
def place_order_mt5(symbol, action, lot, price, sl, tp):
    ...
    try:
        ...
        # try to capture order-ticket info if present
...
# ---------------- Open positions counting (MT5 or DB fallback) ----------------
...
# ---------------- Reconcile closed deals and update trade PnL (unchanged logic) ----------
...
def make_decision_for_symbol(symbol: str, live: bool=False):
    ...
    try:
        ...
        try:
            ...
            try:
                news_sent = fetch_news_for_symbol(symbol, lookback_days=NEWS_LOOKBACK_DAYS).get("sentiment", 0.0)
            except Exception:
                news_sent = 0.0
            try:
                ...
                econ_sent = 0.0
            # combine news + econ calendar into fundamental_score
            fundamental_score = float(0.7 * news_sent + 0.3 * econ_sent)
        except Exception:
            fundamental_score = 0.0
        ...
        # port scale adjustment
        total_score = total_score * (0.5 + 0.5 * port_scale)

        # additional small influence: if recent news had many risk keywords, push magnitude slightly up
        try:
            news = fetch_news_for_symbol(symbol, lookback_days=NEWS_LOOKBACK_DAYS)
            kh = int(news.get("keyword_hits", 0))
            if kh >= 2:
                # amplify magnitude by up to 20% depending on hits
...
# ---------------- Adaptation (Proportional + Clamp) (kept intact) ----------------
