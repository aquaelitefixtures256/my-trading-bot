...
def sanitize_source(src: str) -> str:
    ...
    s = s.replace("client.run_until_disconnected()", "await start_telethon_disconnect_placeholder()")
...
def start_all_components(client_var_name="client", trading_entry="main_loop", news_entry="_poll_newsdata_loop", risk_entry="uvx_main"):
    """Start trading, news, and risk workers under Supervisor threads.

    This implementation uses functions available in globals() (or direct callables)
    and does not attempt to load any embedded/original module.
    """
    supervisors = []

    def resolve_target(entry):
        # accept either a callable or a name in globals()
        if callable(entry):
            return entry
        if isinstance(entry, str):
            return globals().get(entry) or (lambda *a, **k: logger.info("No entry '%s' found", entry))
        return (lambda *a, **k: logger.info("Invalid entry: %r", entry))

    trading_target = resolve_target(trading_entry)
    news_target = resolve_target(news_entry)
    risk_target = resolve_target(risk_entry)
    ...
    # start supervisors with explicit logging
    for s in supervisors:
        logger.info("Supervisor starting %s", s.name)
        s.start()
...
# === START: Threaded event loop executor helpers (injected to avoid Windows socket exhaustion) ===
...
def start_telethon_background(client, name="telethon"):
    ...
    async def _runner():
        try:
            await start_telethon_client_placeholder(client)
...
def _start_telegram_listener():
    ...
    try:
        ...
        def start_telethon_background(client):
            """
            Run Telethon in its own dedicated asyncio event loop on a background thread.
            This avoids relying on ThreadedEventLoopExecutor and ensures Telethon has its own loop.
            """
            import asyncio as _asyncio
            import threading as _threading
            import logging as _logging

            async def _telethon_runner():
                try:
                    # start the client (this may be a coroutine or regular function)
                    await start_telethon_client_placeholder(client)
                    ...
                    # Wait until client disconnects. Prefer coroutine run_until_disconnected if available.
                    run_until_disc = getattr(client, "run_until_disconnected", None)
                    if run_until_disc is None:
                        # fallback placeholder
                        await start_telethon_disconnect_placeholder()
                    elif _asyncio.iscoroutinefunction(run_until_disc):
            ...
            def _thread_target():
                try:
                    ...
                    # Run the telethon runner until completion (which should be when disconnected)
                    loop.run_until_complete(_telethon_runner())
                except Exception:
...
SYMBOL_PARAMS: Dict[str, Dict[str, Any]] = {
    ...
    "XAUUSD": {"lookback": 252, "atr_mult": 2.8, "risk_pct": 0.007, "pip_value": 0.01, "liquidity": 0.7},
...
if __name__ == "__main__":
    logging.info("Starting Xarath trading system")

    start_all_components(
        client_var_name="client",
        trading_entry="main_loop",
        news_entry="_poll_newsdata_loop",
        risk_entry="uvx_main"
    )