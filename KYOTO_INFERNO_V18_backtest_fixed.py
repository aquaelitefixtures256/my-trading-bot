...
CONFIG["BACKTEST_PARAMS"].update({
    ...
    "USDJPYm": {"signal_thresh": 0.88, "atr_pct_thresh": 0.0009, "max_hold": 60},
        ...
        "XAUUSD":  {"signal_thresh": 0.92, "dxy_gate_thresh": 0.20, "atr_pct_thresh": 0.0015, "sl_atr_mult": 1.0, "tp_atr_mult": 2.8, "max_hold": 30, "max_loss_abs": 18.0},
    "XAUUSDm": {"signal_thresh": 0.92, "dxy_gate_thresh": 0.20, "atr_pct_thresh": 0.0015, "sl_atr_mult": 1.0, "tp_atr_mult": 2.8, "max_hold": 30, "max_loss_abs": 18.0},
...
try:
    GLOBAL_MAX_OPEN_TRADES = int(globals().get("GLOBAL_MAX_OPEN_TRADES", 15))
except Exception:
    GLOBAL_MAX_OPEN_TRADES = 15

try:
    SYMBOL_TRADE_LIMITS = dict(globals().get("SYMBOL_TRADE_LIMITS", {"USOIL": 5, "BTCUSD": 5, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}))
except Exception:
    SYMBOL_TRADE_LIMITS = {"USOIL": 5, "BTCUSD": 5, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}
...
try:
    SYMBOL_TRADE_LIMITS = dict(globals().get(
        "SYMBOL_TRADE_LIMITS",
        {"USOIL": 5, "BTCUSD": 5, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}
    ))
except Exception:
    SYMBOL_TRADE_LIMITS = {"USOIL": 5, "BTCUSD": 5, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}
...
# ===== BACKTEST FIX (M30/H1 live-like, normalized signal scale) =====
# This override keeps live trading untouched and makes backtests use the same
# intraday structure the bot uses in production.

def _bt_norm_signal(x, norm=0.85):
    try:
        n = float(norm) if norm else 0.85
    except Exception:
        n = 0.85
    if n <= 0:
        n = 0.85
...
def _bt_bar_timeframe_df(df):
    """Ensure we have a time column and indicators attached."""
    import pandas as pd
    if df is None or getattr(df, 'empty', True):
        return None
    d = df.copy()
    if 'time' not in d.columns:
        # best-effort recovery if time is the index
        try:
            d = d.reset_index()
        except Exception:
            pass
    ...
    try:
        ...
        pass
    return d.dropna(subset=['time']) if 'time' in d.columns else d


def _bt_compute_series_scores(df):
    """Compute rolling technical scores using the same live feature logic, then normalize to [-1,1]."""
    scores = []
    if df is None or getattr(df, 'empty', True):
        return scores
    for i in range(len(df)):
        try:
            window = df.iloc[: i + 1]
            raw = technical_signal_score(window)
            scores.append(_bt_norm_signal(raw, norm=float(CONFIG.get('BACKTEST_SCORE_NORM', 0.85))))
        except Exception:
            scores.append(0.0)
    return scores


def run_backtest(v15_module=None, symbol='XAUUSD', days=30):
    """
    Backtest the bot on M30 entries with H1 confirmation (intraday),
    using normalized technical scores so strict live thresholds remain meaningful.
    """
    import math
    import pandas as pd

    logger.info("Starting backtest for %s for %s days (M30/H1 live-like)", symbol, days)

    try:
        import MetaTrader5 as mt5
    except Exception as e:
        logger.exception("MT5 import failed in backtest: %s", e)
        return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "mt5_import_failed"}

    try:
        if not mt5.initialize():
            logger.error("MT5 initialize failed in backtest: %s", mt5.last_error())
            return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "mt5_init_failed"}
    except Exception as e:
        logger.exception("MT5 initialize exception in backtest: %s", e)
        return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "mt5_init_exception"}

    try:
        # Pull the same intraday structure the live bot uses.
        tfs = fetch_multi_timeframes(symbol, period_days=int(days))
    except Exception as e:
        logger.exception("fetch_multi_timeframes failed for %s: %s", symbol, e)
        return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "fetch_failed"}

    df_m30 = _bt_bar_timeframe_df(tfs.get('M30'))
    df_h1 = _bt_bar_timeframe_df(tfs.get('H1'))

    if df_m30 is None or len(df_m30) < 80:
        logger.error("Not enough M30 data for %s", symbol)
        return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "no_m30_data"}

    # Build scores on both frames.
    df_m30 = df_m30.sort_values('time').reset_index(drop=True)
    df_m30['score_m30'] = _bt_compute_series_scores(df_m30)

    if df_h1 is not None and len(df_h1) >= 40:
        df_h1 = df_h1.sort_values('time').reset_index(drop=True)
        df_h1['score_h1'] = _bt_compute_series_scores(df_h1)
        # Align H1 score to M30 bars using last known H1 value.
        try:
            df_m30 = pd.merge_asof(
                df_m30.sort_values('time'),
                df_h1[['time', 'score_h1', 'atr14']].rename(columns={'atr14': 'atr_h1'}).sort_values('time'),
                on='time',
                direction='backward'
            )
        except Exception:
    ...
    # Optional DXY gate for XAU backtests (same live idea, but only if data exists).
    dxy_df = None
    dxy_score_series = None
    if str(symbol).upper().startswith('XAU'):
        try:
            dxy_tfs = fetch_multi_timeframes('DXY', period_days=int(days))
            dxy_df = _bt_bar_timeframe_df(dxy_tfs.get('M30'))
            if dxy_df is not None and len(dxy_df) >= 40:
                dxy_df = dxy_df.sort_values('time').reset_index(drop=True)
                dxy_df['score_dxy'] = _bt_compute_series_scores(dxy_df)
                dxy_score_series = dxy_df[['time', 'score_dxy']]
                df_m30 = pd.merge_asof(
                    df_m30.sort_values('time'),
                    dxy_score_series.sort_values('time'),
                    on='time',
                    direction='backward'
                )
            else:
                df_m30['score_dxy'] = 0.0
        except Exception:
            df_m30['score_dxy'] = 0.0
    else:
        df_m30['score_dxy'] = 0.0
    ...
    # Params / thresholds
    params = CONFIG.get('BACKTEST_PARAMS', {}).get(symbol, {}) if isinstance(CONFIG.get('BACKTEST_PARAMS', {}), dict) else {}
    signal_thresh = float(params.get('signal_thresh', 0.88 if not str(symbol).upper().startswith('XAU') else 0.92))
    h1_confirm_min = float(params.get('h1_confirm_min', 0.18))
    dxy_gate_thresh = params.get('dxy_gate_thresh', None)
    sl_mult = float(params.get('sl_atr_mult', 4.0))
    tp_mult = float(params.get('tp_atr_mult', 6.0))
    max_hold = int(params.get('max_hold', 30))
    norm = float(params.get('backtest_score_norm', CONFIG.get('BACKTEST_SCORE_NORM', 0.85)))

    logger.info(
        "Backtest params for %s -> signal_thresh=%.3f h1_confirm_min=%.3f norm=%.3f sl_mult=%.2f tp_mult=%.2f max_hold=%d",
        symbol, signal_thresh, h1_confirm_min, norm, sl_mult, tp_mult, max_hold
    )
    ...
    # Use M30 as entry frame. H1 is only confirmation context.
    warmup = 30
    last_idx = len(df_m30) - max_hold - 2
    if last_idx <= warmup:
        logger.error("Not enough M30 history for %s to run backtest", symbol)
        return {"net": 0.0, "trades": 0, "win_rate": 0.0, "max_dd": 0.0, "error": "insufficient_history"}

    for i in range(warmup, last_idx):
        try:
            row = df_m30.iloc[i]
            raw_m30 = float(row.get('score_m30', 0.0) or 0.0)
            raw_h1 = float(row.get('score_h1', 0.0) or 0.0)
            raw_dxy = float(row.get('score_dxy', 0.0) or 0.0)
            score_m30 = _bt_norm_signal(raw_m30, norm=norm)
            score_h1 = _bt_norm_signal(raw_h1, norm=norm)
            score_dxy = _bt_norm_signal(raw_dxy, norm=norm)

            # Main entry threshold: M30 must be strong.
            if abs(score_m30) < signal_thresh:
                continue

            # H1 confirmation: same direction, but intentionally lighter than entry.
            if score_m30 > 0 and score_h1 < h1_confirm_min:
                continue
            if score_m30 < 0 and score_h1 > -h1_confirm_min:
                continue

            # XAU macro filter: if DXY available, require opposite direction.
            if str(symbol).upper().startswith('XAU') and dxy_gate_thresh is not None:
                if score_m30 > 0 and not (score_dxy <= -abs(float(dxy_gate_thresh))):
                    continue
                if score_m30 < 0 and not (score_dxy >= abs(float(dxy_gate_thresh))):
                    continue

            side = 'BUY' if score_m30 > 0 else 'SELL'
            entry = float(row['close'])
            atr = float(row.get('atr14', 0.0) or row.get('atr_h1', 0.0) or 0.0)
            if atr <= 0.0:
                # fallback to a modest percent stop if ATR is missing in backtest bars.
                atr = max(entry * 0.002, 1e-9)
            ...
            exit_price = None
            exit_time = None
            for j in range(i + 1, min(i + 1 + max_hold, len(df_m30))):
                hi = float(df_m30.iloc[j]['high'])
                lo = float(df_m30.iloc[j]['low'])
                if side == 'BUY':
                    if hi >= tp:
                        outcome = (tp - entry) / max(sl_dist, 1e-9)
                        exit_price = tp
                        exit_time = df_m30.iloc[j]['time']
                        break
                    if lo <= sl:
                        outcome = -1.0
                        exit_price = sl
                    ...
                    if lo <= tp:
                        outcome = (entry - tp) / max(sl_dist, 1e-9)
                        exit_price = tp
                        exit_time = df_m30.iloc[j]['time']
                        break
                    if hi >= sl:
            ...
            if exit_price is None:
                # time exit at last bar in holding window
                end_i = min(i + max_hold, len(df_m30) - 1)
                exit_price = float(df_m30.iloc[end_i]['close'])
                exit_time = df_m30.iloc[end_i]['time']
                if side == 'BUY':
                    outcome = (exit_price - entry) / max(sl_dist, 1e-9)
                else:
                    outcome = (entry - exit_price) / max(sl_dist, 1e-9)
            ...
            logger.debug("Backtest bar error for %s at %s: %s", symbol, i, e)
            continue

    n = len(trades)
    wins = sum(1 for t in trades if t['r'] > 0)
    win_rate = (wins / n) if n else 0.0
    net = float(sum(t['r'] for t in trades))
    avg_r = (net / n) if n else 0.0

    logger.info("Backtest complete: net=%.4f trades=%d win_rate=%.3f avg_r=%.3f max_dd=%.4f", net, n, win_rate, avg_r, max_dd)
    return {"net": net, "trades": n, "win_rate": win_rate, "max_dd": max_dd, "avg_r": avg_r}
# ===== END BACKTEST FIX =====
