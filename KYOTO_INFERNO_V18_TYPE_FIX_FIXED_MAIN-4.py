...
# === FINAL LIVE EXECUTION SELF-HEAL PATCH (appended last) ===
try:
    _KYOTO_MT5_LOGIN = globals().get("_KYOTO_MT5_LOGIN", None)
    _KYOTO_MT5_PASSWORD = globals().get("_KYOTO_MT5_PASSWORD", None)
    _KYOTO_MT5_SERVER = globals().get("_KYOTO_MT5_SERVER", None)

    def connect_mt5(login=None, password=None, server=None) -> bool:
        global _mt5, _mt5_connected, _KYOTO_MT5_LOGIN, _KYOTO_MT5_PASSWORD, _KYOTO_MT5_SERVER
        if not MT5_AVAILABLE:
            logger.warning("MetaTrader5 python package not installed")
            return False
        try:
        ...
        login = login or _KYOTO_MT5_LOGIN or (int(MT5_LOGIN) if MT5_LOGIN and str(MT5_LOGIN).isdigit() else None)
        password = password or _KYOTO_MT5_PASSWORD or MT5_PASSWORD
        server = server or _KYOTO_MT5_SERVER or MT5_SERVER
        _KYOTO_MT5_LOGIN, _KYOTO_MT5_PASSWORD, _KYOTO_MT5_SERVER = login, password, server
        ...
        try:
            ...
            if _mt5_connected:
                try:
                    _mt5.symbol_select("BTCUSDm", True)
                except Exception:
                    pass
                logger.info("MT5 initialized (login=%s server=%s)", login, server)
            else:
    ...
    def _kyoto_ensure_mt5_live(mt5_module):
        if mt5_module is None:
            return None, "NO_MT5_MODULE"
        try:
            if mt5_module.account_info() is not None:
                return mt5_module, "OK"
        except Exception:
            pass

        login = _KYOTO_MT5_LOGIN or (int(MT5_LOGIN) if MT5_LOGIN and str(MT5_LOGIN).isdigit() else None)
        password = _KYOTO_MT5_PASSWORD or MT5_PASSWORD
        server = _KYOTO_MT5_SERVER or MT5_SERVER
        ...
        try:
            ...
            if login is not None and password is not None and server is not None:
                if mt5_module.initialize(login=login, password=password, server=server):
                    try:
                        mt5_module.symbol_select("BTCUSDm", True)
                    except Exception:
                        pass
                    return mt5_module, "RECONNECTED"
        except Exception:
            pass

        try:
            if mt5_module.account_info() is not None:
                return mt5_module, "OK_AFTER_RETRY"
        except Exception:
            pass
        return mt5_module, "NO_ACCOUNT"

    def _kyoto_wait_for_tick(mt5_module, sym, retries=3, delay=0.5):
        tick = None
    ...
    def _kyoto_final_send_request(mt5_module, req):
        mt5_module, mt5_state = _kyoto_ensure_mt5_live(mt5_module)
        if mt5_module is None:
            return {"retcode": -1, "comment": "NO_MT5_MODULE"}
        if req is None:
            return {"retcode": -1, "comment": "ORDER_REQUEST_NONE"}
        ...
        try:
            info = mt5_module.account_info()
            if info is None:
                return {"retcode": -1, "comment": "NO_ACCOUNT", "request": req}
        except Exception:
            return {"retcode": -1, "comment": "NO_ACCOUNT", "request": req}
        ...
        try:
            ...
            if price_f <= 0.0:
                tick = _kyoto_wait_for_tick(mt5_module, sym, retries=4, delay=0.5)
                if tick is not None:
                    side_is_buy = req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0)
                    side_is_sell = req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1)
                    if side_is_buy and float(getattr(tick, "ask", 0.0) or 0.0) > 0.0:
                        price_f = float(tick.ask)
                    elif side_is_sell and float(getattr(tick, "bid", 0.0) or 0.0) > 0.0:
                        price_f = float(tick.bid)
                    else:
                        price_f = float(getattr(tick, "last", 0.0) or 0.0)
                ...
                if price_f <= 0.0:
                    try:
                        # one last reconnect attempt before giving up
                        if _KYOTO_MT5_LOGIN is not None and _KYOTO_MT5_PASSWORD is not None and _KYOTO_MT5_SERVER is not None:
                            try:
                                mt5_module.shutdown()
                            except Exception:
                                pass
                            if mt5_module.initialize(login=_KYOTO_MT5_LOGIN, password=_KYOTO_MT5_PASSWORD, server=_KYOTO_MT5_SERVER):
                                tick = _kyoto_wait_for_tick(mt5_module, sym, retries=3, delay=0.5)
                                if tick is not None:
                                    side_is_buy = req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0)
                                    side_is_sell = req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1)
                                    if side_is_buy and float(getattr(tick, "ask", 0.0) or 0.0) > 0.0:
                ...
                if price_f <= 0.0:
                    return {"retcode": -1, "comment": "NO_VALID_PRICE", "request": req, "state": mt5_state}

            req["price"] = float(price_f)
    ...
    def place_order_mt5(*args, **kwargs):
        try:
            mt5_module = kwargs.pop("mt5_module", None) or kwargs.pop("module", None) or kwargs.pop("mts_module", None) or globals().get("mt5") or globals().get("MT5")
            req = {}

            if len(args) == 2 and hasattr(args[0], "order_send") and isinstance(args[1], dict):
                mt5_module = args[0]
                req = dict(args[1])
    ...
    globals()["connect_mt5"] = connect_mt5
    ...
    if "UVXExecutionEngine" in globals() and hasattr(UVXExecutionEngine, "market_order"):
        def _kyoto_single_door_uvx_market_order(self, symbol, side, size, sl=None, tp=None):
            ...
            tick = _kyoto_wait_for_tick(self._mt5, sym, retries=4, delay=0.5)
            if tick is not None:
                if str(side).lower() == "buy" and float(getattr(tick, "ask", 0.0) or 0.0) > 0.0:
                    req["price"] = float(tick.ask)
                elif str(side).lower() != "buy" and float(getattr(tick, "bid", 0.0) or 0.0) > 0.0:
                    req["price"] = float(tick.bid)
                else:
                    req["price"] = float(getattr(tick, "last", 0.0) or 0.0)
            if sl is not None:
            ...
            if tp is not None:
                req["tp"] = float(tp)
            return _kyoto_final_send_request(self._mt5, req)

        UVXExecutionEngine.market_order = _kyoto_single_door_uvx_market_order
except Exception:
    try:
        logger.exception("FINAL LIVE EXECUTION SELF-HEAL PATCH failed")
    except Exception:
        pass
# === END FINAL LIVE EXECUTION SELF-HEAL PATCH ===
