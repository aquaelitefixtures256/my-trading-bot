...
_KYOTO_FINAL_FILLING_CACHE = globals().get("_KYOTO_FINAL_FILLING_CACHE", {})

def _kyoto_symbol_fill_modes(mt5_module, symbol, req=None):
    """
    Symbol-aware filling mode order:
    - prefer the symbol's own MT5-reported mode
    - then try any explicit req["type_filling"]
    - then try the common MT5 modes as fallbacks
    """
    modes = []
    try:
        sym = _kyoto_final_symbol(symbol)
    ...
    try:
        info = mt5_module.symbol_info(sym) if mt5_module is not None else None
    except Exception:
        info = None

    try:
        fm = int(getattr(info, "filling_mode", None))
        if fm is not None:
            modes.append(fm)
    except Exception:
        pass
    ...
    try:
        if isinstance(req, dict) and req.get("type_filling") is not None:
            modes.insert(0, int(req["type_filling"]))
    except Exception:
        pass

    for cand in (
        getattr(mt5_module, "ORDER_FILLING_IOC", 1),
        getattr(mt5_module, "ORDER_FILLING_FOK", 0),
        getattr(mt5_module, "ORDER_FILLING_RETURN", 2),
    ):
        try:
            cand = int(cand)
            modes.append(cand)
...
def _kyoto_try_order_send_with_filling_modes(mt5_module, req, *, source="live"):
    """
    Try order_check/order_send across multiple filling modes until one works.
    If a mode is rejected as unsupported, it falls back to the next one.
    """
    if mt5_module is None:
    ...
    if not isinstance(req, dict):
        try:
            ...
            return {"retcode": -1, "comment": "BAD_ORDER_REQUEST", "request": req}
    ...
    tried = []
    last_res = None
    fill_modes = _kyoto_symbol_fill_modes(mt5_module, sym, req)

    for fill_mode in fill_modes:
        trial = dict(req)
        trial["type_filling"] = fill_mode
        tried.append(fill_mode)

        try:
            # order_check is read-only and can reveal unsupported filling mode early.
            try:
                chk = mt5_module.order_check(trial)
                chk_rc = getattr(chk, "retcode", None)
                if isinstance(chk, dict):
                    chk_rc = chk.get("retcode", chk_rc)
                if chk_rc is not None:
                    try:
                        if int(chk_rc) == 10030:
                            logger.info("%s: %s rejected filling=%s (unsupported), retrying fallback", source, sym, fill_mode)
                            last_res = chk
                            continue
                    except Exception:
                        pass
            except Exception:
                # if order_check fails, still attempt send
                pass
            ...
            if res is None:
                try:
                    logger.info(
                        "%s: order_send returned None for %s with filling=%s; last_error=%s",
                        source, sym, fill_mode, mt5_module.last_error()
                    )
                except Exception:
                    logger.info("%s: order_send returned None for %s with filling=%s", source, sym, fill_mode)
                continue

            try:
                rc = getattr(res, "retcode", None)
                if isinstance(res, dict):
                    rc = res.get("retcode", rc)
                if rc is not None:
                    try:
                        if int(rc) == 10030:
                            logger.info("%s: %s returned unsupported filling=%s, retrying fallback", source, sym, fill_mode)
                            continue
                    except Exception:
                        pass
            except Exception:
            ...
            try:
                _KYOTO_FINAL_FILLING_CACHE[sym] = fill_mode
                globals()["_KYOTO_FINAL_FILLING_CACHE"] = _KYOTO_FINAL_FILLING_CACHE
            except Exception:
                pass
            ...
            try:
                logger.exception("%s: order_send exception for %s with filling=%s", source, sym, fill_mode)
            except Exception:
                pass
            continue

    if last_res is None:
        last_res = {"retcode": -1, "comment": "ORDER_SEND_FAILED_ALL_FILLING_MODES", "request": req}
    elif isinstance(last_res, dict):
        last_res = dict(last_res)
        last_res.setdefault("comment", "ORDER_SEND_FAILED_ALL_FILLING_MODES")
        last_res["tried_filling_modes"] = tried
    else:
        try:
            last_res = last_res._asdict()
        except Exception:
            last_res = {"raw": str(last_res)}
        last_res["tried_filling_modes"] = tried
    return last_res
    ...
    try:
        ...
        if req.get("price") in (None, 0, 0.0):
            ...
            try:
                ...
                tick = None
            if tick is not None:
                side = str(req.get("type", "")).lower()
                if side in {"buy", "long"} and hasattr(tick, "ask"):
                    req["price"] = float(tick.ask)
                elif side in {"sell", "short"} and hasattr(tick, "bid"):
        ...
        if "tp" in req and req["tp"] is not None:
            req["tp"] = float(req["tp"])
        # Use symbol-aware filling-mode retries so the same request works across all traded symbols.
        return _kyoto_try_order_send_with_filling_modes(mt5_module, req, source="_kyoto_final_send_request")
    except Exception as e:
        try:
