...
def run_telethon_news_listener():
    ...
    the listener exits cleanly so the main bot keeps running.
    """
    try:
    ...
    async def _main():
        ...
        if session_string and StringSession is not None:
            try:
                ...
                logger.warning("Telethon StringSession invalid; falling back to file session '%s'.", session_name)
                session_obj = session_name
        ...
        if not authorized:
            logger.warning("Telethon session not authorized; listener disabled cleanly.")
            try:
