...
if not _KYOTO_RISK_LAYER_INSTALLED:
    ...
    def calc_trade_plan(symbol, tick, atr_value, equity):
        ...
        if estimated_loss > risk_cash:
            logger.info(
                "RISK BLOCK %s: loss_exceeds_budget est_loss=%.6f risk_cash=%.6f lot=%.4f atr=%.6f spread=%.6f sl=%.6f tp=%.6f",
...
# === BEGIN FINAL DUPLICATE PATH ALIGNMENT ===
try:
    def _kyoto_align_legacy_order_request(mt5_module, order_request, *, source="legacy_duplicate"):
        ok, new_req, reason, plan = _kyoto_risk_apply_to_request(mt5_module, order_request, source=source)
        if not ok:
            return new_req
        final_send = globals().get("_kyoto_final_send_request")
    ...
    _KYOTO_DUP_PREV_order_wrapper = globals().get("order_wrapper")
    if callable(_KYOTO_DUP_PREV_order_wrapper):
        def order_wrapper(mt5_module, order_request):
            return _kyoto_align_legacy_order_request(mt5_module, order_request, source="order_wrapper")
        globals()["order_wrapper"] = order_wrapper

    _KYOTO_DUP_PREV_place_order_mt5 = globals().get("place_order_mt5")
    if callable(_KYOTO_DUP_PREV_place_order_mt5):
        def place_order_mt5(*args, **kwargs):
            req = {}
            if len(args) == 1 and isinstance(args[0], dict):
                req = dict(args[0])
            ...
            if kwargs:
                req.update(kwargs)
            mt5_module = kwargs.get("mt5_module") or globals().get("_mt5") or _kyoto_risk_get_mt5()
            return _kyoto_align_legacy_order_request(mt5_module, req, source="place_order_mt5")
        globals()["place_order_mt5"] = place_order_mt5

    _KYOTO_DUP_PREV_rt_send_request = globals().get("_kyoto_rt_send_request")
    if callable(_KYOTO_DUP_PREV_rt_send_request):
        def _kyoto_rt_send_request(mt5_module, req):
            return _kyoto_align_legacy_order_request(mt5_module, req, source="_kyoto_rt_send_request")
        globals()["_kyoto_rt_send_request"] = _kyoto_rt_send_request

    _KYOTO_DUP_PREV_final_send_request = globals().get("_kyoto_final_send_request")
    if callable(_KYOTO_DUP_PREV_final_send_request):
        def _kyoto_final_send_request(mt5_module, req):
            return _kyoto_align_legacy_order_request(mt5_module, req, source="_kyoto_final_send_request")
        globals()["_kyoto_final_send_request"] = _kyoto_final_send_request

    _KYOTO_DUP_PREV_single_door_submit = globals().get("_kyoto_single_door_submit")
    if callable(_KYOTO_DUP_PREV_single_door_submit):
        def _kyoto_single_door_submit(mt5_module, order_request, *, source="live"):
            return _kyoto_align_legacy_order_request(mt5_module, order_request, source=source)
        globals()["_kyoto_single_door_submit"] = _kyoto_single_door_submit

    _KYOTO_DUP_PREV_UVX = globals().get("UVXExecutionEngine", None)
    if _KYOTO_DUP_PREV_UVX is not None and hasattr(_KYOTO_DUP_PREV_UVX, "market_order"):
        _KYOTO_DUP_ORIG_UVX = getattr(_KYOTO_DUP_PREV_UVX, "market_order")
        def _kyoto_duplicate_uvx_market_order(self, symbol, side, size, sl=None, tp=None):
            try:
                if getattr(self, "mode", "dry_run") != "mt5":
                    return _KYOTO_DUP_ORIG_UVX(self, symbol, side, size, sl=sl, tp=tp)
                mt5_module = getattr(self, "_mt5", None) or _kyoto_risk_get_mt5()
                if mt5_module is None:
                    return {"order_id": None, "status": "blocked", "comment": "NO_MT5_MODULE"}
                resolved = _kyoto_risk_resolve_mt5_symbol(mt5_module, symbol)
                ...
                req = {
                    ...
                    "type": getattr(mt5_module, "ORDER_TYPE_BUY", None) if side_u == "buy" else getattr(mt5_module, "ORDER_TYPE_SELL", None),
                ...
                if side_u == "buy":
                    req["sl"] = float(ask - plan["sl_distance"])
                    req["tp"] = float(ask + plan["tp_distance"])
                else:
                    req["sl"] = float(bid + plan["sl_distance"])
                    req["tp"] = float(bid - plan["tp_distance"])
                return _kyoto_align_legacy_order_request(mt5_module, req, source="UVXExecutionEngine.market_order")
            except Exception:
                logger.exception("duplicate-path UVX market_order failed")
                return _KYOTO_DUP_ORIG_UVX(self, symbol, side, size, sl=sl, tp=tp)
        UVXExecutionEngine.market_order = _kyoto_duplicate_uvx_market_order

    _KYOTO_DUP_PREV_allowed_to_open = globals().get("allowed_to_open")
    if callable(_KYOTO_DUP_PREV_allowed_to_open):
        def allowed_to_open(symbol):
            return _KYOTO_DUP_PREV_allowed_to_open(symbol)
        globals()["allowed_to_open"] = allowed_to_open

    _KYOTO_DUP_PREV_deprecated_order_wrapper = globals().get("_deprecated_order_wrapper")
    if callable(_KYOTO_DUP_PREV_deprecated_order_wrapper):
        def _deprecated_order_wrapper(mt5_module, order_request):
            return _kyoto_align_legacy_order_request(mt5_module, order_request, source="_deprecated_order_wrapper")
        globals()["_deprecated_order_wrapper"] = _deprecated_order_wrapper

    _KYOTO_DUP_PREV_deprecated_execute_signal = globals().get("_deprecated_execute_signal")
    if callable(_KYOTO_DUP_PREV_deprecated_execute_signal):
        def _deprecated_execute_signal(sym, signal, price, mt5_module, symbol_map):
            return _KYOTO_DUP_PREV_deprecated_execute_signal(sym, signal, price, mt5_module, symbol_map)
        globals()["_deprecated_execute_signal"] = _deprecated_execute_signal

    logger.info("KYOTO duplicate live paths aligned to the final MT5 risk door")
except Exception:
    try:
        logger.exception("KYOTO duplicate path alignment failed")
    except Exception:
        pass
# === END FINAL DUPLICATE PATH ALIGNMENT ===


# === BEGIN PART 2: BREAK-EVEN + TRAILING STOP MANAGER OVERRIDE ===
try:
    KYOTO_BE_TRIGGER_R = float(globals().get("KYOTO_BE_TRIGGER_R", 1.0))
except Exception:
    KYOTO_BE_TRIGGER_R = 1.0

try:
    KYOTO_TRAIL_START_R = float(globals().get("KYOTO_TRAIL_START_R", 1.5))
except Exception:
    KYOTO_TRAIL_START_R = 1.5

try:
    KYOTO_STOP_MANAGER_POLL_SEC = float(globals().get("KYOTO_STOP_MANAGER_POLL_SEC", 5.0))
except Exception:
    KYOTO_STOP_MANAGER_POLL_SEC = 5.0

try:
    KYOTO_STOP_MANAGER_MIN_MODIFY_SEC = float(globals().get("KYOTO_STOP_MANAGER_MIN_MODIFY_SEC", 10.0))
except Exception:
    KYOTO_STOP_MANAGER_MIN_MODIFY_SEC = 10.0

try:
    KYOTO_TRAIL_ATR_MULT = float(globals().get("KYOTO_TRAIL_ATR_MULT", 1.0))
except Exception:
    KYOTO_TRAIL_ATR_MULT = 1.0

try:
    KYOTO_BE_BUFFER_POINTS = float(globals().get("KYOTO_BE_BUFFER_POINTS", 2.0))
except Exception:
    KYOTO_BE_BUFFER_POINTS = 2.0

_KYOTO_TRAIL_LAST_MODIFY_TS = {}
_KYOTO_TRAIL_LAST_REASON = {}

def _kyoto_get_cfg(name, default):
    try:
...
def _kyoto_symbol_root(symbol):
    try:
        s = str(symbol or "").upper().strip()
        for suf in (".PRO", "PRO", ".M", "M"):
            if s.endswith(suf):
                return s[:-len(suf)]
        return s
    except Exception:
...
def _kyoto_resolve_active_symbol(mt5_module, symbol, symbol_map=None):
    try:
        if mt5_module is None:
            return None, None
        raw = str(symbol or "").strip()
        root = _kyoto_symbol_root(raw)
        candidates = []
        if isinstance(symbol_map, dict):
            try:
                mapped = symbol_map.get(raw)
                if mapped:
                    candidates.append(str(mapped))
            except Exception:
                pass
            try:
                mapped = symbol_map.get(root)
                if mapped:
                    candidates.append(str(mapped))
            except Exception:
                pass
        ...
        for cand in uniq:
            try:
                info = mt5_module.symbol_info(cand)
                if info is not None:
                    try:
                        mt5_module.symbol_select(cand, True)
                    except Exception:
        ...
        try:
            all_symbols = mt5_module.symbols_get() or []
            for item in all_symbols:
                name = getattr(item, "name", None)
                if not name:
                    continue
                nu = str(name).upper()
                if nu == root or nu.startswith(root):
                    try:
                        info = mt5_module.symbol_info(name)
                        if info is not None:
                            try:
                                mt5_module.symbol_select(name, True)
                            except Exception:
                                pass
                            return name, info
                    except Exception:
...
def _kyoto_tick_is_fresh(tick, max_age_sec=3.0):
    try:
        if tick is None:
            return False
        now = time.time()
        t = getattr(tick, "time_msc", None)
        if t is not None:
            try:
                age = now - (float(t) / 1000.0)
...
def _kyoto_symbol_point(symbol_info):
    try:
        return float(getattr(symbol_info, "point", 0.0) or 0.0)
    except Exception:
        return 0.0

def _kyoto_symbol_stops_points(symbol_info):
    try:
    ...
    try:
        ...
        freeze = 0
    return max(0, stops, freeze)

def _kyoto_min_stop_distance(symbol_info):
    try:
        point = _kyoto_symbol_point(symbol_info)
        if point <= 0:
            return 0.0
        return point * max(1, _kyoto_symbol_stops_points(symbol_info))
    except Exception:
        return 0.0

def _kyoto_spread_ok(symbol, symbol_info, tick, atr_value=None):
    try:
        bid, ask = _kyoto_tick_prices(tick)
        if bid <= 0 or ask <= 0 or ask <= bid:
            return False, "invalid_tick_spread"
        spread = ask - bid
        point = _kyoto_symbol_point(symbol_info) or 0.0
        if point <= 0:
            point = spread / 2.0 if spread > 0 else 0.0
        root = _kyoto_symbol_root(symbol)
        stops = _kyoto_symbol_stops_points(symbol_info)
        base_min = max(_kyoto_min_stop_distance(symbol_info), point * max(1.0, float(stops or 0)))
        if root.startswith("BTC"):
            atr_spread_cap = float(atr_value) * 0.20 if atr_value else 0.0
            price_cap = max(ask * 0.003, point * 1500.0)
            limit = max(base_min, atr_spread_cap, price_cap)
        elif root.startswith("XAU"):
            atr_spread_cap = float(atr_value) * 0.15 if atr_value else 0.0
            limit = max(base_min, atr_spread_cap, point * 150.0)
        elif root.startswith("USOIL"):
            atr_spread_cap = float(atr_value) * 0.15 if atr_value else 0.0
            limit = max(base_min, atr_spread_cap, point * 100.0)
        elif root.endswith("JPY"):
            limit = max(base_min, point * 25.0)
        else:
            limit = max(base_min, point * 20.0)
        if spread > limit:
            return False, f"spread_too_wide:{spread:.8f}>{limit:.8f}"
        return True, "ok"
    except Exception as e:
        return False, f"spread_check_error:{e}"

def _kyoto_timeframe_from_config(mt5_module):
    try:
        cfg = globals().get("CONFIG", {})
        tf = None
        if isinstance(cfg, dict):
            for key in ("EXECUTION_TIMEFRAME", "TIMEFRAME", "TRADE_TIMEFRAME", "SCAN_TIMEFRAME", "BAR_TIMEFRAME"):
                if cfg.get(key) is not None:
                    tf = cfg.get(key)
                    break
        if tf is None:
            tf = globals().get("EXECUTION_TIMEFRAME", None) or globals().get("TIMEFRAME", None) or "M30"
        if isinstance(tf, int):
            return tf
        tfs = str(tf).upper().strip()
        if not tfs.startswith("TIMEFRAME_"):
            tfs = "TIMEFRAME_" + tfs
        return getattr(mt5_module, tfs, getattr(mt5_module, "TIMEFRAME_M30", None))
    except Exception:
        return getattr(mt5_module, "TIMEFRAME_M30", None)

def _kyoto_compute_atr(mt5_module, symbol, timeframe=None, period=14, bars=60):
    try:
        if mt5_module is None:
            return None
        tf = timeframe if timeframe is not None else _kyoto_timeframe_from_config(mt5_module)
        if tf is None:
            tf = getattr(mt5_module, "TIMEFRAME_M30", None)
        rates = mt5_module.copy_rates_from_pos(symbol, tf, 0, max(int(bars), int(period) + 2))
        ...
        for bar in rates:
            try:
                highs.append(float(bar["high"] if isinstance(bar, dict) else bar[2]))
                lows.append(float(bar["low"] if isinstance(bar, dict) else bar[3]))
                closes.append(float(bar["close"] if isinstance(bar, dict) else bar[4]))
            except Exception:
                continue
        if len(highs) < int(period) + 1:
            return None
        trs = []
        prev_close = closes[0]
        ...
        if len(trs) < int(period):
            return None
        window = trs[-int(period):]
        atr = sum(window) / float(len(window))
        return float(atr) if atr > 0 else None
    except Exception:
        return None

def _kyoto_position_risk_distance(pos):
    try:
        entry = _kyoto_position_entry(pos)
        sl = _kyoto_position_sl(pos)
        if entry is None or sl is None:
...
def _kyoto_position_profit_r(pos, tick):
    try:
        side = _kyoto_position_side(pos)
        entry = _kyoto_position_entry(pos)
        if side is None or entry is None or tick is None:
            return None
        bid, ask = _kyoto_tick_prices(tick)
        if side == "BUY":
            if bid <= 0:
                return None
            return (bid - float(entry))
        if ask <= 0:
            return None
...
def should_arm_breakeven(position, tick, risk_distance, be_trigger_r=None):
    try:
        if position is None or tick is None or risk_distance is None or float(risk_distance) <= 0:
            return False, None
        side = _kyoto_position_side(position)
        entry = _kyoto_position_entry(position)
        if side not in ("BUY", "SELL") or entry is None:
            return False, None
        trigger = float(be_trigger_r if be_trigger_r is not None else _kyoto_get_cfg("KYOTO_BE_TRIGGER_R", KYOTO_BE_TRIGGER_R))
        bid, ask = _kyoto_tick_prices(tick)
        if side == "BUY":
            if bid <= 0:
                return False, None
            profit_r = (bid - float(entry)) / float(risk_distance)
        else:
            if ask <= 0:
                return False, None
            profit_r = (float(entry) - ask) / float(risk_distance)
        return bool(profit_r >= trigger), float(profit_r)
    except Exception:
        return False, None

def compute_breakeven_sl(position, tick, symbol_info=None, buffer_points=None):
    try:
        side = _kyoto_position_side(position)
        entry = _kyoto_position_entry(position)
        if side not in ("BUY", "SELL") or entry is None or tick is None:
            return None, "missing_position_data"
        bid, ask = _kyoto_tick_prices(tick)
        if bid <= 0 or ask <= 0:
            return None, "invalid_tick"
        point = _kyoto_symbol_point(symbol_info) if symbol_info is not None else 0.0
        min_stop = _kyoto_min_stop_distance(symbol_info) if symbol_info is not None else 0.0
        buffer_pts = float(buffer_points if buffer_points is not None else _kyoto_get_cfg("KYOTO_BE_BUFFER_POINTS", KYOTO_BE_BUFFER_POINTS))
        positive_buffer = max(point * max(1.0, buffer_pts), (ask - bid) * 0.25, min_stop)
        if side == "BUY":
            return float(entry + positive_buffer), "ok"
        return float(entry - positive_buffer), "ok"
    except Exception as e:
        return None, f"breakeven_calc_error:{e}"

def compute_trailing_sl(position, tick, atr_value, symbol_info=None):
    try:
        side = _kyoto_position_side(position)
        if side not in ("BUY", "SELL") or tick is None:
            return None, "missing_position_data"
        bid, ask = _kyoto_tick_prices(tick)
        if bid <= 0 or ask <= 0:
            return None, "invalid_tick"
        ...
        min_stop = _kyoto_min_stop_distance(symbol_info) if symbol_info is not None else 0.0
        spread = ask - bid
        atr_part = float(atr_value) * float(_kyoto_get_cfg("KYOTO_TRAIL_ATR_MULT", KYOTO_TRAIL_ATR_MULT)) if atr_value else 0.0
        trail_distance = max(atr_part, spread * 3.0, min_stop, point * 2.0)
        if trail_distance <= 0:
            return None, "invalid_trail_distance"
        if side == "BUY":
            return float(bid - trail_distance), "ok"
        return float(ask + trail_distance), "ok"
    except Exception as e:
        return None, f"trail_calc_error:{e}"

def is_better_stop(current_sl, candidate_sl, side):
    try:
...
def enforce_broker_min_stop(symbol, tick, entry_price, sl_price, tp_price, side, mt5_module=None, symbol_info=None, current_sl=None, current_tp=None):
    try:
        if tick is None:
        ...
        if symbol_info is None and mt5_module is not None:
            try:
                symbol_info = mt5_module.symbol_info(symbol)
            except Exception:
                symbol_info = None
        if symbol_info is None:
            return {"ok": False, "reason": "missing_symbol_info"}
        point = _kyoto_symbol_point(symbol_info)
        if point <= 0:
            return {"ok": False, "reason": "missing_point"}
        min_stop = _kyoto_min_stop_distance(symbol_info)
        freeze_points = max(0, _kyoto_symbol_stops_points(symbol_info))
        freeze_dist = point * freeze_points
        side_u = str(side).upper()
        entry = float(entry_price) if entry_price is not None else None
        candidate_sl = float(sl_price) if sl_price is not None else None
        candidate_tp = float(tp_price) if tp_price is not None else None

        if candidate_sl is None:
            return {"ok": False, "reason": "missing_sl"}

        if side_u == "BUY":
            max_valid_sl = bid - max(min_stop, freeze_dist)
            if candidate_sl > max_valid_sl:
                adjusted = max_valid_sl
            else:
                adjusted = candidate_sl
            if adjusted >= bid:
                return {"ok": False, "reason": "sl_not_below_market"}
            if current_sl is not None and adjusted <= float(current_sl):
                return {"ok": False, "reason": "new_sl_not_better_than_current"}
            if entry is not None and adjusted <= entry:
                # allow breakeven buffer that may land at/above entry only when it is valid and better than current
                pass
            if candidate_tp is not None:
                min_valid_tp = ask + max(min_stop, freeze_dist)
                tp_adjusted = candidate_tp if candidate_tp >= min_valid_tp else min_valid_tp
                if current_tp is not None and tp_adjusted < float(current_tp):
                    tp_adjusted = current_tp
            else:
                tp_adjusted = current_tp
        elif side_u == "SELL":
            min_valid_sl = ask + max(min_stop, freeze_dist)
            if candidate_sl < min_valid_sl:
                adjusted = min_valid_sl
            else:
                adjusted = candidate_sl
            if adjusted <= ask:
                return {"ok": False, "reason": "sl_not_above_market"}
            if current_sl is not None and adjusted >= float(current_sl):
                return {"ok": False, "reason": "new_sl_not_better_than_current"}
            if candidate_tp is not None:
                max_valid_tp = bid - max(min_stop, freeze_dist)
                tp_adjusted = candidate_tp if candidate_tp <= max_valid_tp else max_valid_tp
                if current_tp is not None and tp_adjusted > float(current_tp):
                    tp_adjusted = current_tp
            else:
                tp_adjusted = current_tp
        ...
        return {
            ...
            "side": side_u,
            "sl": float(adjusted),
            "tp": None if tp_price is None and current_tp is None else float(tp_adjusted) if tp_adjusted is not None else None,
            "min_stop_distance": float(min_stop),
            "freeze_distance": float(freeze_dist),
            "point": float(point),
            "bid": float(bid),
            "ask": float(ask),
        }
    except Exception as e:
        return {"ok": False, "reason": f"broker_stop_enforce_error:{e}"}

def modify_position_sl(mt5_module, pos, new_sl, new_tp=None, reason=""):
    try:
        if mt5_module is None or pos is None:
            return False
        ticket = getattr(pos, "ticket", None) or getattr(pos, "order", None) or getattr(pos, "position_id", None)
        symbol = getattr(pos, "symbol", None)
        if ticket is None or not symbol:
            logger.info("Skipped stop update: missing ticket/symbol")
            return False
        req = {
            "action": getattr(mt5_module, "TRADE_ACTION_SLTP", None),
            "position": int(ticket),
        ...
        if new_tp is not None:
            ...
            current_tp = _kyoto_position_tp(pos)
            if current_tp is not None:
                req["tp"] = float(current_tp)
        res = mt5_module.order_send(req)
        retcode = getattr(res, "retcode", None)
        ok = bool(retcode in (0, 10009, 10008) or str(retcode) == "0")
        if ok:
            logger.info("Stop modified for %s ticket=%s reason=%s new_sl=%s", symbol, ticket, reason or "update", new_sl)
            return True
        logger.info("Skipped stop update: modify rejected ticket=%s retcode=%s reason=%s", ticket, retcode, reason or "update")
        return False
    except Exception as e:
        try:
            logger.exception("Failed to modify SL for position %s: %s", getattr(pos, "ticket", "?"), e)
        except Exception:
            pass
        return False

def _kyoto_modify_position_sl_tp(mt5_module, pos, new_sl, new_tp=None, reason=""):
    try:
        symbol = getattr(pos, "symbol", None)
        if not symbol:
            logger.info("Skipped stop update: missing symbol")
            return False
        resolved_symbol, symbol_info = _kyoto_resolve_active_symbol(mt5_module, symbol)
        if not resolved_symbol or symbol_info is None:
            logger.info("Skipped stop update: symbol not active %s", symbol)
            return False
        try:
            tick = mt5_module.symbol_info_tick(resolved_symbol)
        except Exception:
            tick = None
        if not _kyoto_tick_is_fresh(tick, max_age_sec=3.0):
            logger.info("Skipped stop update: stale tick %s", resolved_symbol)
            return False

        side = _kyoto_position_side(pos)
        current_sl = _kyoto_position_sl(pos)
        current_tp = _kyoto_position_tp(pos)
        entry = _kyoto_position_entry(pos)

        enforced = enforce_broker_min_stop(
            resolved_symbol,
            tick,
            entry,
            new_sl,
        ...
        if not enforced.get("ok"):
            logger.info(
                "Skipped stop update: %s symbol=%s ticket=%s",
                enforced.get("reason", "invalid"),
                resolved_symbol,
                getattr(pos, "ticket", "?"),
            )
            return False

        final_sl = float(enforced["sl"])
        final_tp = enforced.get("tp", current_tp)

        if not is_better_stop(current_sl, final_sl, side):
            logger.info(
                "Skipped stop update: new SL not better than current SL symbol=%s ticket=%s",
                resolved_symbol,
                getattr(pos, "ticket", "?"),
            )
            return False

        return modify_position_sl(mt5_module, pos, final_sl, final_tp, reason=reason or "breakeven_trailing")
    except Exception as e:
        try:
            logger.exception("Failed to modify SL/TP for position %s: %s", getattr(pos, "ticket", "?"), e)
        except Exception:
            pass
        return False
...
def trailing_manager_thread(stop_event, mt5_module, symbol_map):
    """
    Break-even then trailing stop manager.
    """
    logger.info("Started background thread: trailing-manager")
    while not stop_event.is_set():
        try:
            if mt5_module is None:
                time.sleep(float(_kyoto_get_cfg("KYOTO_STOP_MANAGER_POLL_SEC", KYOTO_STOP_MANAGER_POLL_SEC)))
                continue

            try:
                positions = mt5_module.positions_get() or []
            except Exception as e:
                logger.info("Skipped stop update: MT5 positions unavailable: %s", e)
                time.sleep(float(_kyoto_get_cfg("KYOTO_STOP_MANAGER_POLL_SEC", KYOTO_STOP_MANAGER_POLL_SEC)))
                continue
            ...
            for pos in positions:
                try:
                    ...
                    resolved_symbol, symbol_info = _kyoto_resolve_active_symbol(mt5_module, symbol, symbol_map=symbol_map)
                    if not resolved_symbol or symbol_info is None:
                        logger.info("Skipped stop update: symbol not active %s", symbol)
                        continue
                    ...
                    ok_spread, spread_reason = _kyoto_spread_ok(resolved_symbol, symbol_info, tick)
                    if not ok_spread:
                        logger.info("Skipped stop update: %s symbol=%s", spread_reason, resolved_symbol)
                        continue

                    current_sl = _kyoto_position_sl(pos)
                    current_tp = _kyoto_position_tp(pos)
                    entry = _kyoto_position_entry(pos)
                    risk_distance = _kyoto_position_risk_distance(pos)
                    if entry is None or risk_distance is None:
                        continue

                    modify_key = getattr(pos, "ticket", None) or getattr(pos, "order", None) or getattr(pos, "position_id", None)
                    last_mod = _KYOTO_TRAIL_LAST_MODIFY_TS.get(modify_key, 0.0)
                    if now - float(last_mod) < float(_kyoto_get_cfg("KYOTO_STOP_MANAGER_MIN_MODIFY_SEC", KYOTO_STOP_MANAGER_MIN_MODIFY_SEC)):
                        continue

                    armed, profit_r = should_arm_breakeven(pos, tick, risk_distance)
                    if armed:
                        be_sl, be_reason = compute_breakeven_sl(pos, tick, symbol_info=symbol_info)
                        if be_sl is not None and is_better_stop(current_sl, be_sl, side):
                            done = _kyoto_modify_position_sl_tp(
                                mt5_module,
                                pos,
                                be_sl,
                                current_tp,
                            ...
                            if done:
                                _KYOTO_TRAIL_LAST_MODIFY_TS[modify_key] = now
                                logger.info(
                                    "Break-even armed for %s ticket=%s profit_r=%.2f",
                                    resolved_symbol,
                                    modify_key,
                                    float(profit_r if profit_r is not None else 0.0),
                                )
                            continue

                    if profit_r is None or float(profit_r) < float(_kyoto_get_cfg("KYOTO_TRAIL_START_R", KYOTO_TRAIL_START_R)):
                        continue

                    atr_value = _kyoto_compute_atr(mt5_module, resolved_symbol)
                    trail_sl, trail_reason = compute_trailing_sl(pos, tick, atr_value, symbol_info=symbol_info)
                    if trail_sl is None:
                        logger.info(
                            "Skipped stop update: trailing calc failed symbol=%s reason=%s",
                            resolved_symbol,
                            trail_reason,
                        )
                        continue

                    if not is_better_stop(current_sl, trail_sl, side):
                        continue
                    ...
                    if done:
                        _KYOTO_TRAIL_LAST_MODIFY_TS[modify_key] = now
                        logger.info(
                            "Trailing stop updated for %s ticket=%s profit_r=%.2f",
                            resolved_symbol,
                            modify_key,
                            float(profit_r if profit_r is not None else 0.0),
                        )
                    ...
                    try:
                        logger.exception("Trailing manager skipped a position due to error: %s", e)
                    except Exception:
                        pass
                    continue
            ...
            try:
                ...
                pass
            time.sleep(float(_kyoto_get_cfg("KYOTO_STOP_MANAGER_POLL_SEC", KYOTO_STOP_MANAGER_POLL_SEC)))
# === END PART 2: BREAK-EVEN + TRAILING STOP MANAGER OVERRIDE ===
