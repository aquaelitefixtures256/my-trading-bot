...
def load_v15_module():
    ...
    try:
        ...
        logger.exception("Failed to load embedded V15 module in load_v15_module: %s", e)
        # Keep the bot deterministic: attach the existing MT5-based adapter instead of
        # falling back to random or simulated signals.
        try:
...
try:
    GLOBAL_MAX_OPEN_TRADES = 15
    MAX_OPEN_TRADES = 15
    SYMBOL_TRADE_LIMITS = {"BTCUSD": 5, "USOIL": 5, "XAUUSD": 2, "EURUSD": 10, "USDJPY": 10}
except Exception:
    pass
try:
    CONFIG.setdefault("BACKTEST_PARAMS", {})
