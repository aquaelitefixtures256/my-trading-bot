    ...
    while not stop_event.is_set():
        try:
            for sym in CONFIG.get("WATCH_SYMBOLS", []):
                ...
                # Detect regime
                params = CONFIG.get("BACKTEST_PARAMS", {}).get(sym, {})
                live_signal_thresh = params.get("signal_thresh", 0.88)
...
def run_backtest(v15_module, symbol='XAUUSD', days=30):
    ...
    for i, bar in enumerate(bars):
        ...
        try:
            ...
            params = {}
        signal_thresh = params.get("signal_thresh", 0.88) # default old behavior
    ...
    while not (stop_event and getattr(stop_event, "is_set", lambda: False)()):
        try:
            for sym in watch:
                ...
                params = CONFIG.get("BACKTEST_PARAMS", {}).get(sym, {})
                live_signal_thresh = float(params.get("signal_thresh", 0.88))
...
try:
    if "CONFIG" in globals():
        CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = max(float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.88)
except Exception:
    pass
...
def _deprecated_execute_signal(sym, signal, price, mt5_module, symbol_map):
    ...
    try:
        ...
        symbol_thresh = float(params.get("signal_thresh", CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.88)))
        exec_thresh = float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.88))
