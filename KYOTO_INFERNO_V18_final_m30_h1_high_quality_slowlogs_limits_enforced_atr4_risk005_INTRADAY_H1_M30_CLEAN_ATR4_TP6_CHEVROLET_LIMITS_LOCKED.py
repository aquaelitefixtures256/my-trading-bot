...
def allowed_to_open(symbol: str):
    ...
    try:
        ...
        if total >= int(globals().get("GLOBAL_MAX_OPEN_TRADES", 15)):
            return False, f"global_max_open_reached:{total}"
...
# Preserve the v15 contract, but never crash the bot if the embedded source is malformed.
def load_v15_module():
    m = types.ModuleType("v15_impl")
    m.__dict__.update({"__name__": "v15_impl", "__file__": "<embedded V15>"})
    try:
        exec(__V15_SOURCE__, m.__dict__)
    except Exception as e:
        try:
...
def _kyoto_limit_reserve(symbol):
    ...
    try:
        ...
        with lock:
            ...
            gmax = int(globals().get("GLOBAL_MAX_OPEN_TRADES", 15))
            limits = dict(globals().get("SYMBOL_TRADE_LIMITS", {"USOIL": 5, "BTCUSD": 5, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}))
