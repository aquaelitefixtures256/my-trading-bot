...
def _kyoto_final_send_request(mt5_module, req):
    ...
    try:
        ...
        return {"retcode": -1, "comment": "NO_ACCOUNT"}
    try:
        raw_sym = req.get("symbol") or req.get("instrument")
        sym = _kyoto_final_symbol(raw_sym)
        if not sym:
            return {"retcode": -1, "comment": "NO_SYMBOL", "request": req}

        # Resolve broker symbol before asking MT5 for tick data.
        broker_sym = sym
        try:
            ...
            if callable(mapper):
                mapped = mapper(sym)
                if mapped:
                    broker_sym = _kyoto_final_symbol(mapped)
        except Exception:
            broker_sym = sym

        req["symbol"] = broker_sym
        ...
        # Make sure the broker symbol is selected before we ask for a tick.
        try:
            if hasattr(mt5_module, "symbol_select"):
                mt5_module.symbol_select(broker_sym, True)
        except Exception:
            pass
        ...
        if req.get("price") in (None, 0, 0.0):
            ...
            for _ in range(3):
                ...
                if tick is not None and (
                    getattr(tick, "ask", 0.0) not in (None, 0, 0.0)
                    or getattr(tick, "bid", 0.0) not in (None, 0, 0.0)
                    or getattr(tick, "last", 0.0) not in (None, 0, 0.0)
                ):
                    break
                try:
                    import time as _time
            ...
            if tick is not None:
                if req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0) and getattr(tick, "ask", 0.0) not in (None, 0, 0.0):
                    req["price"] = float(tick.ask)
                elif req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1) and getattr(tick, "bid", 0.0) not in (None, 0, 0.0):
            ...
            # Final fallback to symbol info if the tick still failed.
            if req.get("price") in (None, 0, 0.0):
                try:
                    si = mt5_module.symbol_info(broker_sym)
                except Exception:
                    si = None
                if si is not None:
                    if req["type"] == getattr(mt5_module, "ORDER_TYPE_BUY", 0) and getattr(si, "ask", 0.0) not in (None, 0, 0.0):
                        req["price"] = float(si.ask)
                    elif req["type"] == getattr(mt5_module, "ORDER_TYPE_SELL", 1) and getattr(si, "bid", 0.0) not in (None, 0, 0.0):
                        req["price"] = float(si.bid)
                    elif getattr(si, "last", 0.0) not in (None, 0, 0.0):
                        req["price"] = float(si.last)
