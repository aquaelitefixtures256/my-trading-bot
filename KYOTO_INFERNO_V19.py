...
CONFIG["BACKTEST_PARAMS"].update({
    ...
    "XAUUSD":  {
        "signal_thresh": 0.60,        # stronger entry filter (fewer noise trades)
        "dxy_gate_thresh": 0.20,      # require stronger DXY confirmation (helps direction)
        "atr_pct_thresh": 0.0015,     # unchanged
        "sl_atr_mult": 1.0,           # tighten SL (less room for mid-size losers)
        "tp_atr_mult": 2.8,           # keep TP > SL to preserve R:R (give winners room)
...
CONFIG["BACKTEST_PARAMS"].update({
    "BTCUSD": {"signal_thresh": 0.50, "atr_pct_thresh": 0.0012, "max_hold": 60},
    "BTCUSDm": {"signal_thresh": 0.50, "atr_pct_thresh": 0.0012, "max_hold": 60},

    "EURUSD": {"signal_thresh": 0.40, "atr_pct_thresh": 0.0008, "max_hold": 60},
    "EURUSDm": {"signal_thresh": 0.40, "atr_pct_thresh": 0.0008, "max_hold": 60},

    "USDJPY": {"signal_thresh": 0.40, "atr_pct_thresh": 0.0009, "max_hold": 60},
    "USDJPYm": {"signal_thresh": 0.40, "atr_pct_thresh": 0.0009, "max_hold": 60},

        "XAUUSD":  {"signal_thresh": 0.58, "dxy_gate_thresh": 0.20, "atr_pct_thresh": 0.0015, "sl_atr_mult": 1.0, "tp_atr_mult": 2.8, "max_hold": 30, "max_loss_abs": 18.0},
    "XAUUSDm": {"signal_thresh": 0.58, "dxy_gate_thresh": 0.20, "atr_pct_thresh": 0.0015, "sl_atr_mult": 1.0, "tp_atr_mult": 2.8, "max_hold": 30, "max_loss_abs": 18.0},




    "USOIL": {"signal_thresh": 0.42, "atr_pct_thresh": 0.0010, "max_hold": 60},
    "USOILm": {"signal_thresh": 0.42, "atr_pct_thresh": 0.0010, "max_hold": 60},

    "DXY": {"signal_thresh": 0.45, "atr_pct_thresh": 0.0008, "max_hold": 60},
    "DXYm": {"signal_thresh": 0.45, "atr_pct_thresh": 0.0008, "max_hold": 60},
})
...
def load_v15_module():
    """Executes the embedded V15 source into a fresh module and returns it.
    This ensures V15 code is available but not executed at import time for V16."""
    m = types.ModuleType("v15_impl")
    m.__dict__.update({
        "__name__": "v15_impl",
        "__file__": "<embedded V15>",
    })
    exec(__V15_SOURCE__, m.__dict__)
    return m
        ...
        try:
            ...
            if bars_list is None:
                ...
                try:
                    ...
                    pass
                tf = getattr(mt5, "TIMEFRAME_M1", mt5.TIMEFRAME_M1)
...
def order_wrapper(mt5_module, order_request):
    ...
    if CONFIG.get("DRY_RUN") or CONFIG.get("DRY_RUN_FLAG") or mt5_module is None:
        logger.info("DRY_RUN active or MT5 not available - simulated order: %s", order_request)
        return {"retcode": 0, "comment": "DRY_RUN_SIMULATED", "order": order_request}
...
def execute_signal(sym, signal, price, mt5_module, symbol_map):
    ...
    try:
        ...
        if CONFIG.get("DRY_RUN") or CONFIG.get("DRY_RUN_FLAG"):
            ...
            return {"simulated": True, "reason": "DRY_RUN"}
        if signal is None:
            ...
            for sym in CONFIG.get("WATCH_SYMBOLS", []):
                ...
                try:
                    if mt5_module:
                        mapped = symbol_map.get(sym, sym) if symbol_map else sym
                        try:
                            rates = mt5_module.copy_rates_from_pos(mapped, getattr(mt5_module, "TIMEFRAME_M1", 0), 0, 120) or []
                        except Exception:
                            rates = []
                ...
                try:
                    if v15_module and hasattr(v15_module, "compute_signal"):
                        signal = v15_module.compute_signal(sym, price, {})
            ...
            try:
                ...
                if 'dxy_bars' not in globals():
                    try:
                        ...
                        dxy_raw = mt5.copy_rates_from_pos(d_res if d_res else "DXY", mt5.TIMEFRAME_M1, 0, len(bars))
...
# compute SL/TP for XAU with absolute cap (insert where you compute sl_price/tp_price)
                    ...
                    if sl_mult is not None or tp_mult is not None:
                        ...
                        try:
                            ...
                            pass
                        _sl_mult = (_sl_mult if (_sl_mult is not None) else locals().get("sl_mult", 1.0))
                        _tp_mult = (_tp_mult if (_tp_mult is not None) else locals().get("tp_mult", 2.0))
                        try:
    ...
    try:
        ...
        def _recent_closes(mapped_symbol, count=50):
            if not mt5_module or not mapped_symbol:
                return []
            try:
                rates = mt5_module.copy_rates_from_pos(mapped_symbol, getattr(mt5_module, "TIMEFRAME_M1", mt5_module.TIMEFRAME_M1), 0, count)
            except Exception:
                rates = None
            if rates is None:
                return []
...
# ===================== V19 PATCH: execution rules and intraday guard =====================

GLOBAL_MAX_OPEN_TRADES = 17
SYMBOL_TRADE_LIMITS = {
    "BTCUSD": 7,
    "USOIL": 7,
    "EURUSD": 10,
...
CONFIG["TRADE_TIMEFRAME"] = "M30"
CONFIG["TRADE_TIMEFRAME_LABEL"] = "M30"
CONFIG["TRADE_TIMEFRAME_BARS"] = 180
CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.50))

def _v19_normalize_symbol(symbol):
    s = str(symbol or "").upper().strip()
    for suf in ("M", ".M", ".PRO", "PRO", "-M", "_M"):
        if s.endswith(suf):
            s = s[: -len(suf)]
    return s
...
def count_open_positions(mt5_module=None):
    """
    Returns (total_open, per_symbol_dict) using MT5 positions when available.
    """
    total = 0
    per = {}
    try:
        if mt5_module and hasattr(mt5_module, "positions_get"):
            positions = mt5_module.positions_get() or []
            total = len(positions)
            for p in positions:
                try:
...
def allowed_to_open(symbol, mt5_module=None):
    s = _v19_normalize_symbol(symbol)
    limit = int(SYMBOL_TRADE_LIMITS.get(s, 10))
    try:
        total, per = count_open_positions(mt5_module)
        sym_count = int(per.get(s, 0))
        if total >= int(GLOBAL_MAX_OPEN_TRADES):
            return False, f"global_max_open_reached:{total}/{GLOBAL_MAX_OPEN_TRADES}"
        if sym_count >= limit:
            return False, f"symbol_limit_reached:{s}:{sym_count}/{limit}"
        return True, "ok"
    except Exception as e:
        logger.exception("allowed_to_open failed for %s", symbol)
        return False, f"error:{e}"

def _v19_build_sl_tp(mt5_module, symbol, side, price):
    """
    Always returns (sl, tp). Uses M30 bars when available, otherwise a safe price-distance fallback.
    """
    symbol_u = _v19_normalize_symbol(symbol)
    side_l = str(side).lower()
    price_f = float(price or 0.0)

    atr = 0.0
    try:
        tf = getattr(mt5_module, "TIMEFRAME_M30", None) if mt5_module else None
        if mt5_module and tf is not None and hasattr(mt5_module, "copy_rates_from_pos"):
            rates = mt5_module.copy_rates_from_pos(symbol, tf, 0, int(CONFIG.get("TRADE_TIMEFRAME_BARS", 180))) or []
            bars = []
            for r in (rates[-90:] if rates else []):
                try:
                    bars.append({
                        "time": int(r[0]),
                        "open": float(r[1]),
            ...
            if bars:
                try:
                    atr = float(_compute_atr_from_recent(bars, period=14))
                except Exception:
                    try:
                        atr, _sma = compute_atr_from_ctx(bars)
                    except Exception:
                        atr = 0.0
    except Exception:
        atr = 0.0
    ...
    if atr <= 0:
        if symbol_u.startswith("XAU"):
            atr = max(price_f * 0.0025, 1.0)
        elif symbol_u.startswith("BTC"):
            atr = max(price_f * 0.0060, 50.0)
        elif symbol_u.startswith("USOIL") or "OIL" in symbol_u:
            atr = max(price_f * 0.0040, 0.50)
        elif symbol_u.endswith("JPY"):
            atr = max(price_f * 0.0015, 0.10)
        else:
            atr = max(price_f * 0.0012, 0.0008)

    params = (CONFIG.get("BACKTEST_PARAMS", {}) or {}).get(symbol_u, {}) or {}
    sl_mult = float(params.get("sl_atr_mult", 1.25))
    tp_mult = float(params.get("tp_atr_mult", 2.0))

    stop_dist = max(atr * sl_mult, price_f * 0.0008 if price_f > 0 else atr * sl_mult)
    take_dist = max(atr * tp_mult, stop_dist * 1.2)

    if side_l in ("buy", "long"):
        sl = price_f - stop_dist
        tp = price_f + take_dist
    else:
        sl = price_f + stop_dist
        tp = price_f - take_dist
...
def _v19_send_trade_telegram(sym, side, volume, entry, sl, tp, result=None):
    try:
        msg = (
            f"Trade opened\n"
            f"Symbol: {sym}\n"
        ...
        if result is not None:
            try:
                if isinstance(result, dict):
                    msg += f"\nResult: {result.get('retcode', result.get('comment', 'ok'))}"
                else:
                    msg += f"\nResult: {getattr(result, 'retcode', 'ok')}"
            except Exception:
                pass
        ...
        logger.exception("Telegram notification failed for %s", sym)
    return False

def order_wrapper(mt5_module, order_request):
    """
    V19 wrapper:
    - enforces global/per-symbol open trade limits
    - guarantees SL/TP on every open order
    - sends Telegram notifications after successful opens
    """
    if file_flag(CONFIG.get("KILL_TRADING_FILE")):
        logger.critical("KILL_TRADING flag present - refusing to send orders.")
        return {"retcode": -1, "comment": "KILLED"}
    ...
    try:
        ...
        if side in ("buy", "long", "sell", "short"):
            allowed, reason = allowed_to_open(sym, mt5_module)
            if not allowed:
                logger.info("Trade blocked for %s: %s", sym, reason)
                return {"retcode": -1, "comment": reason}
        ...
        if tick is not None and hasattr(tick, "ask") and hasattr(tick, "bid"):
            ...
            if isinstance(base_max_spread, dict):
                max_spread = float(
                    base_max_spread.get(sym,
                    base_max_spread.get(str(sym).upper(), base_max_spread.get("DEFAULT", 0.6)))
                )
            else:
                max_spread = float(base_max_spread)
        ...
        if req.get("sl") in (None, 0, 0.0) or req.get("tp") in (None, 0, 0.0):
            sl, tp = _v19_build_sl_tp(mt5_module, sym, side, price_f)
            req["sl"] = sl
            req["tp"] = tp
        ...
        ok = False
        if isinstance(res, dict):
            ok = str(res.get("retcode", "")) in ("0", "10009", "10008")
        else:
            ok = str(getattr(res, "retcode", "")) in ("0", "10009", "10008")

        if ok:
            _v19_send_trade_telegram(sym, "buy" if side_is_buy else ("sell" if side_is_sell else side), req.get("volume"), req.get("price"), req.get("sl"), req.get("tp"), result=res)
...
def execute_signal(sym, signal, price, mt5_module, symbol_map):
    """
    V19 execution helper that keeps the bot intraday and ensures trade limits + SL/TP.
    """
    try:
        threshold = float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.40))
        if str(sym).upper().startswith(("DXY", "US10Y")):
        ...
        if not allowed:
            ...
            return {"simulated": False, "reason": reason}

        side = "buy" if signal > 0 else "sell"
        mapped = symbol_map.get(sym, sym) if symbol_map else sym
        volume = float(CONFIG.get("DEFAULT_ORDER_VOLUME", 0.01))
        sl, tp = _v19_build_sl_tp(mt5_module, mapped, side, price)
        order = {"symbol": mapped, "volume": volume, "type": side, "price": price, "sl": sl, "tp": tp}
        logger.info("Attempting execution for %s: side=%s vol=%s price=%.6f signal=%.4f threshold=%.3f", sym, side, volume, price, signal, threshold)
        return order_wrapper(mt5_module, order)
    except Exception:
        logger.exception("execute_signal failed for %s", sym)
        return None
    ...
    while not stop_event.is_set():
        try:
            for sym in CONFIG.get("WATCH_SYMBOLS", []):
                ...
                try:
                    if v15_module and hasattr(v15_module, "compute_signal"):
                        signal = v15_module.compute_signal(sym, price, {"bars": recent_for_ctx, "timeframe": "M30"})
                    elif v15_module and hasattr(v15_module, "signal_to_side"):
                        signal = v15_module.signal_to_side(sym, price)
                except Exception:
                    signal = None
                ...
                if str(sym).upper().startswith("XAU"):
                    ...
                    if not macro_ok:
                        regime = "dxy_blocked"
                        try:
                            logger.info("XAU entry suppressed by macro confirmation gate | sym=%s | signal=%.4f", sym, float(signal) if signal is not None else None)
                        except Exception:
                            logger.info("XAU entry suppressed by macro confirmation gate | sym=%s", sym)
                        continue
                ...
                if regime == "trending":
                    logger.info(f"EXECUTION ENGINE TRIGGERED | {sym} | signal={signal:.4f} | price={price}")
