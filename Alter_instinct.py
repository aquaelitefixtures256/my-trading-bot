#!/usr/bin/env python3
"""
Ultra_instinct (full) - fundamentals replaced with Finnhub + NewsData + Alpha Vantage crypto endpoint.
All other bot logic kept unchanged from your prior file:
- Per-symbol max-open-trades (XAG/XAU = 5, others = 10)
- MT5-first position checks with DB fallback
- Debug snapshot printed only on first cycle
- Robust live confirmation (env or typed)
- total_score normalized to [-1,1]
- Proportional + Clamp threshold adaptation preserved
- Robust order-confirmation/recording/Telegram handling unchanged
- reconcile_closed_deals included and called at start of each cycle
"""
...
# New fundamentals providers (use env vars)
FINNHUB_KEY = os.getenv("FINNHUB_KEY", "")
NEWSDATA_KEY = os.getenv("NEWSDATA_KEY", "")
# Alpha Vantage crypto key: user provided default, can be overridden by env var
...
# ---------------- News & Fundamentals module (Finnhub + NewsData + Alpha Vantage) ----------------
# Keyword lists and simple sentiment words (kept for fallback)
...
# Caching small helper to avoid hitting free-tier rate limits
...
# -------- Finnhub calendar + NewsData headlines + AlphaVantage crypto price --------
def fetch_finnhub_calendar(lookback_hours: int = 1, lookahead_hours: int = 48):
    """Return list of events (as dicts) from Finnhub calendar endpoint. Requires FINNHUB_KEY."""
    if not FUNDAMENTAL_AVAILABLE or not FINNHUB_KEY:
        return []
    try:
        now = datetime.utcnow()
        ...
        if r.status_code != 200:
            logger.debug("fetch_finnhub_calendar non-200 %s", r.status_code)
            return []
        j = r.json()
        # Finnhub returns {"economicCalendar": [...]} or a list depending on plan - normalize
        if isinstance(j, dict) and "economicCalendar" in j:
            events = j.get("economicCalendar") or []
        ...
        for e in events:
            try:
                normalized.append({
                    ...
                    "importance": e.get("importance") or e.get("impact") or e.get("importanceLevel") or e.get("actual") or e.get("prior"),
                })
            except Exception:
                continue
        return normalized
    except Exception:
        logger.exception("fetch_finnhub_calendar failed")
        return []

def fetch_newsdata(q: str, pagesize: int = 20) -> Dict[str, Any]:
    """Fetch headlines from NewsData.io (NewsData). Returns dict like {'count':..., 'articles':[...] }"""
    out = {"count": 0, "articles": []}
    if not FUNDAMENTAL_AVAILABLE or not NEWSDATA_KEY:
        return out
    ...
    try:
        ...
        params = {"q": q, "language": "en", "page": 1, "page_size": pagesize, "apikey": NEWSDATA_KEY}
        r = requests.get(url, params=params, timeout=8)
        if r.status_code != 200:
            logger.debug("fetch_newsdata non-200 %s", r.status_code)
            return out
        j = r.json()
        ...
        _news_cache["data"][cache_key] = out
        _news_cache["ts"] = now_ts
        return out
    except Exception:
        logger.exception("fetch_newsdata failed")
        return out

def fetch_alpha_vantage_crypto_intraday(symbol: str = "BTC", market: str = "USD"):
    """Use Alpha Vantage DIGITAL_CURRENCY_INTRADAY (note: limited free calls). Returns dict or {}."""
    if not FUNDAMENTAL_AVAILABLE or not ALPHAVANTAGE_KEY:
        return {}
    # Symbol like BTC, market USD
    try:
        url = "https://www.alphavantage.co/query"
        params = {"function": "DIGITAL_CURRENCY_INTRADAY", "symbol": symbol, "market": market, "apikey": ALPHAVANTAGE_KEY}
        r = requests.get(url, params=params, timeout=10)
        if r.status_code != 200:
            logger.debug("fetch_alpha_vantage_crypto_intraday non-200 %s", r.status_code)
            return {}
        j = r.json()
        return j
    except Exception:
        logger.exception("fetch_alpha_vantage_crypto_intraday failed")
        return {}

def coindata_price_shock_crypto(symbol: str = "BTC"):
    """
    Lightweight crypto shock detector using Alpha Vantage intraday or fallback to NewsData price mentions.
    Returns a float in [-1,1] indicating short-term shock (positive -> upward shock).
    """
    now_ts = time.time()
    ...
    shock = 0.0
    try:
        # Try Alpha Vantage intraday series
        av = fetch_alpha_vantage_crypto_intraday(symbol=symbol, market="USD")
        # Response contains "Time Series (Digital Currency Intraday)" or similar
        series_key = None
        for k in av.keys():
        ...
        if series_key and isinstance(av.get(series_key), dict):
            ...
            if len(times) >= 2:
                try:
                    ...
                    # map to [-1,1] by dividing by a 5% short window threshold
...
# ---------------- Economic calendar & blocking (using Finnhub) ----------------
...
def should_pause_for_events(symbol: str, lookahead_minutes: int = 30) -> (bool, Optional[Dict[str, Any]]):
    """
    Uses Finnhub calendar fetch; numeric impact mapping supported.
...
def fetch_fundamental_score(symbol: str, lookback_days: int = NEWS_LOOKBACK_DAYS) -> float:
    """
    New composition using:
     - NewsData headlines (primary news feed) --> news_sentiment
     - Finnhub calendar events  --> blocking / calendar signal
     - Alpha Vantage crypto intraday --> short-term crypto shock detector
    ...
    crypto_shock = 0.0
    try:
        # 1) fetch news headlines for symbol via NewsData
        ...
        if articles:
            ...
            for a in articles:
                ...
                s = _vader_score(txt)
                scores.append(s)
                # count risk keyword hits
                kh = sum(1 for k in _RISK_KEYWORDS if k in txt.lower())
                hits += kh
            avg = sum(scores) / max(1, len(scores))
            # bias news_sent by keyword hits
            if hits >= 2:
                # if many risk keywords, push sentiment negative slightly (risk aversion)
    ...
    try:
        # 2) calendar signal - will be treated as blocking if high-impact imminent
        pause, ev = should_pause_for_events(symbol, lookahead_minutes=PAUSE_BEFORE_EVENT_MINUTES)
        if pause:
            # large negative/positive bias not applied; instead we return strong blocking by raising calendar_signal
            # calendar_signal could be +/-1 but we will use it for blocking externally
            calendar_signal = -1.0
        else:
            calendar_signal = 0.0
    except Exception:
    ...
    try:
        # 3) if crypto, evaluate short-term shock via Alpha Vantage
    ...
    # Combine: news is main contributor; calendar can block (we expose blocking separately),
    # crypto shock amplifies magnitude slightly for crypto symbols
    combined = 0.6 * news_sent + 0.3 * 0.0 + 0.1 * crypto_shock  # calendar handled separately as pause
...
def make_decision_for_symbol(symbol: str, live: bool=False):
    ...
    try:
        ...
        try:
            # new fundamentals integration (Finnhub + NewsData + AlphaVantage)
        ...
        try:
            # use NewsData to detect keyword hits quickly
