...
CONFIG = {
    ...
    "WATCH_SYMBOLS": ["BTCUSD", "GBPUSD", "USDJPY", "XAUUSD", "USOIL", "DXY", "US10Y"],
...
CONFIG["BACKTEST_PARAMS"].update({
    ...
    "GBPUSD": {"signal_thresh": 0.88, "atr_pct_thresh": 0.0008, "max_hold": 60},
    "GBPUSDm": {"signal_thresh": 0.88, "atr_pct_thresh": 0.0008, "max_hold": 60},
    ...
    while not stop_event.is_set():
        try:
            for sym in CONFIG.get("WATCH_SYMBOLS", []):
                ...
                # Detect regime
                params = CONFIG.get("BACKTEST_PARAMS", {}).get(sym, {})
                live_signal_thresh = params.get("signal_thresh", 0.92)
...
def run_backtest(v15_module, symbol='XAUUSD', days=30):
    ...
    for i, bar in enumerate(bars):
        ...
        try:
            ...
            params = {}
        signal_thresh = params.get("signal_thresh", 0.92) # default old behavior
...
def start_bot():
    ...
    try:
        ...
        symbols_text = "BTCUSD GBPUSD USDJPY XAUUSD USOIL"
    ...
    while not (stop_event and getattr(stop_event, "is_set", lambda: False)()):
        try:
            for sym in watch:
                ...
                params = CONFIG.get("BACKTEST_PARAMS", {}).get(sym, {})
                live_signal_thresh = float(params.get("signal_thresh", 0.92))
...
if "SYMBOL_TRADE_LIMITS" not in globals():
    SYMBOL_TRADE_LIMITS = {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
...
try:
    SYMBOL_TRADE_LIMITS = dict(globals().get("SYMBOL_TRADE_LIMITS", {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}))
except Exception:
    SYMBOL_TRADE_LIMITS = {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
...
def _deprecated_execute_signal(sym, signal, price, mt5_module, symbol_map):
    ...
    try:
        ...
        symbol_thresh = float(params.get("signal_thresh", CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.92)))
        exec_thresh = float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.92))
...
try:
    SYMBOL_TRADE_LIMITS = dict(globals().get(
        "SYMBOL_TRADE_LIMITS",
        {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
    ))
except Exception:
    SYMBOL_TRADE_LIMITS = {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
...
try:
    GLOBAL_MAX_OPEN_TRADES = 8
    MAX_OPEN_TRADES = 8
    SYMBOL_TRADE_LIMITS = {"BTCUSD": 3, "USOIL": 3, "XAUUSD": 2, "GBPUSD": 10, "USDJPY": 10}
except Exception:
    pass
try:
    CONFIG.setdefault("BACKTEST_PARAMS", {})
    for _sym, _params in {
        ...
        "GBPUSD": {"sl_atr_mult": 4.0, "tp_atr_mult": 6.0, "risk_pct": 0.005},
        "GBPUSDm": {"sl_atr_mult": 4.0, "tp_atr_mult": 6.0, "risk_pct": 0.005},
...
try:
    CONFIG.setdefault("BACKTEST_PARAMS", {})
    for _sym in ("BTCUSD", "BTCUSDm", "USOIL", "USOILm", "XAUUSD", "XAUUSDm", "GBPUSD", "GBPUSDm", "USDJPY", "USDJPYm"):
        CONFIG["BACKTEST_PARAMS"].setdefault(_sym, {})
...
def _kyoto_limit_reserve(symbol):
    ...
    try:
        ...
        with lock:
            ...
            limits = dict(globals().get("SYMBOL_TRADE_LIMITS", {"USOIL": 3, "BTCUSD": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}))
...
def _kyoto_final_reserve(symbol):
    ...
    try:
        with _KYOTO_FINAL_LIMIT_LOCK:
            ...
            gmax = int(globals().get("GLOBAL_MAX_OPEN_TRADES", 8))
            limits = dict(globals().get("SYMBOL_TRADE_LIMITS", {"BTCUSD": 3, "USOIL": 3, "GBPUSD": 10, "USDJPY": 10, "XAUUSD": 2}))
            limit = int(limits.get(sym, int(os.getenv("BEAST_MAX_PER_SYMBOL_DEFAULT", "10"))))
            if total >= gmax:
                return False, f"global_max_open_reached:{total}", None
            if per >= limit:
...
try:
    SYMBOL_TRADE_LIMITS.update({"BTCUSD": 3, "USOIL": 3, "GBPUSD": 10, "USDJPY": 10, "XAUUSD": 2})
    GLOBAL_MAX_OPEN_TRADES = 8
except Exception:
    pass
...
try:
    ...
    def _kyoto_rt_map_symbol_to_broker(requested: str) -> str:
        ...
        key = key[:-1] if key.endswith("m") and key[:-2].endswith(("BTCUSD", "XAUUSD", "USDJPY", "GBPUSD", "USOIL")) else key
        if key in direct:
            return direct[key]
    ...
    def _kyoto_unify_symbol_map(symbol_map):
        try:
            if isinstance(symbol_map, dict):
                for canon in ("BTCUSD", "XAUUSD", "USDJPY", "GBPUSD", "USOIL"):
...
def execute_signal(sym, signal, price, mt5_module, symbol_map):
    try:
        ...
        try:
            ...
            params = {}
        threshold = float(params.get("signal_thresh", 0.88))
        if abs(signal) < threshold:
            logger.info(
                "Execution skipped for %s: signal below execution threshold (%.3f) signal=%.4f",
...
# --- FINAL TRADE LIMIT ENFORCEMENT SAFETY NET ---
# This block is intentionally appended at the very end so it overrides any older
# duplicate entry points that may have been left in the file.
try:
    _KYOTO_LIMIT_ENFORCER_PREV_PLACE_ORDER_MT5 = globals().get("place_order_mt5")
    _KYOTO_LIMIT_ENFORCER_PREV_ORDER_WRAPPER = globals().get("order_wrapper")
    _KYOTO_LIMIT_ENFORCER_PREV_EXECUTE_SIGNAL = globals().get("execute_signal")
    _KYOTO_LIMIT_ENFORCER_PREV_MAKE_DECISION = globals().get("make_decision_for_symbol")
except Exception:
    _KYOTO_LIMIT_ENFORCER_PREV_PLACE_ORDER_MT5 = None
    _KYOTO_LIMIT_ENFORCER_PREV_ORDER_WRAPPER = None
    _KYOTO_LIMIT_ENFORCER_PREV_EXECUTE_SIGNAL = None
    _KYOTO_LIMIT_ENFORCER_PREV_MAKE_DECISION = None

def _kyoto_limit_enforcer_symbol(value):
    try:
        normalizer = globals().get("_kyoto_final_normalize_symbol")
...
def _kyoto_limit_enforcer_guard(symbol):
    try:
        sym = _kyoto_limit_enforcer_symbol(symbol)
        if not sym:
        ...
        try:
            logger.exception("trade limit guard failed for %s", symbol)
        except Exception:
            pass
        return False, "error"
...
def place_order_mt5(symbol, action, lot, price, sl, tp):
    sym = _kyoto_limit_enforcer_symbol(symbol)
    ok, reason = _kyoto_limit_enforcer_guard(sym)
    if not ok:
        return {"status": "skipped", "comment": reason, "symbol": sym}
    prev = _KYOTO_LIMIT_ENFORCER_PREV_PLACE_ORDER_MT5
    if callable(prev):
        return prev(symbol, action, lot, price, sl, tp)
    return {"status": "skipped", "comment": "place_order_unavailable", "symbol": sym}

def order_wrapper(mt5_module, order_request):
    req = dict(order_request) if isinstance(order_request, dict) else dict(order_request or {})
    sym = _kyoto_limit_enforcer_symbol(req.get("symbol") or req.get("instrument") or req.get("symbol_name") or "")
    ok, reason = _kyoto_limit_enforcer_guard(sym)
    if not ok:
        return {"retcode": -1, "comment": reason, "request": req}
    prev = _KYOTO_LIMIT_ENFORCER_PREV_ORDER_WRAPPER
    if callable(prev):
        return prev(mt5_module, order_request)
    return {"retcode": -1, "comment": "order_wrapper_unavailable", "request": req}

def execute_signal(sym, signal, price, mt5_module, symbol_map):
    sym_u = _kyoto_limit_enforcer_symbol(sym)
    ok, reason = _kyoto_limit_enforcer_guard(sym_u)
    if not ok:
        try:
            logger.info("Execution skipped for %s: %s", sym_u, reason)
        except Exception:
            pass
        return None
    prev = _KYOTO_LIMIT_ENFORCER_PREV_EXECUTE_SIGNAL
    if callable(prev):
        return prev(sym, signal, price, mt5_module, symbol_map)
    return None
...
def make_decision_for_symbol(symbol: str, live: bool=False):
    sym = _kyoto_limit_enforcer_symbol(symbol)
    prev = _KYOTO_LIMIT_ENFORCER_PREV_MAKE_DECISION
    result = prev(symbol, live) if callable(prev) else None
    ok, reason = _kyoto_limit_enforcer_guard(sym)
    if isinstance(result, dict):
        if not ok:
            for key in ("final", "signal", "score", "side", "action"):
                if key in result:
                    result[key] = 0.0 if key in ("final", "signal", "score") else None
            result["trade_blocked"] = True
            result["trade_block_reason"] = reason
        else:
            result.setdefault("trade_blocked", False)
            result.setdefault("trade_block_reason", "")
    return result

# Rebind the final hardened wrappers so every duplicate path lands on the same trade limit gate.
globals()["place_order_mt5"] = place_order_mt5
globals()["order_wrapper"] = order_wrapper
globals()["execute_signal"] = execute_signal
globals()["make_decision_for_symbol"] = make_decision_for_symbol
try:
    logger.info("FINAL TRADE LIMIT ENFORCEMENT SAFETY NET installed")
except Exception:
    pass
# --- END FINAL TRADE LIMIT ENFORCEMENT SAFETY NET ---


# === FINAL ABSOLUTE TRADE LIMIT LOCKDOWN ===
# This block is intentionally last so it becomes the single source of truth for
# live trade limits and the execution send layer.
try:
    import threading as _k_threading
    import time as _k_time
    import uuid as _k_uuid
except Exception:
    _k_threading = None
    _k_time = None
    _k_uuid = None
...
try:
    SYMBOL_TRADE_LIMITS = {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
    GLOBAL_MAX_OPEN_TRADES = 8
    globals()["SYMBOL_TRADE_LIMITS"] = dict(SYMBOL_TRADE_LIMITS)
    globals()["GLOBAL_MAX_OPEN_TRADES"] = int(GLOBAL_MAX_OPEN_TRADES)
except Exception:
...
try:
    _KYOTO_ABSOLUTE_LIMIT_LOCK
except NameError:
    _KYOTO_ABSOLUTE_LIMIT_LOCK = _k_threading.RLock() if _k_threading is not None else None

try:
    _KYOTO_ABSOLUTE_LIMIT_PENDING
except NameError:
    _KYOTO_ABSOLUTE_LIMIT_PENDING = {}

try:
    _KYOTO_ABSOLUTE_LIMIT_TTL_SECONDS = int(globals().get("_KYOTO_ABSOLUTE_LIMIT_TTL_SECONDS", 90))
except Exception:
    _KYOTO_ABSOLUTE_LIMIT_TTL_SECONDS = 90


def _kyoto_absolute_symbol(symbol):
    try:
    ...
    for suffix in (".M", "-M"):
        if s.endswith(suffix):
            s = s[: -len(suffix)]
    if s.endswith("M") and len(s) > 1 and s[:-1] in {"BTCUSD", "USOIL", "USDJPY", "GBPUSD", "XAUUSD"}:
        s = s[:-1]
    return s


def _kyoto_absolute_cleanup(now=None):
    try:
        if now is None:
            now = _k_time.time() if _k_time is not None else 0.0
        ttl = int(globals().get("_KYOTO_ABSOLUTE_LIMIT_TTL_SECONDS", 90))
        pending = globals().get("_KYOTO_ABSOLUTE_LIMIT_PENDING", {})
        if not isinstance(pending, dict):
            globals()["_KYOTO_ABSOLUTE_LIMIT_PENDING"] = {}
            pending = globals()["_KYOTO_ABSOLUTE_LIMIT_PENDING"]
        for tok, meta in list(pending.items()):
            try:
                if now - float(meta.get("ts", now)) > ttl:
                    pending.pop(tok, None)
...
def _kyoto_absolute_count_open(symbol=None, ignore_token=None):
    sym = _kyoto_absolute_symbol(symbol) if symbol is not None else None
    total = 0
    per = 0
    ...
    try:
        ...
        if globals().get("MT5_AVAILABLE") and globals().get("_mt5_connected") and mt5_mod is not None:
            ...
            if sym is not None:
                for p in positions:
                    psym = _kyoto_absolute_symbol(getattr(p, "symbol", "") or "")
                    if psym == sym:
                        per += 1
    except Exception:
        pass
    ...
    try:
        ...
        if callable(fn):
            result = fn()
            if isinstance(result, tuple) and len(result) >= 2:
                t2 = int(result[0] or 0)
                pm = result[1] if isinstance(result[1], dict) else {}
                total = max(total, t2)
                if sym is not None and isinstance(pm, dict):
                    per = max(per, int(pm.get(sym, pm.get(sym.replace("M", ""), 0)) or 0))
            elif isinstance(result, dict):
                pm = {str(k).upper(): int(v or 0) for k, v in result.items()}
                total = max(total, sum(pm.values()))
                if sym is not None:
                    per = max(per, int(pm.get(sym, pm.get(sym.replace("M", ""), 0)) or 0))
    except Exception:
        pass
        ...
        if isinstance(pending, dict):
            for tok, meta in pending.items():
                if ignore_token is not None and tok == ignore_token:
                    continue
                total += 1
                if sym is not None and _kyoto_absolute_symbol(meta.get("symbol")) == sym:
                    per += 1
    except Exception:
        pass
...
def _kyoto_absolute_guard(symbol, reserve=False):
    try:
        sym = _kyoto_absolute_symbol(symbol)
        if not sym:
            return False, "NO_SYMBOL", None
        if sym.startswith(("DXY", "US10Y")):
            return False, "macro_filter_symbol_only", None
        lock = globals().get("_KYOTO_ABSOLUTE_LIMIT_LOCK")
        if lock is None:
            return False, "limit_lock_missing", None
        with lock:
            _kyoto_absolute_cleanup()
            total, per = _kyoto_absolute_count_open(sym, None)
            gmax = int(globals().get("GLOBAL_MAX_OPEN_TRADES", 8))
            limits = dict(globals().get("SYMBOL_TRADE_LIMITS", {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}))
            limit = int(limits.get(sym, int(globals().get("BEAST_MAX_PER_SYMBOL_DEFAULT", 10))))
            if total >= gmax:
                return False, f"global_max_open_reached:{total}", None
            if per >= limit:
                return False, f"symbol_limit_reached:{sym}:{per}/{limit}", None
            token = None
            if reserve:
                token = _k_uuid.uuid4().hex if _k_uuid is not None else str(sym) + str(total) + str(per)
                globals()["_KYOTO_ABSOLUTE_LIMIT_PENDING"][token] = {"symbol": sym, "ts": _k_time.time() if _k_time is not None else 0.0}
            return True, "ok", token
    except Exception:
        try:
            logger.exception("absolute guard failed for %s", symbol)
        except Exception:
            pass
        return False, "error", None


def _kyoto_absolute_release(token):
    try:
        if not token:
            return
        lock = globals().get("_KYOTO_ABSOLUTE_LIMIT_LOCK")
...
try:
    _KYOTO_ABSOLUTE_PREV_ALLOWED_TO_OPEN = globals().get("allowed_to_open")
    def allowed_to_open(symbol):
        ok, reason, _token = _kyoto_absolute_guard(symbol, reserve=False)
        return bool(ok), str(reason)
    globals()["allowed_to_open"] = allowed_to_open
except Exception:
    try:
        logger.exception("absolute override failed for allowed_to_open")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_FINAL_SEND = globals().get("_kyoto_final_send_request")
    if callable(_KYOTO_ABSOLUTE_PREV_FINAL_SEND):
        def _kyoto_final_send_request(mt5_module, req):
            req = dict(req) if isinstance(req, dict) else dict(req or {})
            sym = _kyoto_absolute_symbol(req.get("symbol") or req.get("instrument") or req.get("symbol_name") or "")
            ok, reason, token = _kyoto_absolute_guard(sym, reserve=True)
            if not ok:
                return {"retcode": -1, "comment": reason, "request": req}
            try:
                return _KYOTO_ABSOLUTE_PREV_FINAL_SEND(mt5_module, req)
            finally:
                _kyoto_absolute_release(token)
        globals()["_kyoto_final_send_request"] = _kyoto_final_send_request
except Exception:
    try:
        logger.exception("absolute override failed for _kyoto_final_send_request")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_RT_SEND = globals().get("_kyoto_rt_send_request")
    if callable(_KYOTO_ABSOLUTE_PREV_RT_SEND):
        def _kyoto_rt_send_request(mt5_module, req):
            req = dict(req) if isinstance(req, dict) else dict(req or {})
            sym = _kyoto_absolute_symbol(req.get("symbol") or req.get("instrument") or req.get("symbol_name") or "")
            ok, reason, token = _kyoto_absolute_guard(sym, reserve=True)
            if not ok:
                return {"retcode": -1, "comment": reason, "request": req}
            try:
                return _KYOTO_ABSOLUTE_PREV_RT_SEND(mt5_module, req)
            finally:
                _kyoto_absolute_release(token)
        globals()["_kyoto_rt_send_request"] = _kyoto_rt_send_request
except Exception:
    try:
        logger.exception("absolute override failed for _kyoto_rt_send_request")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_PLACE = globals().get("place_order_mt5")
    if callable(_KYOTO_ABSOLUTE_PREV_PLACE):
        def place_order_mt5(symbol, action, lot, price, sl, tp):
            sym = _kyoto_absolute_symbol(symbol)
            ok, reason, _token = _kyoto_absolute_guard(sym, reserve=False)
            if not ok:
                return {"status": "skipped", "comment": reason, "symbol": sym}
            return _KYOTO_ABSOLUTE_PREV_PLACE(symbol, action, lot, price, sl, tp)
        globals()["place_order_mt5"] = place_order_mt5
except Exception:
    try:
        logger.exception("absolute override failed for place_order_mt5")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_ORDER_WRAPPER = globals().get("order_wrapper")
    if callable(_KYOTO_ABSOLUTE_PREV_ORDER_WRAPPER):
        def order_wrapper(mt5_module, order_request):
            req = dict(order_request) if isinstance(order_request, dict) else dict(order_request or {})
            sym = _kyoto_absolute_symbol(req.get("symbol") or req.get("instrument") or req.get("symbol_name") or "")
            ok, reason, _token = _kyoto_absolute_guard(sym, reserve=False)
            if not ok:
                return {"retcode": -1, "comment": reason, "request": req}
            return _KYOTO_ABSOLUTE_PREV_ORDER_WRAPPER(mt5_module, order_request)
        globals()["order_wrapper"] = order_wrapper
except Exception:
    try:
        logger.exception("absolute override failed for order_wrapper")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_EXECUTE_SIGNAL = globals().get("execute_signal")
    if callable(_KYOTO_ABSOLUTE_PREV_EXECUTE_SIGNAL):
        def execute_signal(sym, signal, price, mt5_module, symbol_map):
            sym_u = _kyoto_absolute_symbol(sym)
            ok, reason, _token = _kyoto_absolute_guard(sym_u, reserve=False)
            if not ok:
                try:
                    logger.info("Execution skipped for %s: %s", sym_u, reason)
                except Exception:
                    pass
                return None
            return _KYOTO_ABSOLUTE_PREV_EXECUTE_SIGNAL(sym, signal, price, mt5_module, symbol_map)
        globals()["execute_signal"] = execute_signal
except Exception:
    try:
        logger.exception("absolute override failed for execute_signal")
    except Exception:
        pass

try:
    _KYOTO_ABSOLUTE_PREV_MAKE_DECISION = globals().get("make_decision_for_symbol")
    if callable(_KYOTO_ABSOLUTE_PREV_MAKE_DECISION):
        def make_decision_for_symbol(symbol: str, live: bool=False):
            sym = _kyoto_absolute_symbol(symbol)
            result = _KYOTO_ABSOLUTE_PREV_MAKE_DECISION(symbol, live)
            ok, reason, _token = _kyoto_absolute_guard(sym, reserve=False)
            if isinstance(result, dict):
                if not ok:
                    for key in ("final", "signal", "score", "side", "action"):
    ...
    try:
        logger.exception("absolute override failed for make_decision_for_symbol")
    except Exception:
        pass

try:
    globals()["SYMBOL_TRADE_LIMITS"] = {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "GBPUSD": 10, "XAUUSD": 2}
    globals()["GLOBAL_MAX_OPEN_TRADES"] = 8
except Exception:
    pass

try:
    logger.info("FINAL ABSOLUTE TRADE LIMIT LOCKDOWN installed: global=8, BTCUSD=3, USOIL=3, USDJPY=10, GBPUSD=10, XAUUSD=2")
except Exception:
    pass
# === END FINAL ABSOLUTE TRADE LIMIT LOCKDOWN ===


# ===========================================================================================
# === V18_GOLD_FIX: TRUE PRE-ORDER TRADE LIMIT ENFORCEMENT (appended last, highest priority)
# ===========================================================================================
# ROOT CAUSE FIX:
# All previous wrappers call the underlying function FIRST, then check limits.
# That means orders are already sent to MT5 before the check runs.
# This patch wraps place_order_mt5 with a HARD PRE-CHECK that returns immediately
# if limits are breached — without calling the real MT5 order send at all.
# It also wraps make_decision_for_symbol to abort BEFORE any order logic executes.
# ===========================================================================================

import threading as _fix_threading
import time as _fix_time
import logging as _fix_logging

_fix_logger = _fix_logging.getLogger("v18_gold_fix")

# ---- Authoritative limit values (user-specified) ----
_FIX_SYMBOL_LIMITS = {
    "GBPUSD":  10,
    "BTCUSD":  5,
    "USOIL":   5,
...
# Push these into globals so all other checks also use correct values
globals()["SYMBOL_TRADE_LIMITS"] = dict(_FIX_SYMBOL_LIMITS)
globals()["GLOBAL_MAX_OPEN_TRADES"] = int(_FIX_GLOBAL_MAX)

_fix_logger.info(
    "V18_GOLD_FIX limits set: global=%d, per-symbol=%s",
    _FIX_GLOBAL_MAX, _FIX_SYMBOL_LIMITS
)

_fix_lock = _fix_threading.RLock()


def _fix_normalize_symbol(sym):
    """Strip broker suffixes (m, M, .m, -m) and uppercase."""
    try:
        s = str(sym or "").upper().strip()
        for suffix in (".M", "-M"):
            if s.endswith(suffix):
                s = s[: -len(suffix)]
        if s.endswith("M") and len(s) > 1 and s[:-1] in _FIX_SYMBOL_LIMITS:
            s = s[:-1]
        return s
    except Exception:
...
def _fix_count_open_positions(symbol_normalized):
    """
    Count open positions for a specific symbol (and globally).
    Returns (global_total, symbol_count).
    Queries MT5 first; falls back to existing helpers.
    """
    global_total = 0
    symbol_count = 0

    # --- MT5 direct query (most reliable) ---
    try:
        mt5_mod = globals().get("_mt5") or globals().get("mt5")
        if globals().get("MT5_AVAILABLE") and globals().get("_mt5_connected") and mt5_mod is not None:
            all_positions = mt5_mod.positions_get() or []
            global_total = len(all_positions)
            for p in all_positions:
                p_sym = _fix_normalize_symbol(getattr(p, "symbol", "") or "")
                if p_sym == symbol_normalized:
                    symbol_count += 1
            return global_total, symbol_count
    except Exception:
        pass

    # --- Fallback: use existing get_open_positions_count ---
    try:
        fn = globals().get("get_open_positions_count")
        if callable(fn):
            symbol_count = max(symbol_count, int(fn(symbol_normalized) or 0))
    except Exception:
        pass

    # --- Fallback: use existing count_open_positions ---
    try:
        fn2 = globals().get("count_open_positions")
        if callable(fn2):
            result = fn2()
            if isinstance(result, tuple) and len(result) >= 2:
                global_total = max(global_total, int(result[0] or 0))
                pm = result[1] if isinstance(result[1], dict) else {}
                symbol_count = max(symbol_count, int(pm.get(symbol_normalized, 0)))
    except Exception:
        pass
...
def _fix_pre_check(symbol):
    """
    TRUE PRE-ORDER CHECK.
    Returns (allowed: bool, reason: str).
    Must be called BEFORE any order is sent.
    """
    with _fix_lock:
        sym = _fix_normalize_symbol(symbol)
        if not sym:
            return False, "no_symbol"

        global_total, symbol_count = _fix_count_open_positions(sym)

        if global_total >= _FIX_GLOBAL_MAX:
            reason = f"GLOBAL_MAX_REACHED:{global_total}/{_FIX_GLOBAL_MAX}"
            _fix_logger.warning("TRADE BLOCKED %s - %s", sym, reason)
            return False, reason

        sym_limit = _FIX_SYMBOL_LIMITS.get(sym, 10)
        if symbol_count >= sym_limit:
            reason = f"SYMBOL_LIMIT_REACHED:{sym}:{symbol_count}/{sym_limit}"
            _fix_logger.warning("TRADE BLOCKED %s - %s", sym, reason)
            return False, reason

        _fix_logger.debug(
            "TRADE ALLOWED %s (global=%d/%d symbol=%d/%d)",
            sym, global_total, _FIX_GLOBAL_MAX, symbol_count, sym_limit
        )
        return True, "ok"


# ---- Wrap place_order_mt5 with hard pre-check ----
_fix_prev_place_order_mt5 = globals().get("place_order_mt5")

def place_order_mt5(symbol, action, lot, price, sl, tp):
    """Hard pre-check wrapper around place_order_mt5."""
    allowed, reason = _fix_pre_check(symbol)
    if not allowed:
        _fix_logger.warning(
            "place_order_mt5 BLOCKED for %s action=%s: %s", symbol, action, reason
        )
        return {"status": "blocked", "comment": reason, "symbol": symbol}
    if callable(_fix_prev_place_order_mt5):
        return _fix_prev_place_order_mt5(symbol, action, lot, price, sl, tp)
    return {"status": "no_executor", "symbol": symbol}

globals()["place_order_mt5"] = place_order_mt5


# ---- Wrap make_decision_for_symbol with hard pre-check ----
_fix_prev_make_decision = globals().get("make_decision_for_symbol")

def make_decision_for_symbol(symbol: str, live: bool = False):
    """
    Hard pre-check wrapper: if limits are already at max,
    return immediately WITHOUT calling the underlying function,
    so no order send code runs at all.
    """
    allowed, reason = _fix_pre_check(symbol)
    if not allowed:
        return {
    ...
    if callable(_fix_prev_make_decision):
        return _fix_prev_make_decision(symbol, live)
    return {"symbol": symbol, "final": None, "trade_blocked": False}

globals()["make_decision_for_symbol"] = make_decision_for_symbol


_fix_logger.info(
    "V18_GOLD_FIX APPLIED: place_order_mt5 and make_decision_for_symbol "
    "now enforce limits PRE-ORDER. Global=%d, Limits=%s",
    _FIX_GLOBAL_MAX, _FIX_SYMBOL_LIMITS
)
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  V18 FINAL DEFINITIVE TRADE LIMIT FIX — THE LAST OF THE CENTURY            ║
# ║  Strategy: Replace MetaTrader5 in sys.modules with a limit-enforcing        ║
# ║  proxy, then use gc.get_referrers() to swap EVERY existing reference to    ║
# ║  the real mt5 object across ALL namespaces (globals, exec'd g-dict,         ║
# ║  instance attrs, etc.) with the proxy. This is unconditional and            ║
# ║  namespace-agnostic — there is no code path that bypasses it.               ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

import sys as _v18f_sys
import gc as _v18f_gc
import threading as _v18f_threading
import logging as _v18f_logging
import time as _v18f_time

_V18F_LOG = _v18f_logging.getLogger("v18_final_fix")

# ═══════════════════════════════════════════════════════════════
# USER-SPECIFIED TRADE LIMITS (exact values requested)
# ═══════════════════════════════════════════════════════════════
_V18F_SYMBOL_LIMITS = {
    "GBPUSD":  10,
    "BTCUSD":  5,
...
# Broadcast correct limits to all globals-level variables used by older layers
globals()["SYMBOL_TRADE_LIMITS"]   = dict(_V18F_SYMBOL_LIMITS)
globals()["GLOBAL_MAX_OPEN_TRADES"] = int(_V18F_GLOBAL_MAX)
globals()["MAX_OPEN_TRADES"]        = int(_V18F_GLOBAL_MAX)

# Lock to make position-count + order-send atomic
_V18F_ORDER_LOCK = _v18f_threading.RLock()


def _v18f_normalize(sym):
    """Strip broker suffixes (m, M, .m, -m) and uppercase."""
    try:
        s = str(sym or "").upper().strip()
        for sfx in (".M", "-M"):
            if s.endswith(sfx):
                s = s[:-len(sfx)]
                break
        # single trailing M when base is a known symbol
        if s.endswith("M") and len(s) > 1 and s[:-1] in _V18F_SYMBOL_LIMITS:
            s = s[:-1]
        return s
    except Exception:
        return str(sym or "").upper().strip()
...
def _v18f_count(real_mt5, sym_normalized):
    """
    Query open positions directly from the real MT5 object.
    Returns (global_total, symbol_count).
    Uses the real (unwrapped) mt5 object to avoid proxy recursion.
    """
    try:
        positions = real_mt5.positions_get()
        if positions is None:
            return 0, 0
        total = len(positions)
        per = sum(
            1 for p in positions
            if _v18f_normalize(getattr(p, "symbol", "") or "") == sym_normalized
        )
        return total, per
    except Exception as e:
        _V18F_LOG.debug("_v18f_count failed: %s", e)
        return 0, 0


class _V18FResult:
    """Mimics an MT5 OrderSendResult object for a blocked order."""
    ...
    def __repr__(self):
        return f"<BLOCKED retcode={self.retcode} comment='{self.comment}'>"


class _V18FProxy:
    """
    Transparent proxy for the MetaTrader5 C-extension module.
    All attribute access passes through to the real module EXCEPT
    order_send(), which first checks live position counts and blocks
    ...
    # ── the single intercepted method ─────────────────────────────
    def order_send(self, request):
        real = object.__getattribute__(self, "_real")

        # extract symbol from request (dict or named-tuple style)
        try:
            if isinstance(request, dict):
                sym_raw = str(request.get("symbol", "") or "")
            else:
                sym_raw = str(getattr(request, "symbol", "") or "")
            sym = _v18f_normalize(sym_raw)
        except Exception:
            sym = ""

        with _V18F_ORDER_LOCK:
            # ── live MT5 position count (authoritative) ────────────
            global_total, sym_count = _v18f_count(real, sym)

            # ── global cap ─────────────────────────────────────────
            if global_total >= _V18F_GLOBAL_MAX:
                msg = f"BLOCKED:GLOBAL_MAX:{global_total}/{_V18F_GLOBAL_MAX}"
                _V18F_LOG.warning("ORDER %s  symbol=%s", msg, sym)
                return _V18FResult(msg)

            # ── per-symbol cap ─────────────────────────────────────
            sym_limit = _V18F_SYMBOL_LIMITS.get(sym, 999)
            if sym_count >= sym_limit:
                msg = f"BLOCKED:SYMBOL_MAX:{sym}:{sym_count}/{sym_limit}"
                _V18F_LOG.warning("ORDER %s", msg)
                return _V18FResult(msg)

        _V18F_LOG.info(
            "ORDER ALLOWED sym=%s (global=%d/%d sym=%d/%d)",
            sym, global_total, _V18F_GLOBAL_MAX, sym_count, sym_limit
        )
        return real.order_send(request)
...
def _v18f_install():
    """
    Replace MetaTrader5 everywhere:
    1. sys.modules  — covers all future imports
    2. gc.get_referrers(real_mt5) — covers every dict that already
       holds a reference: module globals, exec'd g-namespace, instance
       __dict__, class __dict__, etc.
    """
    # find the real MT5 module
    real = _v18f_sys.modules.get("MetaTrader5")
    if real is None:
        try:
            import MetaTrader5 as _tmp_mt5
            real = _tmp_mt5
        except ImportError:
            _V18F_LOG.warning("MetaTrader5 not installed — proxy not needed")
            return None

    # avoid double-wrapping
    if isinstance(real, _V18FProxy):
        _V18F_LOG.info("V18F proxy already installed")
        return real

    proxy = _V18FProxy(real)

    # 1 ── sys.modules (covers new imports / re-imports)
    _v18f_sys.modules["MetaTrader5"] = proxy
    # some code does: import MetaTrader5 as mt5  →  mt5.order_send
    # other code does: sys.modules["mt5"] if imported under that alias
    for alias in list(_v18f_sys.modules.keys()):
        if _v18f_sys.modules[alias] is real:
            _v18f_sys.modules[alias] = proxy

    # 2 ── every dict that holds the real module object
    replaced = 0
    try:
        for referrer in _v18f_gc.get_referrers(real):
            if not isinstance(referrer, dict):
                continue
            # skip sys.modules itself (already handled) and our own locals
            if referrer is _v18f_sys.modules:
                continue
            for k in list(referrer.keys()):
                try:
                    if referrer[k] is real:
                        referrer[k] = proxy
                        replaced += 1
                except Exception:
                    pass
    except Exception as e:
        _V18F_LOG.warning("gc replacement partial: %s", e)

    _V18F_LOG.info(
        "V18F MT5 PROXY INSTALLED — replaced %d references | "
        "global=%d GBPUSD=%d BTCUSD=%d USOIL=%d USDJPY=%d XAUUSD=%d",
        replaced, _V18F_GLOBAL_MAX,
        _V18F_SYMBOL_LIMITS["GBPUSD"],
        _V18F_SYMBOL_LIMITS["BTCUSD"],
        _V18F_SYMBOL_LIMITS["USOIL"],
        _V18F_SYMBOL_LIMITS["USDJPY"],
        _V18F_SYMBOL_LIMITS["XAUUSD"],
    )
    return proxy


# Install immediately when file loads
_V18F_PROXY = _v18f_install()


# ═══════════════════════════════════════════════════════════════
# BACKGROUND WATCHDOG — re-applies proxy every 30 s
# Catches any new references created after file load (e.g. when
# connect_mt5() runs and does  _mt5 = mt5  inside exec'd g dict)
# ═══════════════════════════════════════════════════════════════

def _v18f_watchdog():
    """
    Every 30 seconds re-scan all gc referrers of the real MT5 object
    and replace any that still hold the raw module with our proxy.
    This ensures that dynamically-created references (e.g., after
    MT5 connect) are also covered.
    """
    while True:
        try:
            _v18f_time.sleep(30)
            proxy = globals().get("_V18F_PROXY")
            if proxy is None or not isinstance(proxy, _V18FProxy):
                proxy = _v18f_install()
                if proxy:
                    globals()["_V18F_PROXY"] = proxy
                continue
            ...
            for referrer in _v18f_gc.get_referrers(real):
                ...
                for k in list(referrer.keys()):
                    try:
                        ...
                        pass
            if fixed:
                _V18F_LOG.info("V18F watchdog: fixed %d new raw-mt5 references", fixed)
        except Exception as e:
            _V18F_LOG.debug("V18F watchdog iteration error: %s", e)


_v18f_wd = _v18f_threading.Thread(
    target=_v18f_watchdog,
    name="v18f_mt5_proxy_watchdog",
    daemon=True
)
_v18f_wd.start()
...
# ═══════════════════════════════════════════════════════════════
# SECONDARY DEFENSE — patch place_order_mt5 in globals()
# Even if the proxy somehow misses a call, this wrapper catches it.
# ═══════════════════════════════════════════════════════════════

_v18f_prev_place = globals().get("place_order_mt5")

def place_order_mt5(symbol, action, lot, price, sl, tp):
    """Hard pre-check wrapper — secondary defense after the MT5 proxy."""
    sym = _v18f_normalize(symbol)
    proxy = globals().get("_V18F_PROXY")
    if proxy is not None and isinstance(proxy, _V18FProxy):
        real = object.__getattribute__(proxy, "_real")
        g_total, s_count = _v18f_count(real, sym)
        if g_total >= _V18F_GLOBAL_MAX:
            _V18F_LOG.warning("place_order_mt5 BLOCKED GLOBAL sym=%s total=%d", sym, g_total)
            return {"status": "blocked", "comment": f"GLOBAL_MAX:{g_total}/{_V18F_GLOBAL_MAX}"}
        s_lim = _V18F_SYMBOL_LIMITS.get(sym, 999)
        if s_count >= s_lim:
            _V18F_LOG.warning("place_order_mt5 BLOCKED SYMBOL sym=%s count=%d", sym, s_count)
            return {"status": "blocked", "comment": f"SYMBOL_MAX:{sym}:{s_count}/{s_lim}"}
    if callable(_v18f_prev_place):
        return _v18f_prev_place(symbol, action, lot, price, sl, tp)
...
# ═══════════════════════════════════════════════════════════════
# TERTIARY DEFENSE — patch make_decision_for_symbol in globals()
# Returns immediately (NO order logic runs) if limits already full.
# ═══════════════════════════════════════════════════════════════

_v18f_prev_decide = globals().get("make_decision_for_symbol")

def make_decision_for_symbol(symbol, live=False):
    """Early-exit wrapper — tertiary defense."""
    sym = _v18f_normalize(symbol)
    proxy = globals().get("_V18F_PROXY")
    if proxy is not None and isinstance(proxy, _V18FProxy):
        real = object.__getattribute__(proxy, "_real")
        g_total, s_count = _v18f_count(real, sym)
        if g_total >= _V18F_GLOBAL_MAX:
            _V18F_LOG.warning("make_decision BLOCKED GLOBAL sym=%s total=%d", sym, g_total)
            return {"symbol": symbol, "final": None, "trade_blocked": True,
                    "trade_block_reason": f"GLOBAL_MAX:{g_total}/{_V18F_GLOBAL_MAX}"}
        s_lim = _V18F_SYMBOL_LIMITS.get(sym, 999)
        if s_count >= s_lim:
            _V18F_LOG.warning("make_decision BLOCKED SYMBOL sym=%s count=%d lim=%d", sym, s_count, s_lim)
            return {"symbol": symbol, "final": None, "trade_blocked": True,
                    "trade_block_reason": f"SYMBOL_MAX:{sym}:{s_count}/{s_lim}"}
    if callable(_v18f_prev_decide):
        return _v18f_prev_decide(symbol, live)
    return {"symbol": symbol, "final": None}

globals()["make_decision_for_symbol"] = make_decision_for_symbol


_V18F_LOG.info(
    "V18 FINAL FIX COMPLETE — 3-layer protection active: "
    "MT5 proxy + place_order_mt5 wrapper + make_decision wrapper. "
    "Watchdog thread running. "
    "Limits: global=%d | GBPUSD=%d | BTCUSD=%d | USOIL=%d | USDJPY=%d | XAUUSD=%d",
    _V18F_GLOBAL_MAX,
    _V18F_SYMBOL_LIMITS["GBPUSD"],
    _V18F_SYMBOL_LIMITS["BTCUSD"],
    _V18F_SYMBOL_LIMITS["USOIL"],
...
# ══════════════════════════════════════════════════════════════════
# END V18 FINAL DEFINITIVE TRADE LIMIT FIX
# ══════════════════════════════════════════════════════════════════


# ══════════════════════════════════════════════════════════════════════════════
# BEGIN V19 KYOTO RISK & EXECUTION BEAST LAYER
# Appended after V18 Final Fix — ALL existing strategy, ML, signal, regime,
# filter, and entry logic is 100% preserved and untouched above.
# This layer ONLY adds:
#   • automatic symbol resolution (suffix variants)
#   • volatility-aware dynamic ATR stop engine
#   • symbol-aware spread filter
#   • risk gating (cash-loss estimation and rejection)
#   • absolute SL/TP price construction from distances
#   • broker minimum stop enforcement
#   • break-even protection (~1R)
#   • trailing stop management (~1.5R configurable)
#   • position manager background loop
# ══════════════════════════════════════════════════════════════════════════════

import time as _rb_time
import logging as _rb_logging
import threading as _rb_threading
import re as _rb_re

_RB_LOG = _rb_logging.getLogger("kyoto.risk_beast")
if not _RB_LOG.handlers:
    _h = _rb_logging.StreamHandler()
    _h.setFormatter(_rb_logging.Formatter("%(asctime)s %(levelname)s %(message)s"))
    _RB_LOG.addHandler(_h)
    _RB_LOG.setLevel(_rb_logging.INFO)

# ─── Per-symbol configuration ─────────────────────────────────────────────────
# All values configurable per symbol. Defaults apply when symbol not matched.

_RB_SYMBOL_CONFIG: dict = {
    "DEFAULT": {
        "risk_pct": 0.05,               # 5% equity per trade
        "atr_multiplier_min": 1.0,
        "atr_multiplier_max": 3.5,
        "spread_mult": 2.0,             # spread buffer = spread * spread_mult
        "rr_ratio": 2.0,                # TP = stop_distance * rr_ratio
        "max_allowed_spread": 50.0,     # in points
        "be_buffer_distance": 0.3,      # fraction of initial_risk_dist for BE buffer
        "trailing_start_r": 1.5,        # trail kicks in after 1.5R profit
        "trailing_distance_mult": 1.0,  # trailing distance = initial_risk * mult
        "min_stop_factor": 1.2,         # broker min stop safety multiplier
    },
    "GBPUSD": {
        "risk_pct": 0.05,
        "atr_multiplier_min": 1.0,
        ...
        "spread_mult": 1.5,
        "rr_ratio": 2.0,
        "max_allowed_spread": 4.0,      # tight FX threshold (points)
        "be_buffer_distance": 0.25,
    ...
    "XAUUSD": {
        ...
        "spread_mult": 2.5,             # wider spread buffer for gold
        "rr_ratio": 2.5,
        "max_allowed_spread": 60.0,     # gold spread threshold (points)
        "be_buffer_distance": 0.5,
        "trailing_start_r": 1.5,
    ...
    "USOIL": {
        ...
        "spread_mult": 2.0,
        "rr_ratio": 2.0,
        "max_allowed_spread": 40.0,     # oil spread threshold
        "be_buffer_distance": 0.5,
        "trailing_start_r": 1.5,
    ...
    "BTCUSD": {
        ...
        "spread_mult": 3.0,             # wide spread tolerance for crypto
        "rr_ratio": 2.0,
        "max_allowed_spread": 250.0,    # dynamic/wide BTC spread threshold
        "be_buffer_distance": 0.6,
        "trailing_start_r": 1.5,
...
# Common broker suffixes to strip when looking up config
_RB_SUFFIXES = ("M", ".M", "PRO", ".PRO", ".A", ".B", ".C", "-M", ".mm", ".m")


def _rb_get_config(symbol: str) -> dict:
    """Return per-symbol config merged with DEFAULT. Case-insensitive, suffix-aware."""
    s = str(symbol or "").upper().strip()
    # direct match
    if s in _RB_SYMBOL_CONFIG:
        return dict(_RB_SYMBOL_CONFIG["DEFAULT"], **_RB_SYMBOL_CONFIG[s])
    # strip suffix and retry
    for suf in sorted(_RB_SUFFIXES, key=len, reverse=True):
        if s.endswith(suf.upper()):
            base = s[: -len(suf)]
            if base in _RB_SYMBOL_CONFIG:
                return dict(_RB_SYMBOL_CONFIG["DEFAULT"], **_RB_SYMBOL_CONFIG[base])
    return dict(_RB_SYMBOL_CONFIG["DEFAULT"])
...
def _rb_fix_typo(symbol: str) -> str:
    s = str(symbol or "").upper().strip()
    return _RB_TYPO_MAP.get(s, s)
...
def resolve_symbol(requested: str):
    """
    Resolve a broker symbol automatically:
    1. Fix obvious typos.
    2. Try exact match via symbol_info.
    3. Search full symbol list for root-prefix broker variants.
    4. Call symbol_select on the resolved symbol.
    Returns resolved symbol name or None if not found.
    """
    try:
        import MetaTrader5 as mt5
    except ImportError:
        return requested  # no MT5, pass through

    fixed = _rb_fix_typo(requested)
    ...
    # 1. Exact match
    result = _try_select(fixed)
    if result:
        _RB_LOG.debug("resolve_symbol: exact '%s'", result)
        return result

    # 2. Case-insensitive exact search in all symbols
    try:
    ...
    for name in names:
        if name.lower() == fixed_lower:
            mt5.symbol_select(name, True)
            _RB_LOG.info("resolve_symbol: case-insensitive match '%s' for '%s'", name, requested)
            return name

    # 3. Root-prefix search (e.g. GBPUSD → GBPUSDm, GBPUSD.pro)
    root_m = _rb_re.match(r'^[A-Z0-9]+', fixed)
    root = root_m.group(0) if root_m else fixed
    candidates = [n for n in names if n.upper().startswith(root)]
    if candidates:
        # prefer shorter names (fewer suffix chars)
        chosen = sorted(candidates, key=len)[0]
        mt5.symbol_select(chosen, True)
        _RB_LOG.info("resolve_symbol: root-prefix match '%s' for '%s'", chosen, requested)
        return chosen

    _RB_LOG.warning("resolve_symbol: CANNOT RESOLVE '%s' (fixed='%s')", requested, fixed)
    return None


# ─── Live data safety ─────────────────────────────────────────────────────────

# Legacy scalar kept for any direct references elsewhere in the file.
# The symbol-aware config below supersedes it for all freshness decisions.
_RB_TICK_MAX_AGE_SEC: float = 3.0

# ── Symbol-aware tick freshness windows ──────────────────────────────────────
# Each entry: (max_age_seconds, recheck_delay_seconds)
# Recheck: if a tick is initially stale we sleep once and retry before giving up.
_RB_TICK_FRESHNESS: dict = {
    # FX majors — strict; brokers update continuously
    "GBPUSD":  (3.0, 0.25),
    "USDJPY":  (3.0, 0.25),
    "GBPUSD":  (3.0, 0.25),
    "USDCHF":  (3.0, 0.25),
    ...
    "USDCAD":  (3.0, 0.25),
    "NZDUSD":  (3.0, 0.25),
    # Gold — slightly more tolerant than FX on slower broker feeds
    "XAUUSD":  (4.0, 0.30),
    "XAGUSD":  (4.0, 0.30),
    # Oil — some brokers update in batches; give extra room
    "USOIL":   (6.0, 0.40),
    "UKOIL":   (6.0, 0.40),
    "BRENT":   (6.0, 0.40),
    # Crypto — burst-style feeds on many brokers
    # BTCUSD / BTCUSDm / BTCUSD.pro: raised to 7.0 s to tolerate slower
    # broker feeds (observed tick age ~6.2 s on BTCUSDm).  All BTC broker-
    # suffix variants are listed explicitly so both the direct lookup and the
    # suffix-stripping fallback reach the same 7.0 s limit.
    "BTCUSD":      (7.0, 0.35),   # canonical key used by suffix-stripping
    "BTCUSDm":     (7.0, 0.35),   # explicit: broker 'm' suffix
    "BTCUSD.M":    (7.0, 0.35),   # explicit: broker '.M' suffix
    "BTCUSD.PRO":  (7.0, 0.35),   # explicit: broker '.pro' / '.PRO' suffix
    "BTCUSDA":     (7.0, 0.35),   # explicit: broker '.A' suffix (some ECNs)
    "BTCUSDB":     (7.0, 0.35),   # explicit: broker '.B' suffix
    "ETHUSD":  (5.0, 0.35),
    "LTCUSD":  (5.0, 0.35),
}

# Conservative default for any symbol not listed above
_RB_TICK_FRESHNESS_DEFAULT: tuple = (4.0, 0.30)

# Common broker suffixes stripped when looking up the freshness table
_RB_FRESHNESS_SUFFIXES = (
    ".M", ".m", "M", "m", ".PRO", ".pro", "PRO",
    ".A", ".B", ".C", "-M", ".mm",
)


def _rb_tick_freshness_window(symbol: str) -> tuple:
    """
    Return (max_age_sec, recheck_delay_sec) for *symbol*.
    Strips broker suffixes and does a case-insensitive lookup.
    Falls back to _RB_TICK_FRESHNESS_DEFAULT when no match is found.
    """
    s = str(symbol or "").upper().strip()
    if s in _RB_TICK_FRESHNESS:
        return _RB_TICK_FRESHNESS[s]
    for suf in sorted(_RB_FRESHNESS_SUFFIXES, key=len, reverse=True):
        if s.endswith(suf.upper()):
            base = s[: -len(suf)]
            if base in _RB_TICK_FRESHNESS:
                return _RB_TICK_FRESHNESS[base]
    return _RB_TICK_FRESHNESS_DEFAULT


def _rb_symbol_category(symbol: str) -> str:
    """Return a human-readable category label for logging."""
    s = str(symbol or "").upper().strip()
    for suf in sorted(_RB_FRESHNESS_SUFFIXES, key=len, reverse=True):
        if s.endswith(suf.upper()):
            s = s[: -len(suf)]
            break
    if s in ("GBPUSD", "USDJPY", "GBPUSD", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD"):
        return "FX_MAJOR"
    if s in ("XAUUSD", "XAGUSD"):
        return "GOLD_SILVER"
    if s in ("USOIL", "UKOIL", "BRENT"):
        return "OIL"
    if s in ("BTCUSD", "ETHUSD", "LTCUSD"):
        # Return BTCUSDm for all BTC variants so logs clearly show the family
        if s == "BTCUSD":
            return "BTCUSDm"
        return "CRYPTO"
    return "OTHER"
...
def _rb_check_mt5_healthy() -> bool:
    """Verify MT5 connection is alive by reading account_info."""
    try:
        import MetaTrader5 as mt5
...
def _rb_get_live_tick(symbol: str):
    """
    Return a fresh tick or None.

    Symbol-aware freshness windows (replaces single global timeout):
      FX majors (GBPUSD, USDJPY ...)  : 3.0 s  strict
      Gold / Silver (XAUUSD ...)      : 4.0 s  slightly relaxed
      Oil (USOIL, BRENT ...)          : 6.0 s  tolerant (batch feeds)
      BTCUSDm / BTCUSD family         : 7.0 s  wider (broker feed ~6.2 s)
      Other crypto (ETHUSD ...)       : 5.0 s  moderate (burst feeds)
      Other                           : 4.0 s  conservative default

    Retry logic:
      If a tick is initially stale, the function sleeps for the configured
      recheck delay (200-500 ms depending on category) and fetches once more.
      Only after that second attempt is the tick rejected as stale.

    Rejects:
      - None tick
      - tick still older than the symbol freshness window after recheck
      - invalid bid / ask (<=0 or ask < bid)

    Logs symbol name, category, tick age, allowed limit, and every decision.
    """
    try:
        import MetaTrader5 as mt5

        max_age, recheck_delay = _rb_tick_freshness_window(symbol)
        category = _rb_symbol_category(symbol)

        _RB_LOG.debug(
            "live_tick: checking %s [%s] max_age=%.1fs recheck_delay=%.3fs",
            symbol, category, max_age, recheck_delay)

        # ── First fetch ───────────────────────────────────────────────────
        tick = mt5.symbol_info_tick(symbol)
        if tick is None:
            _RB_LOG.warning(
                "live_tick: NO TICK  symbol=%s category=%s",
                symbol, category)
            return None

        age = _rb_time.time() - float(tick.time)

        if age <= max_age:
            # Fresh on first attempt
            _RB_LOG.debug(
                "live_tick: FRESH    symbol=%s category=%s age=%.3fs limit=%.1fs",
                symbol, category, age, max_age)
        else:
            # ── Initially stale — recheck once ───────────────────────────
            _RB_LOG.info(
                "live_tick: INITIAL STALE  symbol=%s category=%s "
                "age=%.3fs limit=%.1fs — rechecking in %.3fs",
                symbol, category, age, max_age, recheck_delay)

            _rb_time.sleep(recheck_delay)

            tick = mt5.symbol_info_tick(symbol)
            if tick is None:
                _RB_LOG.warning(
                    "live_tick: RECHECK NO TICK  symbol=%s category=%s",
                    symbol, category)
                return None
            ...
            if age > max_age:
                _RB_LOG.warning(
                    "live_tick: RECHECK STALE — SKIP  symbol=%s category=%s "
                    "age=%.3fs limit=%.1fs  (recheck confirmed stale)",
                    symbol, category, age, max_age)
                return None

            _RB_LOG.info(
                "live_tick: RECHECK FRESH — CONTINUE  symbol=%s category=%s "
                "age=%.3fs limit=%.1fs  (recovered after recheck)",
                symbol, category, age, max_age)
        ...
        if bid <= 0 or ask <= 0:
            _RB_LOG.warning(
                "live_tick: INVALID BID/ASK  symbol=%s category=%s "
                "bid=%.5f ask=%.5f",
                symbol, category, bid, ask)
            return None

        if ask < bid:
            _RB_LOG.warning(
                "live_tick: ASK<BID ANOMALY  symbol=%s category=%s "
                "bid=%.5f ask=%.5f",
                symbol, category, bid, ask)
            return None

        _RB_LOG.debug(
            "live_tick: ACCEPTED  symbol=%s category=%s "
            "bid=%.5f ask=%.5f age=%.3fs",
            symbol, category, bid, ask, age)
        return tick

    except Exception as e:
        _RB_LOG.warning("live_tick: EXCEPTION  symbol=%s: %s", symbol, e)
        return None


# ─── Broker minimum stop buffer ───────────────────────────────────────────────

def _rb_minimum_symbol_buffer(symbol: str, symbol_info) -> float:
    """
    Compute broker minimum stop distance using real MT5 metadata:
      • trade_stops_level * point (+ 5% safety)
      • fallback: 10 * point
    Never guesses when metadata is available.
    """
    try:
        ...
        effective    = max(stops_level, freeze_level)
        if effective > 0 and point > 0:
            return effective * point * 1.05   # +5% safety margin
        if point > 0:
            return point * 10.0               # fallback: 10 points
        return 0.0
    except Exception as e:
        _RB_LOG.debug("min_symbol_buffer error %s: %s", symbol, e)
        return 0.0


# ─── Loss estimation ──────────────────────────────────────────────────────────

def estimate_loss_cash(symbol: str, volume: float, stop_distance: float,
                       side: str = "BUY", entry_price: float = None):
    """
    Estimate real cash loss at stop-loss in account currency.

    Primary:  mt5.order_calc_profit  (handles FX, gold, oil, crypto, JPY pairs naturally)
    Fallback: tick_value / tick_size metadata
    Returns absolute loss (positive number = loss) or None on failure.
    """
    try:
        import MetaTrader5 as mt5
    except ImportError:
    ...
    # ── Primary: order_calc_profit ──────────────────────────────────────────
    try:
        ep = float(entry_price or 0.0)
        if ep <= 0:
            tick = mt5.symbol_info_tick(symbol)
            ep = float(tick.ask if side_u == "BUY" else tick.bid) if tick else 0.0
        ...
        if ep > 0:
            ...
            profit = mt5.order_calc_profit(action, symbol, vol, ep, stop_price)
            if profit is not None:
                loss = abs(float(profit))
                _RB_LOG.debug("estimate_loss: PRIMARY %s vol=%.4f dist=%.5f loss=%.4f", symbol, vol, dist, loss)
                return loss
    except Exception as e:
        _RB_LOG.debug("estimate_loss: primary failed %s: %s", symbol, e)

    # ── Fallback: tick-value metadata ──────────────────────────────────────
    try:
        si = mt5.symbol_info(symbol)
        if si is None:
            _RB_LOG.warning("estimate_loss: NO symbol_info for %s — skipping trade", symbol)
            return None
        ...
        if tick_value <= 0 or tick_size <= 0:
            _RB_LOG.warning("estimate_loss: CRITICAL metadata missing for %s — skipping", symbol)
            return None

        ticks = dist / tick_size
        loss  = abs(ticks * tick_value * vol)
        _RB_LOG.warning("estimate_loss: FALLBACK method %s loss=%.4f (tick_value=%.6f tick_size=%.6f)",
                         symbol, loss, tick_value, tick_size)
        return loss
    except Exception as e:
        _RB_LOG.warning("estimate_loss: fallback failed %s: %s", symbol, e)
        return None


# ─── Dynamic ATR multiplier ───────────────────────────────────────────────────

def _rb_dynamic_atr_mult(atr_now: float, atr_avg, cfg: dict) -> float:
    """
    Volatility-aware ATR multiplier:
      calm (ratio < 0.8)  → min multiplier
      normal (0.8–1.3)    → linear interpolation
      volatile (> 1.3)    → max multiplier
    Clamped to [atr_multiplier_min, atr_multiplier_max].
    """
    lo = float(cfg.get("atr_multiplier_min", 1.0))
    hi = float(cfg.get("atr_multiplier_max", 3.5))
    try:
        an = float(atr_now)
        aa = float(atr_avg) if atr_avg else 0.0
        if aa > 0:
            ratio = an / aa
            if ratio < 0.8:
                mult = lo
...
def calc_trade_plan(symbol: str, tick, atr_now: float, atr_avg,
                    equity: float, side: str, symbol_info):
    """
    Compute a full trade plan using live data and dynamic stop logic.
    Returns a dict on success or None on rejection (with logged reason).

    Stop distance = max(atr_buffer, spread_buffer, symbol_min_buffer)
    Spread filter applied here using max_allowed_spread per symbol.
    Risk gating: estimated_loss must be <= equity * risk_pct.
    """
    cfg = _rb_get_config(symbol)
    ...
    ask   = float(tick.ask)
    side_u = str(side or "BUY").upper()
    entry_price = ask if side_u == "BUY" else bid
    spread      = ask - bid

    # ── Spread filter ──────────────────────────────────────────────────────
    point = float(getattr(symbol_info, "point", 1e-5) or 1e-5) if symbol_info else 1e-5
    spread_in_pts = spread / point if point > 0 else spread
    max_allowed_spread = float(cfg["max_allowed_spread"])
    if spread_in_pts > max_allowed_spread:
        _RB_LOG.warning(
            "calc_trade_plan: REJECT %s spread=%.2f pts > max=%.2f pts",
            symbol, spread_in_pts, max_allowed_spread)
        return None

    # ── Dynamic stop distance ──────────────────────────────────────────────
    atr_mult     = _rb_dynamic_atr_mult(atr_now, atr_avg, cfg)
    spread_mult  = float(cfg["spread_mult"])
    rr_ratio     = float(cfg["rr_ratio"])

    atr_buffer    = float(atr_now) * atr_mult
    spread_buffer = spread * spread_mult
    symbol_buffer = _rb_minimum_symbol_buffer(symbol, symbol_info)
    stop_distance = max(atr_buffer, spread_buffer, symbol_buffer)

    if stop_distance <= 0:
        _RB_LOG.warning("calc_trade_plan: REJECT %s stop_distance=0", symbol)
        return None

    # ── Lot size (default 0.01, validated against broker constraints) ──────
    lot = 0.01
    if symbol_info:
        vol_min  = float(getattr(symbol_info, "volume_min",  0.01) or 0.01)
        vol_step = float(getattr(symbol_info, "volume_step", 0.01) or 0.01)
        vol_max  = float(getattr(symbol_info, "volume_max",  1e9)  or 1e9)
        lot = max(vol_min, lot)
        if vol_step > 0:
            steps = round((lot - vol_min) / vol_step)
            lot   = vol_min + steps * vol_step
        lot = round(min(lot, vol_max), 8)
        if lot < vol_min:
            _RB_LOG.warning("calc_trade_plan: REJECT %s invalid lot=%.8f < vol_min=%.8f", symbol, lot, vol_min)
            return None

    # ── Risk gating: cash loss estimate ───────────────────────────────────
    estimated_loss = estimate_loss_cash(symbol, lot, stop_distance,
                                        side=side_u, entry_price=entry_price)
    if estimated_loss is None:
        _RB_LOG.warning("calc_trade_plan: REJECT %s cannot estimate loss", symbol)
        return None

    if estimated_loss > risk_cash:
        _RB_LOG.warning(
            "calc_trade_plan: REJECT %s est_loss=%.4f > risk_cash=%.4f "
            "(equity=%.2f risk_pct=%.4f)",
            symbol, estimated_loss, risk_cash, equity, risk_pct)
        return None

    _RB_LOG.info(
        "calc_trade_plan: ACCEPT %s side=%s entry=%.5f spread=%.5f(%.2fpts) "
        "atr_now=%.5f atr_avg=%s atr_mult=%.3f spread_mult=%.2f "
        "stop_dist=%.5f tp_dist=%.5f rr=%.2f lot=%.4f est_loss=%.4f risk_cash=%.4f",
        symbol, side_u, entry_price, spread, spread_in_pts,
        atr_now, f"{float(atr_avg):.5f}" if atr_avg else "N/A",
        atr_mult, spread_mult,
        stop_distance, stop_distance * rr_ratio,
        rr_ratio, lot, estimated_loss, risk_cash)
...
# ─── Absolute SL/TP price construction ───────────────────────────────────────

def build_sl_tp_prices(entry_price: float, side: str,
                       stop_distance: float, rr_ratio: float) -> tuple:
    """
    Convert stop distance to absolute SL/TP prices. Never uses raw distances.
    BUY:  sl = entry - dist,  tp = entry + dist * rr
    SELL: sl = entry + dist,  tp = entry - dist * rr
    Returns (sl_price, tp_price).
    """
    ep     = float(entry_price)
    dist   = float(stop_distance)
    tp_dist = dist * float(rr_ratio)
    if str(side).upper() == "BUY":
        return ep - dist, ep + tp_dist
    else:
        return ep + dist, ep - tp_dist
...
def enforce_broker_min_stop(symbol: str, tick, entry_price: float,
                             sl_price: float, tp_price: float,
                             side: str, symbol_info) -> tuple:
    """
    Adjust SL and TP only if they violate broker minimum stop distance.
    Rules:
      • BUY:  SL < entry, TP > entry
      • SELL: SL > entry, TP < entry
      • Never move SL backward (never loosen protection).
      • Returns (None, None) if a safe fix is impossible.
    """
    try:
        if symbol_info is None:
            return sl_price, tp_price

        point        = float(getattr(symbol_info, "point",              1e-5) or 1e-5)
        stops_level  = float(getattr(symbol_info, "trade_stops_level",  0)   or 0)
        freeze_level = float(getattr(symbol_info, "trade_freeze_level", 0)   or 0)
        broker_min   = max(stops_level, freeze_level) * point
        ...
        cfg    = _rb_get_config(symbol)
        safety = broker_min * float(cfg.get("min_stop_factor", 1.2))
        ...
        if broker_min > 0:
            if side_u == "BUY":
                ...
                if sl_dist < safety:
                    new_sl = ep - safety
                    # never loosen: new_sl must not be higher than current sl
                    sl = min(sl, new_sl)
                    _RB_LOG.info("enforce_broker_min: %s BUY SL adjusted to %.5f (min=%.5f)", symbol, sl, safety)
                if tp_dist < safety:
                    tp = ep + max(tp_dist, safety * 2.0)
            else:
                sl_dist = sl - ep
                tp_dist = ep - tp
                if sl_dist < safety:
                    new_sl = ep + safety
                    # never loosen: new_sl must not be lower than current sl
                    sl = max(sl, new_sl)
                    _RB_LOG.info("enforce_broker_min: %s SELL SL adjusted to %.5f (min=%.5f)", symbol, sl, safety)
                if tp_dist < safety:
                    tp = ep - max(tp_dist, safety * 2.0)

        # Final direction sanity check — reject if relationship broken
        if side_u == "BUY":
            if sl >= ep or tp <= ep:
                _RB_LOG.warning(
                    "enforce_broker_min: INVALID BUY %s sl=%.5f ep=%.5f tp=%.5f", symbol, sl, ep, tp)
                return None, None
        else:
            if sl <= ep or tp >= ep:
                _RB_LOG.warning(
                    "enforce_broker_min: INVALID SELL %s sl=%.5f ep=%.5f tp=%.5f", symbol, sl, ep, tp)
                return None, None

        return sl, tp
    except Exception as e:
        _RB_LOG.warning("enforce_broker_min: exception %s: %s", symbol, e)
        return None, None


# ─── Break-even protection ────────────────────────────────────────────────────

def should_arm_breakeven(position, tick, initial_risk_distance: float, side: str) -> bool:
    """Return True when price has moved ≥1R in favor (with 5% tolerance)."""
    try:
        ep     = float(getattr(position, "price_open", 0.0))
        side_u = str(side).upper()
        if side_u == "BUY":
            profit_dist = float(tick.bid) - ep
        else:
            profit_dist = ep - float(tick.ask)
        return profit_dist >= float(initial_risk_distance) * 0.95
    except Exception:
        return False


def compute_breakeven_sl(position, tick, side: str,
                         buffer_distance: float, symbol_info):
    """
    Compute a break-even SL = entry + tiny buffer.
    Buffer is symbol-aware and broker-safe.
    Returns new SL price or None if already protected.
    """
    try:
        ep         = float(getattr(position, "price_open", 0.0))
        ...
        point       = float(getattr(symbol_info, "point",             1e-5) or 1e-5) if symbol_info else 1e-5
        stops_level = float(getattr(symbol_info, "trade_stops_level", 0)   or 0)   if symbol_info else 0
        min_buf     = max(float(buffer_distance),
                          stops_level * point * 1.1,
                          point * 2)

        if side_u == "BUY":
            new_sl = ep + min_buf
            if current_sl > 0 and new_sl <= current_sl:
                return None   # already at or above break-even
            return new_sl
        else:
            new_sl = ep - min_buf
            if current_sl > 0 and new_sl >= current_sl:
        ...
        _RB_LOG.debug("compute_breakeven_sl error: %s", e)
        return None


# ─── Trailing stop ────────────────────────────────────────────────────────────

def compute_trailing_sl(position, tick, side: str,
                        trailing_distance: float, symbol_info):
    """
    Compute trailing SL candidate.
    BUY:  candidate only accepted if > current SL (tightening only).
    SELL: candidate only accepted if < current SL (tightening only).
    """
    try:
        current_sl = float(getattr(position, "sl", 0.0))
        side_u     = str(side).upper()
        ...
        point       = float(getattr(symbol_info, "point",             1e-5) or 1e-5) if symbol_info else 1e-5
        stops_level = float(getattr(symbol_info, "trade_stops_level", 0)   or 0)   if symbol_info else 0
        min_dist    = max(float(trailing_distance), stops_level * point * 1.1)

        if side_u == "BUY":
            live_price    = float(tick.bid)
            candidate_sl  = live_price - min_dist
            # only move SL upward
        ...
        _RB_LOG.debug("compute_trailing_sl error: %s", e)
        return None


# ─── Position SL modification ─────────────────────────────────────────────────

def modify_position_sl(ticket: int, new_sl: float) -> bool:
    """
    Modify a position's SL via MT5 TRADE_ACTION_SLTP, preserving TP.
    Logs success or failure. Never crashes on API failure.
    """
    try:
        import MetaTrader5 as mt5
        ...
        if not positions:
            _RB_LOG.warning("modify_position_sl: no open positions for ticket=%d", ticket)
            return False
        ...
        if pos is None:
            _RB_LOG.warning("modify_position_sl: ticket=%d not found", ticket)
            return False
        ...
        if retcode in (10009, 0):   # TRADE_RETCODE_DONE
            _RB_LOG.info("modify_position_sl: SUCCESS ticket=%d symbol=%s new_sl=%.5f",
                         ticket, symbol, new_sl)
            return True
        else:
            _RB_LOG.warning("modify_position_sl: FAILED ticket=%d retcode=%s result=%s",
                            ticket, retcode, result)
            return False
    except Exception as e:
        _RB_LOG.warning("modify_position_sl: exception ticket=%d: %s", ticket, e)
        return False


# ─── Position manager state ───────────────────────────────────────────────────

_RB_BE_ARMED: dict  = {}   # ticket (int) → True once break-even is armed
_RB_BE_LOCK         = _rb_threading.Lock()


# ─── Position manager loop ────────────────────────────────────────────────────

def _rb_position_manager_loop(poll_interval: float = 5.0):
    """
    Background daemon loop managing break-even and trailing stops for all
    open positions. Never crashes the thread.
    """
    _RB_LOG.info("position_manager_loop: STARTED (poll=%.1fs)", poll_interval)

    while True:
        try:
            _rb_time.sleep(poll_interval)

            if not _rb_check_mt5_healthy():
                _RB_LOG.debug("position_manager_loop: MT5 not healthy — skip")
                continue
            ...
            for pos in positions:
                try:
                    ...
                    symbol     = str(getattr(pos, "symbol",      ""))
                    pos_type   = int(getattr(pos, "type",        0))   # 0=BUY 1=SELL
                    ...
                    if not resolved:
                        _RB_LOG.debug("pos_mgr: skip %s (unresolvable)", symbol)
                        continue

                    # B. Live tick with staleness check
                    tick = _rb_get_live_tick(resolved)
                    if tick is None:
                        _RB_LOG.debug("pos_mgr: skip %s (stale tick)", symbol)
                        continue

                    # C. symbol_info for broker constraints
                    si = mt5.symbol_info(resolved)

                    # D. Initial risk distance from entry → current SL
                    if current_sl > 0:
                        initial_risk_dist = abs(entry_price - current_sl)
                    else:
                        # conservative fallback: 1% of price
                        initial_risk_dist = entry_price * 0.01
                    ...
                    with _RB_BE_LOCK:
                        be_armed = _RB_BE_ARMED.get(ticket, False)

                    # ── Break-even check ─────────────────────────────────
                    if not be_armed:
                        if should_arm_breakeven(pos, tick, initial_risk_dist, side):
                            buf_frac = float(cfg.get("be_buffer_distance", 0.3))
                            buf_dist = initial_risk_dist * buf_frac
                            be_sl    = compute_breakeven_sl(pos, tick, side, buf_dist, si)
                            if be_sl is not None:
                                be_sl, _ = enforce_broker_min_stop(
                                    resolved, tick, entry_price,
                                    be_sl, float(getattr(pos, "tp", entry_price)),
                                    side, si)
                                if be_sl is not None:
                                    if modify_position_sl(ticket, be_sl):
                                        with _RB_BE_LOCK:
                                            _RB_BE_ARMED[ticket] = True
                                        _RB_LOG.info(
                                            "pos_mgr: BE ARMED ticket=%d %s %s sl=%.5f",
                                            ticket, symbol, side, be_sl)
                                    else:
                                        _RB_LOG.warning(
                                            "pos_mgr: BE modify FAILED ticket=%d %s",
                                            ticket, symbol)

                    # ── Trailing stop check (only after BE is armed) ──────
                    with _RB_BE_LOCK:
                        be_armed_now = _RB_BE_ARMED.get(ticket, False)

                    if be_armed_now:
                        if side == "BUY":
                            profit_dist = float(tick.bid) - entry_price
                        else:
                            profit_dist = entry_price - float(tick.ask)

                        trailing_start_r = float(cfg.get("trailing_start_r", 1.5))
                        if profit_dist >= initial_risk_dist * trailing_start_r:
                            trail_mult = float(cfg.get("trailing_distance_mult", 1.0))
                            trail_dist = initial_risk_dist * trail_mult
                            trail_sl   = compute_trailing_sl(pos, tick, side, trail_dist, si)
                            if trail_sl is not None:
                                trail_sl, _ = enforce_broker_min_stop(
                                    resolved, tick, entry_price,
                                    trail_sl, float(getattr(pos, "tp", entry_price)),
                                    side, si)
                                if trail_sl is not None:
                                    if modify_position_sl(ticket, trail_sl):
                                        _RB_LOG.info(
                                            "pos_mgr: TRAIL updated ticket=%d %s %s "
                                            "sl=%.5f profit_r=%.2f",
                                            ticket, symbol, side, trail_sl,
                                            profit_dist / initial_risk_dist)

                except Exception as inner_e:
                    _RB_LOG.debug("pos_mgr: per-position error: %s", inner_e)
                    continue

        except Exception as outer_e:
            _RB_LOG.warning("pos_mgr: outer loop error: %s", outer_e)
            continue


def _rb_start_position_manager(poll_interval: float = 5.0):
    """Launch position manager as a daemon thread. Safe to call once."""
    t = _rb_threading.Thread(
        target=_rb_position_manager_loop,
        args=(poll_interval,),
        name="kyoto_risk_beast_pos_mgr",
        daemon=True,
    )
    t.start()
    _RB_LOG.info("position_manager_loop: daemon thread launched")
    return t


# ─── Primary execution entry point ───────────────────────────────────────────

def kyoto_execute_trade(symbol: str, side: str,
                        atr_now: float, atr_avg=None) -> dict:
    """
    Full production-safe execution flow:
    A. Resolve symbol (suffix-aware, typo-corrected).
    B. Check MT5 connection health.
    C. Read live tick, reject if stale.
    D. Read symbol_info and account_info, reject if missing.
    E. Compute trade plan (stop, lot, risk gate).
    F. Reject if estimated loss > risk budget.
    G. Convert stop distance to absolute SL/TP prices by side.
    H. Enforce broker minimum stop distance.
    I. Reject if final SL/TP are invalid.
    J. Send order with final absolute SL/TP via existing execution chain.
    """
    _RB_LOG.info("kyoto_execute_trade: START symbol=%s side=%s atr_now=%.5f atr_avg=%s",
                 symbol, side, atr_now, f"{float(atr_avg):.5f}" if atr_avg else "N/A")

    # A. Resolve symbol
    resolved = resolve_symbol(symbol)
    if not resolved:
        _RB_LOG.warning("kyoto_execute_trade: SKIP %s — unresolvable", symbol)
        return {"status": "rejected", "reason": "symbol_unresolvable", "symbol": symbol}

    # B. MT5 health
    if not _rb_check_mt5_healthy():
        _RB_LOG.warning("kyoto_execute_trade: SKIP %s — MT5 not healthy", resolved)
        return {"status": "rejected", "reason": "mt5_not_healthy", "symbol": resolved}

    # C. Live tick
    tick = _rb_get_live_tick(resolved)
    if tick is None:
        # _rb_get_live_tick already logged the full stale-tick detail
        # (symbol, category, age, limit, first-check-failed, recheck result).
        # Use debug here to avoid a duplicate WARNING line for BTCUSDm rejections.
        _RB_LOG.debug("kyoto_execute_trade: SKIP %s — stale/missing tick (see live_tick log above)", resolved)
        return {"status": "rejected", "reason": "stale_tick", "symbol": resolved}
    ...
    try:
        ...
        si = mt5.symbol_info(resolved)
        if si is None:
            _RB_LOG.warning("kyoto_execute_trade: SKIP %s — no symbol_info", resolved)
            return {"status": "rejected", "reason": "no_symbol_info", "symbol": resolved}

        trade_mode = getattr(si, "trade_mode", 1)
        if trade_mode == 0:
            _RB_LOG.warning("kyoto_execute_trade: SKIP %s — symbol inactive (trade_mode=0)", resolved)
            return {"status": "rejected", "reason": "symbol_inactive", "symbol": resolved}

        acc = mt5.account_info()
        if acc is None:
            _RB_LOG.warning("kyoto_execute_trade: SKIP %s — no account_info", resolved)
            return {"status": "rejected", "reason": "no_account_info", "symbol": resolved}

        equity = float(getattr(acc, "equity", 0.0) or 0.0)
        if equity <= 0:
            _RB_LOG.warning("kyoto_execute_trade: SKIP %s — invalid equity=%.2f", resolved, equity)
            return {"status": "rejected", "reason": "invalid_equity", "symbol": resolved}

    except Exception as e:
        _RB_LOG.warning("kyoto_execute_trade: SKIP %s — metadata error: %s", resolved, e)
        return {"status": "rejected", "reason": f"metadata_error:{e}", "symbol": resolved}

    # E/F. Trade plan + risk gate
    plan = calc_trade_plan(resolved, tick, atr_now, atr_avg, equity, side, si)
    if plan is None:
        return {"status": "rejected", "reason": "plan_rejected", "symbol": resolved}

    # G. Absolute SL/TP prices
    sl_price, tp_price = build_sl_tp_prices(
        plan["entry_price"], side,
        plan["sl_distance"], plan["rr_ratio"])

    _RB_LOG.info("kyoto_execute_trade: ABS prices %s entry=%.5f sl=%.5f tp=%.5f",
                 resolved, plan["entry_price"], sl_price, tp_price)

    # H. Enforce broker min stop
    sl_price, tp_price = enforce_broker_min_stop(
        resolved, tick, plan["entry_price"],
        sl_price, tp_price, side, si)

    # I. Validate final SL/TP
    if sl_price is None or tp_price is None:
        _RB_LOG.warning("kyoto_execute_trade: SKIP %s — SL/TP invalid after broker enforcement", resolved)
        return {"status": "rejected", "reason": "sl_tp_invalid", "symbol": resolved}

    # J. Send order via existing chain (V18F limits already active)
    _place_fn = globals().get("place_order_mt5")
    if not callable(_place_fn):
        _RB_LOG.warning("kyoto_execute_trade: no callable place_order_mt5 found")
        return {"status": "rejected", "reason": "no_executor", "symbol": resolved}

    _RB_LOG.info(
        "kyoto_execute_trade: SENDING %s side=%s lot=%.4f entry=%.5f "
        "sl=%.5f tp=%.5f est_loss=%.4f risk_cash=%.4f",
        resolved, side, plan["lot"], plan["entry_price"],
        sl_price, tp_price, plan["estimated_loss"], plan["risk_cash"])

    try:
        result = _place_fn(resolved, side, plan["lot"],
                           plan["entry_price"], sl_price, tp_price)
        _RB_LOG.info("kyoto_execute_trade: RESULT %s → %s", resolved, result)
        return {
            "status":    "sent",
            "symbol":    resolved,
            "side":      side,
        ...
        _RB_LOG.warning("kyoto_execute_trade: send exception %s: %s", resolved, e)
        return {"status": "error", "reason": str(e), "symbol": resolved}


# ─── Auto-start position manager ──────────────────────────────────────────────

if not globals().get("_RB_POSITION_MANAGER_STARTED"):
    try:
        _rb_pm_thread = _rb_start_position_manager(poll_interval=5.0)
        globals()["_RB_POSITION_MANAGER_STARTED"] = True
        globals()["_rb_pm_thread"] = _rb_pm_thread
    except Exception as _rb_init_err:
        _RB_LOG.warning("Failed to start position manager: %s", _rb_init_err)

_RB_LOG.info(
    "V19 KYOTO RISK & EXECUTION BEAST LAYER ACTIVE — "
    "auto_symbol_resolution | spread_filter | dynamic_atr_stops | "
    "risk_gating | abs_sl_tp | broker_min_stop | break_even_1R | "
    "trailing_1.5R | position_manager_running"
)

# ══════════════════════════════════════════════════════════════════════════════
# END V19 KYOTO RISK & EXECUTION BEAST LAYER
# ══════════════════════════════════════════════════════════════════════════════
