...
try:
    SYMBOL_TRADE_LIMITS = {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}
    GLOBAL_MAX_OPEN_TRADES = 8
    globals()["SYMBOL_TRADE_LIMITS"] = dict(SYMBOL_TRADE_LIMITS)
    globals()["GLOBAL_MAX_OPEN_TRADES"] = int(GLOBAL_MAX_OPEN_TRADES)
except Exception:
...
def _kyoto_absolute_symbol(symbol):
    ...
    for suffix in (".M", "-M"):
        if s.endswith(suffix):
            s = s[: -len(suffix)]
    if s.endswith("M") and len(s) > 1 and s[:-1] in {"BTCUSD", "USOIL", "USDJPY", "EURUSD", "XAUUSD"}:
        s = s[:-1]
    return s
...
def _kyoto_absolute_guard(symbol, reserve=False):
    try:
        ...
        with lock:
            ...
            gmax = int(globals().get("GLOBAL_MAX_OPEN_TRADES", 8))
            limits = dict(globals().get("SYMBOL_TRADE_LIMITS", {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}))
...
try:
    globals()["SYMBOL_TRADE_LIMITS"] = {"BTCUSD": 3, "USOIL": 3, "USDJPY": 10, "EURUSD": 10, "XAUUSD": 2}
    globals()["GLOBAL_MAX_OPEN_TRADES"] = 8
except Exception:
    pass

try:
    logger.info("FINAL ABSOLUTE TRADE LIMIT LOCKDOWN installed: global=8, BTCUSD=3, USOIL=3, USDJPY=10, EURUSD=10, XAUUSD=2")
except Exception:
    pass
...
def _v18f_install():
    ...
    _V18F_LOG.info(
        ...
        "global=%d EURUSD=%d BTCUSD=%d USOIL=%d USDJPY=%d XAUUSD=%d",
        replaced, _V18F_GLOBAL_MAX,
        _V18F_SYMBOL_LIMITS["EURUSD"],
...
_V18F_LOG.info(
    ...
    "Limits: global=%d | EURUSD=%d | BTCUSD=%d | USOIL=%d | USDJPY=%d | XAUUSD=%d",
    _V18F_GLOBAL_MAX,
    _V18F_SYMBOL_LIMITS["EURUSD"],
    _V18F_SYMBOL_LIMITS["BTCUSD"],
    _V18F_SYMBOL_LIMITS["USOIL"],
...
def resolve_symbol(requested: str):
    ...
    # 3. Root-prefix search (e.g. EURUSD → EURUSDm, EURUSD.pro)
...
def _rb_get_live_tick(symbol: str):
    ...
    Rejects:
      • None tick
      • tick older than _RB_TICK_MAX_AGE_SEC seconds
      • invalid bid/ask
    """
    try:
        import MetaTrader5 as mt5
        ...
        if tick is None:
            _RB_LOG.warning("live_tick: no tick for %s", symbol)
            return None
        age = _rb_time.time() - float(tick.time)
        if age > _RB_TICK_MAX_AGE_SEC:
            _RB_LOG.warning("live_tick: STALE tick %s (age=%.1fs)", symbol, age)
            return None
        if float(tick.bid) <= 0 or float(tick.ask) <= 0:
            _RB_LOG.warning("live_tick: invalid bid/ask %s bid=%.5f ask=%.5f", symbol, tick.bid, tick.ask)
            return None
        if float(tick.ask) < float(tick.bid):
            _RB_LOG.warning("live_tick: ask<bid anomaly %s bid=%.5f ask=%.5f", symbol, tick.bid, tick.ask)
            return None
        return tick
    except Exception as e:
        _RB_LOG.warning("live_tick: exception %s: %s", symbol, e)
        return None
