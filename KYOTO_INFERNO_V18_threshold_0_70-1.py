...
CONFIG = {
    ...
    "DRY_RUN_FLAG": False,
    "EXECUTION_SIGNAL_THRESHOLD": 0.70,
    "BACKTEST_DAYS": 7,
}
...
CONFIG["BACKTEST_PARAMS"].update({
    "BTCUSD": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0012, "max_hold": 60},
    "BTCUSDm": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0012, "max_hold": 60},

    "EURUSD": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0008, "max_hold": 60},
    "EURUSDm": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0008, "max_hold": 60},

    "USDJPY": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0009, "max_hold": 60},
    "USDJPYm": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0009, "max_hold": 60},
    ...
    "USOIL": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0010, "max_hold": 60},
    "USOILm": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0010, "max_hold": 60},

    "DXY": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0008, "max_hold": 60},
    "DXYm": {"signal_thresh": 0.70, "atr_pct_thresh": 0.0008, "max_hold": 60},
})
...
try:
    if "CONFIG" in globals():
        CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = max(float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.70)), 0.70)
except Exception:
    pass
...
try:
    CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.70))
except Exception:
    pass
...
try:
    CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = 0.70
except Exception:
    pass
...
def execute_signal(sym, signal, price, mt5_module, symbol_map):
    try:
        ...
        threshold = float(params.get("signal_thresh", globals().get("CONFIG", {}).get("EXECUTION_SIGNAL_THRESHOLD", 0.70) if isinstance(globals().get("CONFIG", {}), dict) else 0.70))
        if abs(signal) < threshold:
            logger.info("Execution skipped for %s: signal below execution threshold (%.3f) signal=%.4f", sym, threshold, signal)
            return None
