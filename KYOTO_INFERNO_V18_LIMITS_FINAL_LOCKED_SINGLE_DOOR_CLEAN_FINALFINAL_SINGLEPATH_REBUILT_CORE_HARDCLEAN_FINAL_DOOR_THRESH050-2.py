...
try:
    if "CONFIG" in globals():
        CONFIG["EXECUTION_SIGNAL_THRESHOLD"] = max(float(CONFIG.get("EXECUTION_SIGNAL_THRESHOLD", 0.50)), 0.50)
except Exception:
    pass
...
def execute_signal(sym, signal, price, mt5_module, symbol_map):
    try:
        ...
        threshold = float(params.get("signal_thresh", globals().get("CONFIG", {}).get("EXECUTION_SIGNAL_THRESHOLD", 0.50) if isinstance(globals().get("CONFIG", {}), dict) else 0.50))
        if abs(signal) < threshold:
            logger.info("Execution skipped for %s: signal below execution threshold (%.3f) signal=%.4f", sym, threshold, signal)
            return None
        ok, reason = allowed_to_open(sym_u)
