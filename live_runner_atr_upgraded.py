...
Notes:
 - Default DRY_RUN is False per your request, but the script will *force* DRY_RUN
   if it cannot detect the account/server looks like a demo/trial (safety).
 - Writes trades to live_trades_atr.csv (one row per simulated/placed trade).
 - Kill switch: create an empty file named "kill_live.txt" OR "KILL_NOW" in same folder to stop loop.
"""
