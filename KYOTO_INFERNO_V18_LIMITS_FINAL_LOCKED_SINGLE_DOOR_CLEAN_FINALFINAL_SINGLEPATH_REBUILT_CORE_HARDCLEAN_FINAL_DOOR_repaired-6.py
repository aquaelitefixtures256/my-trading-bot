...
def _kyoto_final_send_request(mt5_module, req):
    ...
    try:
        ...
        original_side = str(req.get("type", "")).lower().strip()
        ...
        if req.get("price") in (None, 0, 0.0):
            ...
            try:
                ...
                tick = None
            if tick is not None:
                if original_side in {"buy", "long"} and hasattr(tick, "ask"):
                    req["price"] = float(tick.ask)
                elif original_side in {"sell", "short"} and hasattr(tick, "bid"):
        ...
        if hasattr(res, "_asdict"):
            try:
                ...
                pass
        return {"raw": str(res), "request": req}
    except Exception as e:
        try:
