...
def load_bot_module():
    if not BOT_PATH.exists():
        raise FileNotFoundError(f"Main bot file not found: {BOT_PATH}")
    spec = importlib.util.spec_from_file_location("kyoto_main_system", str(BOT_PATH))
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not load main bot module spec")
...
def main() -> int:
    ...
    # Determine mapped symbol through the main system if possible.
    ...
    # Grab the MT5 module from the bot if present; otherwise import it directly.
    ...
    # Make sure symbol is selected and price is available.
    print("\n5) Symbol/tick check")
    try:
    ...
    try:
        # Prefer the main system's own live execution path if present.
        if hasattr(bot, "UVXExecutionEngine"):
            engine = bot.UVXExecutionEngine()
            # Make sure the engine is in live mode if the class supports it.
            try:
            ...
            # Final fallback: direct MT5 send using same request
